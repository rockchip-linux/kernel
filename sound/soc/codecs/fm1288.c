/*
 * fm1288.c  --  FM1288 Voice Processor Codec driver
 *
 * Copyright 2016 Fortemedia Corp.
 * Author: Yeevan <showy.zhang@rock-chips.com>
 *
 * This program is free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License version 2 as
 * published by the Free Software Foundation.
 */

#include <linux/module.h>
#include <linux/moduleparam.h>
#include <linux/init.h>
#include <linux/delay.h>
#include <linux/pm.h>
#include <linux/i2c.h>
#include <linux/slab.h>
#include <linux/gpio.h>
#include <linux/of_gpio.h>

static int fm1288_i2c_probe(struct i2c_client *i2c_client,
			    const struct i2c_device_id *id)
{
	int ret = -1;
	struct device_node *np = i2c_client->dev.of_node;
	enum of_gpio_flags flags;
	int fm1288_ctl_gpio;

	unsigned char reg[820] = {
		
	};

	fm1288_ctl_gpio = of_get_named_gpio_flags(np, "pwd-con-gpio",
						  0, &flags);
	if (fm1288_ctl_gpio < 0) {
		pr_err("%s Can not read property fm1288 pwd-con-gpio\n",
		       __func__);
		fm1288_ctl_gpio = -1;
	} else {
		pr_debug("fm1288 pwd-con-gpio = %d\n", fm1288_ctl_gpio);
		ret = gpio_request(fm1288_ctl_gpio, NULL);
		if (!ret) {
			pr_debug("requset pwd-con-gpio success!:%d\n", ret);
			gpio_direction_output(fm1288_ctl_gpio, 1);
		} else {
			pr_err("requset pwd-con-gpio failed:%d\n", ret);
			return ret;
		}
	}

	/* To ensure power up completely before i2c trigger,
	   increase the delay of a period of time if necessary */
	/* msleep(200); */

	ret = i2c_master_send(i2c_client, reg, 820);
	if (ret != 820) {
		pr_err("fm1288 probe error\n");
		return ret;
	}
	return 0;
}

static const struct i2c_device_id fm1288_i2c_id[] = {
	{"fm1288", 0},
	{ }
};
MODULE_DEVICE_TABLE(i2c, fm1288_i2c_id);

static struct i2c_driver fm1288_i2c_driver = {
	.driver = {
		.name = "fm1288",
		.owner = THIS_MODULE,
	},
	.probe = fm1288_i2c_probe,
	.id_table = fm1288_i2c_id,
};

static int __init fm1288_init(void)
{
	return i2c_add_driver(&fm1288_i2c_driver);
}

static void __exit fm1288_exit(void)
{
	return i2c_del_driver(&fm1288_i2c_driver);
}

module_init(fm1288_init);
module_exit(fm1288_exit);

MODULE_DESCRIPTION("Voice Processor fm1288 driver");
MODULE_AUTHOR("Yeevan <showy.zhang@rock-chips.com>");
MODULE_LICENSE("GPL");


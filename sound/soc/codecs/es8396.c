			...
			struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:	/* prepare power up */
		...
		regv1 = snd_soc_read(w->codec, ES8396_HP_MIXER_BOOST_REG2B);
		regv1 &= 0xcc;
		snd_soc_write(w->codec, ES8396_HP_MIXER_BOOST_REG2B, regv1);

		regv1 = snd_soc_read(w->codec, ES8396_CPHP_CTRL_3_REG44);
		regv1 &= 0xcc;
		snd_soc_write(w->codec, ES8396_CPHP_CTRL_3_REG44, regv1);

		regv1 = snd_soc_read(w->codec, ES8396_CPHP_CTRL_1_REG42);
		regv1 &= 0xdf;
		snd_soc_write(w->codec, ES8396_CPHP_CTRL_1_REG42, regv1);
	...
	case SND_SOC_DAPM_PRE_PMD:	/* prepare power down */
		...
		snd_soc_write(w->codec, ES8396_SPK_EN_VOL_REG3B, regv1);
		...
		regv1 |= 0x11;
		snd_soc_write(w->codec, ES8396_SPK_MIXER_REG26, regv1);
		snd_soc_update_bits(w->codec, ES8396_SPK_CTRL_1_REG3C, 0x20,
				    0x20);
		break;
			 ...
			 struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		if (es8396_valid_micbias(es8396->mic_bias_lvl) == false) {
			...
			regv = (regv << 4) | 0x08;
			/* enable micbias1 */
			snd_soc_write(w->codec, ES8396_SYS_MICBIAS_CTRL_REG74,
				      regv);
		}
		     ...
		     struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		/* axMixer Gain boost */
		regv = snd_soc_read(w->codec, ES8396_AX_MIXER_BOOST_REG2F);
		regv |= 0x88;
		snd_soc_write(w->codec, ES8396_AX_MIXER_BOOST_REG2F, regv);
		/* axmixer vol = +12db */
		snd_soc_write(w->codec, ES8396_AX_MIXER_VOL_REG30, 0xaa);
		/* axmixer high driver capacility */
		snd_soc_write(w->codec, ES8396_AX_MIXER_REF_LP_REG31, 0x02);

		/* MNMixer Gain boost */
		regv = snd_soc_read(w->codec, ES8396_MN_MIXER_BOOST_REG37);
		regv |= 0x88;
		snd_soc_write(w->codec, ES8396_MN_MIXER_BOOST_REG37, regv);
		/* mnmixer vol = +12db */
		snd_soc_write(w->codec, ES8396_MN_MIXER_VOL_REG38, 0x44);
		/* mnmixer high driver capacility */
		snd_soc_write(w->codec, ES8396_MN_MIXER_REF_LP_REG39, 0x02);

		msleep(200);
		/* ADC STM and Digital Startup, ADC DS Mode */
		snd_soc_write(w->codec, ES8396_ADC_CSM_REG53, 0x00);
		/* force adc stm to normal */
		snd_soc_write(w->codec, ES8396_ADC_FORCE_REG77, 0x40);
		snd_soc_write(w->codec, ES8396_ADC_FORCE_REG77, 0x0);
		/* ADC Volume =0db */
		snd_soc_write(w->codec, ES8396_ADC_LADC_VOL_REG56, 0x0);

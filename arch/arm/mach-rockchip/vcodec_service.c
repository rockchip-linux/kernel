...
#define BIT(x)					(1<<(x))
#endif
static struct workqueue_struct *delay_work_queue = NULL;
...
struct vcodec_iommu_map {
    struct list_head iova_alloc_link;
    struct dma_buf *hdl;
...
typedef struct vpu_service_info {
    ...
    struct list_head mem_region_list;
    struct list_head iova_alloc_list;
    int iova_list_nums;
#endif
...
static void vcodec_enter_mode(struct vpu_subdev_data *data)
{
    ...
    if (pservice->subcnt < 2) {
        ...
        if (data->mmu_dev && !test_bit(MMU_ACTIVATED, &data->state)) {
            ...
            if (atomic_read(&pservice->enabled)) {
                if (arm_iommu_attach_device(data->dev, pservice->mapping))
                    dev_err(data->dev, "vcodec service attach failed\n");
                else
    ...
    list_for_each_entry_safe(subdata, n, &pservice->subdev_list, lnk_service) {
        if (data != subdata && subdata->mmu_dev &&
            ...
            arm_iommu_detach_device(subdata->dev);
        }
    }
#endif
    ...
    if (data->mode == VCODEC_RUNNING_MODE_HEVC)
        regmap_write(pservice->grf_base,
                     pservice->mode_ctrl,
                     raw | bits | (bits << 16));
    else
        regmap_write(pservice->grf_base,
    ...
    if (data->mmu_dev && !test_bit(MMU_ACTIVATED, &data->state)) {
        ...
        if (atomic_read(&pservice->enabled)) {
            if (arm_iommu_attach_device(data->dev, pservice->mapping))
                pr_info("vcodec service attach failed\n");
            else 
                BUG_ON(!atomic_read(&pservice->enabled));
        }
    }
...
static void vcodec_exit_mode(struct vpu_subdev_data *data)
{
    if (data->mmu_dev && test_bit(MMU_ACTIVATED, &data->state)) {
        ...
        arm_iommu_detach_device(data->dev);
...
static void reg_deinit(struct vpu_subdev_data *data,
                       vpu_reg *reg, int is_flush);
static void vpu_service_session_clear(struct vpu_subdev_data *data,
                                      vpu_session *session)
{
    vpu_reg * reg, *n;
    list_for_each_entry_safe(reg, n, &session->waiting, session_link) {
        reg_deinit(data, reg, 1);
    }
...
{
    queue_delayed_work(delay_work_queue,
                       &pservice->power_off_work, VPU_POWER_OFF_DELAY);
}
...
static void vpu_power_off_work(struct work_struct *work_s)
{
    ...
    struct vpu_service_info *pservice = 
        container_of(dlwork, struct vpu_service_info, power_off_work);
...
static inline enum VPU_DEC_FMT reg_check_fmt(vpu_reg *reg)
{
    enum VPU_DEC_FMT type =
        (enum VPU_DEC_FMT)((reg->reg[3] & 0xF0000000) >> 28);
...
#if defined(CONFIG_VCODEC_MMU)
static int vcodec_extend_iommu_mapping(struct dma_iommu_mapping *mapping)
{
    int next_bitmap;

    if (mapping->nr_bitmaps > mapping->extensions) return -EINVAL;

    next_bitmap = mapping->nr_bitmaps;
    mapping->bitmaps[next_bitmap] = kzalloc(mapping->bitmap_size, GFP_ATOMIC);
    if (!mapping->bitmaps[next_bitmap]) return -ENOMEM;

    mapping->nr_bitmaps++;
    return 0;
}

static dma_addr_t vcodec_alloc_iova(struct dma_iommu_mapping *mapping,
                                    size_t size)
{
    ...
    for (i = 0; i < mapping->nr_bitmaps; i++) {
        ...
        if (start > mapping->bits) continue;
    ...
    if (i == mapping->nr_bitmaps) {
        if (vcodec_extend_iommu_mapping(mapping)) {
...
static inline void vcodec_free_iova(struct dma_iommu_mapping *mapping,
...
static struct vcodec_iommu_map* vcodec_iommu_lookup(struct vpu_service_info *pservice,
                                                    struct dma_buf *hdl)
{
    struct vcodec_iommu_map *entry = NULL, *n;

    list_for_each_entry_safe(entry, n,
                             &pservice->iova_alloc_list, iova_alloc_link) {
        if (entry->hdl == hdl)
            return entry;
    }
...
#define IOMMU_FLAGS_PRESENT 0x01
...
static void dump_pagetbl(dma_addr_t fault_address, u32 addr_dte)
{
    ...
    if (!(IOMMU_FLAGS_PRESENT & dte)) goto print_it;

    pte_addr_phys = ((phys_addr_t)dte & 0xfffff000) + (pte_index * 4);
    pte_addr = phys_to_virt(pte_addr_phys);
    pte = *pte_addr;

    if (!(IOMMU_FLAGS_PRESENT & pte)) goto print_it;
...
static int vcodec_map_iommu_iova(struct vpu_subdev_data *data,
                                 struct vcodec_mem_region *mem_region)
{
    struct device *dev = data->dev;
    struct vpu_service_info *pservice = data->pservice;
    struct vcodec_iommu_map *iommu_maps;
    struct dma_buf_attachment *attach;
    struct sg_table *sgt;
    struct scatterlist *sgl;
    dma_addr_t dma_addr;
    int ret = DMA_ERROR_CODE;
    size_t mapped_size = 0;
    DEFINE_DMA_ATTRS(attrs);
    off_t start_off;

    iommu_maps = vcodec_iommu_lookup(pservice, mem_region->hdl);
    if (iommu_maps) {
        kref_get(&iommu_maps->ref);
        mem_region->len = iommu_maps->mapped_size;
        mem_region->iova = iommu_maps->iova_addr;

        return 0;
    }

    iommu_maps = kmalloc(sizeof(*iommu_maps), GFP_ATOMIC);
    if (!data) {
        ret = -ENOMEM;
        return ret;
    }

    time_record(&tasks[TASK_IOVA_MALLOC], 0);

    struct dma_iommu_mapping *mapping = pservice->mapping;
    iommu_maps->hdl = mem_region->hdl;

    struct drm_gem_object *drm_obj = mem_region->hdl->priv;

    sgt = drm_obj->dev->driver->gem_prime_get_sg_table(drm_obj);
    iommu_maps->sgt = sgt;

    sgl = sgt->sgl;

    start_off = offset_in_page(sg_phys(sgl));
    iommu_maps->mapped_size = PAGE_ALIGN(mem_region->hdl->size + start_off);

    dma_addr = vcodec_alloc_iova(mapping, iommu_maps->mapped_size);

    if (dma_addr == DMA_ERROR_CODE) {
        ret = dma_addr;
        vpu_err("dma iova alloc failed\n");
        sg_free_table(sgt);
        kfree(sgt);
        kfree(iommu_maps);
    ...
    iommu_maps->iova_addr = dma_addr + start_off;
    ...
    do {
        ...
        while (sg_next(sgl) && sg_phys(sgl) + sgl->length ==
               sg_phys(sg_next(sgl))) {
            len += (sg_next(sgl))->length;
            sgl = sg_next(sgl);
        }
        ...
        if (len > (iommu_maps->mapped_size - mapped_size))
            len = iommu_maps->mapped_size - mapped_size;

        ret = iommu_map(mapping->domain, dma_addr, phys, len, 0x07);
        ...
        dma_addr += len;
        mapped_size += len;
    } while ((sgl = sg_next(sgl)) && (mapped_size < iommu_maps->mapped_size));

    if (mapped_size < iommu_maps->mapped_size) {
        iommu_unmap(mapping->domain, iommu_maps->iova_addr, mapped_size);
        sg_free_table(sgt);
        kfree(sgt);
        kfree(iommu_maps);
        vcodec_exit_mode(data);
    ...
    iommu_maps->pservice = pservice;
    iommu_maps->dev = data->dev;
    kref_init(&iommu_maps->ref);

    INIT_LIST_HEAD(&iommu_maps->iova_alloc_link);
    list_add_tail(&iommu_maps->iova_alloc_link, &pservice->iova_alloc_list);

    time_record(&tasks[TASK_IOVA_MALLOC], 1);
    time_diff(&tasks[TASK_IOVA_MALLOC]);

    return ret;
}

static void vcodec_iommu_release(struct kref *kref)
{
    time_record(&tasks[TASK_IOVA_FREE], 0);

    struct vcodec_iommu_map *iommu_maps = container_of(kref, struct vcodec_iommu_map, ref);
    dma_addr_t dma_addr;
    struct device *dev = iommu_maps->dev;
    struct dma_iommu_mapping *mapping = iommu_maps->pservice->mapping;
    size_t unmapped_size;

    dma_addr = round_down(iommu_maps->iova_addr, PAGE_SIZE);
    unmapped_size = iommu_unmap(mapping->domain, dma_addr,
                                iommu_maps->mapped_size);

    if (unmapped_size != iommu_maps->mapped_size) {
        dev_err(dev, "umapped size %d mem_region len %d\n",
                unmapped_size, iommu_maps->mapped_size);
    }

    vcodec_free_iova(mapping, dma_addr, iommu_maps->mapped_size);

    sg_free_table(iommu_maps->sgt);
    kfree(iommu_maps->sgt);

    list_del_init(&iommu_maps->iova_alloc_link);

    kfree(iommu_maps);

    time_record(&tasks[TASK_IOVA_FREE], 1);
    time_diff(&tasks[TASK_IOVA_FREE]);
}

static int vcodec_unmap_iommu_iova(struct vpu_subdev_data *data, struct vcodec_mem_region *mem_region)
{
    struct vcodec_iommu_map *iommu_maps =
        vcodec_iommu_lookup(data->pservice, mem_region->hdl);
    if (!iommu_maps) {
        return 0;
    }

    kref_put(&iommu_maps->ref, vcodec_iommu_release);
    data->pservice->iova_list_nums--;
    return 0;
}
...
static int vcodec_fd_to_iova(struct vpu_subdev_data *data, vpu_reg *reg, int fd)
{
    ...
    mem_region->hdl = dma_buf_get(fd);
    if (IS_ERR(mem_region->hdl)) {
        vpu_err("get dma buf failed\n");
        return -1;
    }

    ret = vcodec_map_iommu_iova(data, mem_region);

    if (ret < 0) {
        vpu_err("ion map iommu failed\n");
        dma_buf_put(mem_region->hdl);
        kfree(mem_region);
        return ret;
    }
    ...
    for (i = 0; i < size; i++) {
        ...
        if (usr_fd != 0) {
            ...
            if (tbl[i] == 42 && data->hw_info->hw_id == HEVC_ID) {
                ...
                dma_buf = dma_buf_get(usr_fd);
                pps = dma_buf_vmap(dma_buf);
                for (i = 0; i < 64; i++) {
                    u32 scaling_offset;
                    u32 tmp;
                    int scaling_fd = 0;
                    ...
                    if (scaling_fd > 0) {
                        ...
                        uint32_t address = (uint32_t)&pps[i * 80 + 74];
                    }
                }
                dma_buf_vunmap(dma_buf, pps);
                dma_buf_put(dma_buf);
            }
            ...
            if (mem_region == NULL) {
                dev_err(pservice->dev,
                        "allocate memory for iommu memory region failed\n");
                return -1;
            }

            mem_region->hdl = dma_buf_get(usr_fd);
            mem_region->reg_idx = tbl[i];
            ret = vcodec_map_iommu_iova(data, mem_region);

            if (ret < 0) {
                dev_err(pservice->dev, "vcodec map iommu failed\n");
                vcodec_unmap_iommu_iova(data, mem_region);
                dma_buf_put(mem_region->hdl);
                kfree(mem_region);
                return ret;
            }
...
#if defined(CONFIG_VCODEC_MMU)
    if (data->mmu_dev) INIT_LIST_HEAD(&reg->mem_region_list);
#endif
...
static void reg_deinit(struct vpu_subdev_data *data, vpu_reg *reg, int is_flush)
{
    struct vpu_service_info *pservice = data->pservice;
#if defined(CONFIG_VCODEC_MMU)
    ...
    if (data->mmu_dev) {
        list_for_each_entry_safe(mem_region, n,
            ...
            struct vcodec_iommu_map *iommu_maps =
                vcodec_iommu_lookup(pservice, mem_region->hdl);
                                 ...
                                 vpu_reg *reg)
{
    ...
    case VPU_PP :
        {
            pservice->reg_pproc = NULL;
            reg_copy_from_hw(reg, data->dec_dev.hwregs + PP_INTERRUPT_REGISTER,
                             REG_NUM_9190_PP);
    ...
    if (irq_reg != -1) reg->reg[irq_reg] = pservice->irq_status;
...
static void vpu_service_set_freq(struct vpu_service_info *pservice, vpu_reg *reg)
{
    ...
    default :
        {
            if (soc_is_rk2928g()) clk_set_rate(pservice->aclk_vcodec, 400 * MHZ);
            else clk_set_rate(pservice->aclk_vcodec, 300 * MHZ);
        }break;
    }
}
    ...
    case VPU_ENC :
        {
            ...
            for (i = 0; i < VPU_REG_EN_ENC; i++) dst[i] = src[i];

            for (i = VPU_REG_EN_ENC + 1; i < enc_count; i++) dst[i] = src[i];
    ...
    case VPU_DEC :
        {
            ...
            if (data->hw_info->hw_id != HEVC_ID) {
                dst[VPU_REG_DEC_GATE] =
                    src[VPU_REG_DEC_GATE] | VPU_REG_DEC_GATE_BIT;
    ...
    case VPU_DEC_PP :
        {
            ...
            dst[VPU_REG_DEC_PP_GATE] =
                src[VPU_REG_DEC_PP_GATE] | VPU_REG_PP_GATE_BIT;
            dst[VPU_REG_DEC_GATE] =
                src[VPU_REG_DEC_GATE] | VPU_REG_DEC_GATE_BIT;
            dst[VPU_REG_EN_DEC] = src[VPU_REG_EN_DEC];
...
static void try_set_reg(struct vpu_subdev_data *data)
{
    ...
    if (!list_empty(&pservice->waiting)) {
        int can_set = 0;
        bool change_able =
            (NULL == pservice->reg_codec) && (NULL == pservice->reg_pproc);
            ...
            case VPU_PP :
                {
                    if (NULL == pservice->reg_codec) {
                        ...
                        if (pservice->auto_freq) can_set = 0;
                    }
                }break;
            case VPU_DEC_PP :
                {
                    if (change_able) can_set = 1;
                }break;
            default :
                {
        ...
        if (reset_request && !change_able) reset_request = 0;

        // do reset before setting registers
        if (reset_request) vpu_reset(data);
                      ...
                      vpu_reg *reg, u32 __user *dst)
{
    ...
    case VPU_DEC :
        {
            int reg_len =
                data->hw_info->hw_id == HEVC_ID ? REG_NUM_HEVC_DEC
                : REG_NUM_9190_DEC;
    ...
    case VPU_IOC_GET_REG :
        {
            ...
            if (copy_from_user(&req, (void __user *)arg,
                ...
                int ret = wait_event_timeout(session->wait,
                ...
                if (ret < 0) {
                    ...
                    if (task_running) {
                        ...
                        printk("%d task is running but not return, reset hardware...",
                               task_running);
                        vpu_reset(data);
                        printk("done\n");
                    }
...
{
    struct vpu_subdev_data *data = container_of(inode->i_cdev,
                                                struct vpu_subdev_data, cdev);
    struct vpu_service_info *pservice = data->pservice;
    int task_running;
    vpu_session *session = (vpu_session *)filp->private_data;
                               ...
                               struct vpu_service_info *pservice)
{
...
#if defined(CONFIG_VCODEC_MMU)
    if (iommu_en) {
        if (data->mode == VCODEC_RUNNING_MODE_HEVC) sprintf(mmu_dev_dts_name, "rockchip,hevc_mmu");
        else sprintf(mmu_dev_dts_name, "rockchip,vpu_mmu");

        pr_info("mmu_dev_dts_name %s\n", mmu_dev_dts_name);

        data->mmu_dev =
        rockchip_get_sysmmu_dev(mmu_dev_dts_name);

        if (data->mmu_dev) platform_set_sysmmu(data->mmu_dev, dev);
    }
#endif
    get_hw_info(data);
    ...
    if (data->debugfs_dir == NULL) vpu_err("create debugfs dir %s failed\n", name);
    ...
    return 0;
err:
    if (data->irq_enc > 0) free_irq(data->irq_enc, (void *)data);
    if (data->irq_dec > 0) free_irq(data->irq_dec, (void *)data);
    ...
    if (data->cls) class_destroy(data->cls);
    return -1;
}
...
#ifdef CONFIG_MFD__LOCAL_SYSCON
...
static void vcodec_init_drvdata(struct vpu_service_info *pservice)
{
    ...
    pservice->mapping = arm_iommu_create_mapping(&platform_bus_type, 0x00000000, SZ_2G);

    if (IS_ERR(pservice->mapping)) {
        vpu_err("failed to create iommu mapping for vcodec ret %ld\n",
                PTR_ERR(pservice->mapping));
    } else {
        vpu_debug(DEBUG_IOMMU, "vcodec ion client create success!\n");
    }

    INIT_LIST_HEAD(&pservice->iova_alloc_list);
    pservice->iova_list_nums = 0;
}
...
static int vcodec_probe(struct platform_device *pdev)
{
    ...
    delay_work_queue = create_workqueue("vpu_service_delay_queue");
    if (!delay_work_queue) {
    ...
    if (0 > vpu_get_clk(pservice)) goto err;

    clk_set_rate(pservice->aclk_vcodec, 400 * 1000 * 1000);
...
err:
    ...
    if (res) devm_release_mem_region(&pdev->dev, res->start, resource_size(res));
...
static void get_hw_info(struct vpu_subdev_data *data)
{
    ...
    if (data->mode == VCODEC_RUNNING_MODE_VPU) {
        ...
        if (dec->jpegSupport && ((configReg >> DWL_PJPEG_E) & 0x01U)) dec->jpegSupport = JPEG_PROGRESSIVE;
        ...
        if (dec->refBufSupport) {
            if ((configReg >> DWL_REF_BUFF_ILACE_E) & 0x01U) dec->refBufSupport |= 2;
            if ((configReg >> DWL_REF_BUFF_DOUBLE_E) & 0x01U) dec->refBufSupport |= 4;
        }
        ...
        /* JPEG xtensions */
        if (((asicID >> 16) >= 0x8190U) || ((asicID >> 16) == 0x6731U)) dec->jpegESupport = (configReg >> DWL_JPEG_EXT_E) & 0x01U;
        else dec->jpegESupport = JPEG_EXT_NOT_SUPPORTED;

        if (((asicID >> 16) >= 0x9170U) || ((asicID >> 16) == 0x6731U)) dec->rvSupport = (configReg >> DWL_RV_E) & 0x03U;
        else dec->rvSupport = RV_NOT_SUPPORTED;
        dec->mvcSupport = (configReg >> DWL_MVC_E) & 0x03U;

        if (dec->refBufSupport && (asicID >> 16) == 0x6731U) dec->refBufSupport |= 8; /* enable HW support for offset */
        ...
        if (cpu_is_rk3036() || cpu_is_rk312x()) dec->maxDecPicWidth = 1920;
        else dec->maxDecPicWidth = 4096;
...
static irqreturn_t vdpu_irq(int irq, void *dev_id)
{
    ...
    if (data->hw_info->hw_id != HEVC_ID) {
        ...
        if (pp_status & PP_INTERRUPT_BIT) {
            ...
            if (check_irq_err(&tasks[TASK_VPU_PP], dec_status)) atomic_add(1, &pservice->reset_request);
    ...
    if (atomic_read(&dev->irq_count_pp) ||
        atomic_read(&dev->irq_count_codec)) return IRQ_WAKE_THREAD;
...
static irqreturn_t vepu_irq(int irq, void *dev_id)
{
    ...
    if (likely(irq_status & ENC_INTERRUPT_BIT)) {
        ...
        if (check_irq_err(&tasks[TASK_VPU_ENC], irq_status)) atomic_add(1, &pservice->reset_request);
    ...
    if (atomic_read(&dev->irq_count_codec)) return IRQ_WAKE_THREAD;
    else return IRQ_NONE;
}
...
static int debug_vcodec_show(struct seq_file *s, void *unused)
{
    ...
    if (data->hw_info->hw_id != HEVC_ID) {
        ...
        for (i = 0; i < n; i++) seq_printf(s, "\tswreg%d = %08X\n", i, readl(data->enc_dev.hwregs + i));
    }
    seq_printf(s, "\nDEC Registers:\n");
...
u8* get_align_ptr(u8 *tbl, int len, u32 *phy)
{
    ...
    if (ion_client == NULL) ion_client = rockchip_ion_client_create("vcodec");
...
static int hevc_test_case0(vpu_service_info *pservice)
{
    ...
    while (testidx < TEST_CNT) {
        ...
        reg->reg[42] = phy_pps;
        reg->reg[43] = phy_rps;
        for (i = 10; i <= 24; i++) reg->reg[i] = phy_yuv;
        ...
        if (!list_empty(&session.done)) {
            if (ret < 0) vpu_err("warning: pid %d wait task sucess but wait_evernt ret %d\n", session.pid, ret);
            ret = 0;
        } else {
            if (unlikely(ret < 0)) {
                vpu_err("error: pid %d wait task ret %d\n", session.pid, ret);
        ...
        if (ret < 0) {
            ...
            printk("\nDEC Registers:\n");
            n = data->dec_dev.iosize >> 2;
            for (i = 0; i < n; i++) printk("\tswreg%d = %08X\n", i, readl(data->dec_dev.hwregs + i));

...
static int inv_init_config(struct iio_dev *indio_dev)
{
	...
	if (result)
		return result;
	st->chip_config.lpf = INV_FILTER_98HZ;
...
static int inv_detect_6xxx(struct inv_mpu_iio_s *st)
{
	...
	if (result)
		return result;
	if ((d == MPU6500_ID) || (d == MPU6515_ID) || (d == MPU6500T_ID)) {
	...
	const struct i2c_device_id *id)
{
	...
	dev_info(&client->dev, "%s is ready to go! %s\n",
					indio_dev->name, RK_VER);

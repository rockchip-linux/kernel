...
static struct delay_stat test_delay[DELAY_STAT_MAX];

static void new_tm(int64_t tm, struct delay_stat* st, const char* title) {
	int64_t tm_cur = get_time_ns();
	int64_t tm_delta = tm_cur - tm;
	if (st->tm_min==0 && st->tm_max==0)
		st->tm_min = st->tm_max = tm_delta;
	else if (tm_delta < st->tm_min)
		st->tm_min = tm_delta;
	else if (tm_delta > st->tm_max)
		st->tm_max = tm_delta;
	st->tm_sum += tm_delta;
	st->tm_count++;

	if (unlikely((tm_cur-st->tm_last_print) >= 1000000000)) {
		pr_info("%s: [%lld] %lld,%d,%lld\n", title, tm, st->tm_min, (u32)st->tm_sum/(u32)st->tm_count, st->tm_max);
		st->tm_last_print = tm_cur;
		st->tm_min = st->tm_max = st->tm_count = st->tm_sum = 0;
	}
}
#endif

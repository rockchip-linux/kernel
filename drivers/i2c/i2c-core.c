...
void i2c_parse_fw_timings(struct device *dev, struct i2c_timings *t, bool use_defaults)
{
	...
	ret = of_property_read_u32(np, "clock-frequency", &t->bus_freq_hz);
	if (ret && use_defaults)
		t->bus_freq_hz = 100000;

	ret = of_property_read_u32(np, "i2c-scl-rising-time-ns", &t->scl_rise_ns);
	if (ret && use_defaults) {
	...
	ret = of_property_read_u32(np, "i2c-scl-falling-time-ns", &t->scl_fall_ns);
	if (ret && use_defaults) {
	...
	of_property_read_u32(np, "i2c-scl-internal-delay-ns", &t->scl_int_delay_ns);

	ret = of_property_read_u32(np, "i2c-sda-falling-time-ns", &t->sda_fall_ns);
	if (ret && use_defaults)

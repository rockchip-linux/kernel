...
static int mid_spi_dma_transfer(struct dw_spi *dws, int cs_change)
{
	...
	if (dws->tx)
	{
		...
		DBG_SPI("%s:dst_addr=%pad,tx_dma=%pad,len=%zu,burst=%d,width=%d\n"
			,__func__, &dws->tx_dma_addr, &dws->tx_dma, dws->len, txconf.dst_maxburst, width);
	}
	...
	if (dws->rx)
	{
		...
		DBG_SPI("%s:src_addr=%pad,rx_dma=%pad,len=%zu,burst=%d,width=%d\n"
			,__func__, &dws->rx_dma_addr, &dws->rx_dma, dws->len, rxconf.src_maxburst, width);
	}

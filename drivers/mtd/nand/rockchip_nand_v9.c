...
struct rk_nandc {
	...
	unsigned char banks[NANDC_V9_NUM_BANKS];
	int selected_bank;
...
{
	writel(0, nandc->regs + NANDC_REG_V9_RANDMZ);
	writel(0, nandc->regs + NANDC_REG_V9_DMA_CFG);
	writel(NANDC_V9_FM_WP, nandc->regs + NANDC_REG_V9_FMCTL);
	writel(NANDC_V9_FL_RST, nandc->regs + NANDC_REG_V9_FLCTL);
	writel(0x1081, nandc->regs + NANDC_REG_V9_FMWAIT);
}

static irqreturn_t rk_nandc_interrupt(int irq, void *dev_id)
{
	struct rk_nandc *nandc = dev_id;
	u32 st = readl(nandc->regs + NANDC_REG_V9_INTST);
	u32 ien = readl(nandc->regs + NANDC_REG_V9_INTEN);
	...
	writel(st, nandc->regs + NANDC_REG_V9_INTCLR);
	writel(~st & ien, nandc->regs + NANDC_REG_V9_INTEN);
...
static void rk_nandc_select_chip(struct mtd_info *mtd, int chipnr)
{
	...
	reg = readl(nandc->regs + NANDC_REG_V9_FMCTL);
	...
	if (chipnr == -1) {
		...
		bank_base = nandc->regs + NANDC_REG_V9_BANK0 + banknr * 0x100;
	...
	writel(reg, nandc->regs + NANDC_REG_V9_FMCTL);
...
static void rk_nandc_cmd_ctrl(struct mtd_info *mtd, int dat, unsigned int ctrl)
{
	...
	struct nand_chip *chip = mtd->priv;
	u32 reg;
	void __iomem *bank_base = nandc->regs + NANDC_REG_V9_BANK0
	...
	if (ctrl & NAND_CTRL_CHANGE) {
		...
		reg &= ~NANDC_V9_FM_CE_SEL_M;
		if (ctrl & NAND_NCE)
			reg |= NANDC_V9_FM_CE_SEL(nandc->selected_bank);
		writel(reg, nandc->regs + NANDC_REG_V9_FMCTL);
	}
				...
				dma_addr_t dma_data, dma_addr_t dma_oob)
{
	...
	reg = (1 << 0) | ((!dir) << 1) | (1 << 2) | (2 << 3) | (7 << 6) |
	      (16 << 9);
	writel(reg, nandc->regs + NANDC_REG_V9_DMA_CFG);
	writel((u32)dma_data, nandc->regs + NANDC_REG_V9_DMA_BUF0);
	writel((u32)dma_oob, nandc->regs + NANDC_REG_V9_DMA_BUF1);

	reg = (dir << 1) | (1 << 3) | (1 << 5) | (1 << 10) |
	      (n_KB << 22) | (1 << 29);
	writel(reg, nandc->regs + NANDC_REG_V9_FLCTL);
	reg |= (1 << 2);
	writel(reg, nandc->regs + NANDC_REG_V9_FLCTL);
}
...
static int rk_nand_wait_for_xfer_done(struct rk_nandc *nandc)
{
	u32 reg;
	int ret;
	void __iomem *ptr = nandc->regs + NANDC_REG_V9_FLCTL;
					      ...
					      int oob_required, int page)
{
	...
	writel(NANDC_V9_INT_DMA, nandc->regs + NANDC_REG_V9_INTEN);
	...
	for (i = 0; i < ecc->steps / 2; i++) {
		bch_st = readl(nandc->regs + NANDC_REG_V9_BCHST + i * 4);
				 ...
				 uint32_t strength)
{
	...
	writel(reg, nandc->regs + NANDC_REG_V9_BCHCTL);
				     ...
				     struct nand_ecc_ctrl *ecc)
{
	...
	nandc->max_ecc_strength = 70;
	ver = readl(nandc->regs + NANDC_REG_V9_VER);
	if (ver != 0x56393030)
...
static int rk_nandc_chips_init(struct device *dev, struct rk_nandc *nandc)
{
	...
	chipnr = 0;
	for (bank_idx = 0; bank_idx < NANDC_V9_NUM_BANKS; bank_idx++) {
...
static int rk_nandc_probe(struct platform_device *pdev)
{
	...
	writel(0, nandc->regs + NANDC_REG_V9_INTEN);

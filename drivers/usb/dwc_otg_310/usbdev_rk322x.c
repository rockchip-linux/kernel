...
static void usb20otg_soft_reset(void *pdata, enum rkusb_rst_flag rst_type)
{
	...
	rst_otg_h = reset_control_get(usbpdata->dev, "otg_ahb");
	if (IS_ERR(rst_otg_h)) {
		dev_err(usbpdata->dev, "Fail to get ahb reset from dts\n");
		return;
	}

	rst_otg_p = reset_control_get(usbpdata->dev, "otg_phy");
	if (IS_ERR(rst_otg_p)) {
		dev_err(usbpdata->dev, "Fail to get phy reset from dts\n");
		goto put_rst_ahb;
	}

	rst_otg_c = reset_control_get(usbpdata->dev, "otg_controller");
	if (IS_ERR(rst_otg_c)) {
		dev_err(usbpdata->dev, "Fail to get ctl reset from dts\n");
		goto put_rst_phy;
	}
	...
	reset_control_put(rst_otg_c);

put_rst_phy:
	reset_control_put(rst_otg_p);

put_rst_ahb:
	reset_control_put(rst_otg_h);
}
...
static int usb20otg_get_status(int id)
{
	...
	case USB_STATUS_BVABLID:
		/* bvalid in grf */
		ret = soc_status0 & (0x1 << 4);
		break;
	case USB_STATUS_DPDM:
		/* dpdm in grf */
		ret = soc_status0 & (0x3 << 2);
		break;
	case USB_STATUS_ID:
		/* id in grf */

...
static int gpio_det_parse_dt(struct gpio_detection *gpio_det)
{
	...
	struct gpio_data *gpiod;
	int i = 0;
	int num = of_get_child_count(gpio_det->dev->of_node);
	...
	for_each_child_of_node(gpio_det->dev->of_node, node) {
		...
		if (!gpio_is_valid(gpiod->gpio)) {
			dev_err(gpio_det->dev, "failed to get gpio\n");
			num--;
			i--;
			continue;
		}

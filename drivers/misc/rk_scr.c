...
#define SMC_RESET_TIME_DELAY		50 /*ms*/
#define SMC_CHECK_CONFLICT_TIMEOUT	500 /*ms*/
...
int scr_card_monitor(void *arg)
{
	#define SMC_NOT_READY	0
	#define SMC_READY	1
	#define SMC_UNKNOWN	2
	int smc_ins;
	unsigned status = SMC_READY;
	...
	status = SMC_NOT_READY;
	rk_scr->recv_data_count = 0;
	rk_scr->recv_data_offset = 0;
	scr_activate_card();

	{
		smc_ins = scr_check_card_insert();
		if ((smc_ins == SMC_DRV_INT_CARDIN) && (status != SMC_READY)) {
			status = SMC_READY;
			scr_activate_card();
			DAL_LOGV("Samrtcard insert!\n");
		} else if ((smc_ins == SMC_DRV_INT_CARDOUT) &&
						(status != SMC_NOT_READY)) {
			status = SMC_NOT_READY;
			scr_deactivate_card();
			DAL_LOGV("Samrtcard remove!");
		}
...
static struct task_struct *smart_card_thread;
int scr_open(void)
{
	mutex_lock(&rk_scr->scr_mutex);
	smart_card_thread = kthread_create(scr_card_monitor,
					   NULL, "smart_card_monitor");
	if (IS_ERR(smart_card_thread)) {
		pr_err("smart_card_thread create fail");
		smart_card_thread = NULL;
		mutex_unlock(&rk_scr->scr_mutex);
		return UNSATISFIED;
	}

	wake_up_process(smart_card_thread);
	mutex_unlock(&rk_scr->scr_mutex);
...
int scr_close(void)
{
	return 0;
	mutex_lock(&rk_scr->scr_mutex);
	if (smart_card_thread) {
		kthread_stop(smart_card_thread);
		smart_card_thread = NULL;
	}
	mutex_unlock(&rk_scr->scr_mutex);
...
static struct kobj_attribute scr_sysfs_attribute =
		__ATTR(scr_sysfs, 0666, scr_sysfs_show, scr_sysfs_store);
...
static int __init rk_scr_init(void)
{
	return platform_driver_register(&rk_scr_driver);
}

static void __exit rk_scr_exit(void)
{
	platform_driver_unregister(&rk_scr_driver);
}
module_init(rk_scr_init);
module_exit(rk_scr_exit);

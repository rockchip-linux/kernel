...
static int rtl8365mb_inir_ext_rgmii(struct realtek_priv *priv, int port)
{
	int ext_int;
	int ret;
...
static int rtl8365mb_detect(struct realtek_priv *priv)
{
	...
	case RTL8365MB_CHIP_ID_8365MB_VC:
		...
		ret = rtl8365mb_inir_ext_rgmii(priv, 6);
		if (ret){
			dev_err(priv->dev,"failed to configure RGMII mode on port 6: %d\n", ret);
			return -ENODEV;
		}

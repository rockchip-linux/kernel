...
static int rtl8366rb_setup(struct dsa_switch *ds)
{
	...
	while (i < ARRAY_SIZE(rtl8366rb_green_jam)) {
		ret = regmap_read(priv->map, RTL8366RB_PHY_ACCESS_BUSY_REG,
				  &val);
		if (ret)
			return ret;
		if (!(val & RTL8366RB_PHY_INT_BUSY)) {
	...
	ret = regmap_write(priv->map, RTL8366RB_SSCR0, 0);
	if (ret)
		return ret;

	/* Enable auto ageing for all ports */
	ret = regmap_write(priv->map, RTL8366RB_SSCR1, 0);
	if (ret)
		return ret;
...
static int rtl8366rb_set_mc_index(struct realtek_priv *priv, int port, int index)
{
	...
	return regmap_update_bits(priv->map, RTL8366RB_PORT_VLAN_CTRL_REG(port),
				RTL8366RB_PORT_VLAN_CTRL_MASK <<

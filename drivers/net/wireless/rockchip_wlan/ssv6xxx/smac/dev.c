...
#endif
static u16 bits_per_symbol[][2] = { { 26, 54 },
				    { 52, 108 },
				    { 78, 162 },
				    { 104, 216 },
				    { 156, 324 },
...
unsigned int cal_duration_of_mpdu(struct sk_buff *mpdu_skb)
{
	...
	timeout = (unsigned int)ktime_to_ms(
	    ktime_sub(ktime_get(), mpdu_skb_info->timestamp));
...
u32 ssv6xxx_pbuf_alloc(struct ssv_softc *sc, int size, int type)
{
	...
	if (regval == 0)
		...
		dev_info(sc->dev, "Allocated %d type packet buffer of size %d "
				  "(%d) at address %x.\n",
...
int ssv6xxx_set_channel(struct ssv_softc *sc, int ch)
{
	...
	do {
		if ((ret = SMAC_REG_READ(sh, ADR_SPI_TO_PHY_PARAM1, &regval)) !=
		    0)
			break;
		if ((ret = SMAC_REG_WRITE(sh, ADR_SPI_TO_PHY_PARAM1,
					  (regval & ~0xffff) | 3)) != 0)
			break;
		...
		for (retry_cnt = 20; retry_cnt > 0; retry_cnt--) {
			mdelay(20);
			if ((ret = SMAC_REG_READ(sh, ADR_CBR_READ_ONLY_FLAGS_1,
						 &regval)) != 0)
				break;
			if (regval & 0x00000004) {
				if ((ret = SMAC_REG_SET_BITS(
					 sh, ADR_CBR_SX_ENABLE_RGISTER,
					 (0 << 12), 0x00001000)) != 0)
					break;
				if ((ret = SMAC_REG_READ(
				...
				if ((regval & 0x00001800) == 0) {
					...
					printk("%s(): Lock channel %d fail!\n",
					       __FUNCTION__,
					       vt_tbl[chidx].channel_id);
					if ((ret = SMAC_REG_READ(
						 sh, ADR_CBR_READ_ONLY_FLAGS_1,
						 &regval)) != 0)
						break;
					printk("%s(): dbg: vt-mon read out as "
					       "%d when rdy\n",
					       __FUNCTION__,
					       ((regval & 0x00001800) >> 11));
					if ((ret = SMAC_REG_READ(
						 sh, ADR_CBR_READ_ONLY_FLAGS_2,
						 &regval)) != 0)
						break;
					printk("%s(): dbg: sub-sel read out as "
					       "%d when rdy\n",
					       __FUNCTION__,
					       ((regval & 0x00000fe0) >> 5));
					if ((ret = SMAC_REG_READ(
						 sh, ADR_CBR_SYN_DIV_SDM_XOSC,
						 &regval)) != 0)
						break;
					printk("%s(): dbg: RG_SX_REFBYTWO read "
					       "out as %d when rdy\n",
					       __FUNCTION__,
					       ((regval & 0x00002000) >> 13));
					if ((ret = SMAC_REG_READ(
						 sh, ADR_CBR_SYN_RGISTER_1,
						 &regval)) != 0)
						break;
					printk("%s(): dbg: RG_SX_RFCTRL_F read "
					       "out as 0x%08x when rdy\n",
					       __FUNCTION__,
					       ((regval & 0x00ffffff) >> 0));
					if ((ret = SMAC_REG_READ(
						 sh, ADR_CBR_SYN_RGISTER_2,
						 &regval)) != 0)
						break;
					printk("%s(): dbg: RG_SX_RFCTRL_CH "
					       "read out as 0x%08x when rdy\n",
					       __FUNCTION__,
					       ((regval & 0x000007ff) >> 0));
					if ((ret = SMAC_REG_READ(
						 sh, ADR_CBR_SX_ENABLE_RGISTER,
						 &regval)) != 0)
						break;
					printk("%s(): dbg: RG_EN_SX_VT_MON_DG "
					       "read out as %d when rdy\n",
					       __FUNCTION__,
					       ((regval & 0x00001000) >> 12));
				}
			}
		}
		fail_cnt++;
...
int ssv6xxx_set_channel(struct ssv_softc *sc, int ch)
{
		...
		if ((ch == 13) || (ch == 14)) {
			if (sh->ipd_channel_touch == 0) {
				for (chidx = 0; chidx < sh->ch_cfg_size;
				     chidx++) {
					SMAC_REG_WRITE(
					    sh, sh->p_ch_cfg[chidx].reg_addr,
					    sh->p_ch_cfg[chidx].ch13_14_value);
				}
			...
			if (sh->ipd_channel_touch) {
				for (chidx = 0; chidx < sh->ch_cfg_size;
					...
					SMAC_REG_WRITE(
					    sh, sh->p_ch_cfg[chidx].reg_addr,
					    sh->p_ch_cfg[chidx].ch1_12_value);
				}
	...
	do {
		if ((sh->cfg.crystal_type == SSV6XXX_IQK_CFG_XTAL_26M) ||
			...
			if ((ret = SMAC_REG_SET_BITS(
				 sc->sh, ADR_SYN_DIV_SDM_XOSC, (0x00 << 13),
				 (0x01 << 13))) != 0)
				break;
		} else if (sh->cfg.crystal_type == SSV6XXX_IQK_CFG_XTAL_40M) {
			if ((ret = SMAC_REG_SET_BITS(
				 sc->sh, ADR_SYN_DIV_SDM_XOSC, (0x01 << 13),
				 (0x01 << 13))) != 0)
				break;
		} else {
		...
		if ((ret = SMAC_REG_SET_BITS(sc->sh, ADR_SYN_REGISTER_2,
		    ...
		    0)
			break;
		if ((ret = SMAC_REG_READ(sc->sh, ADR_SX_LCK_BIN_REGISTERS_I,
					 &regval)) != 0)
			break;
		regval =
		...
		do {
			mdelay(1);
			if ((ret = SMAC_REG_READ(sc->sh, ADR_READ_ONLY_FLAGS_1,
						 &regval)) != 0)
				break;
			if (regval & 0x00000002) {
				if ((ret = SMAC_REG_READ(sc->sh,
...
{
#define CTRL_FRAME_INDEX(fc)                                                   \
	((hdr->frame_control - IEEE80211_STYPE_BACK_REQ) >> 4)
...
#endif
static u32 ssv6xxx_ht_txtime(u8 rix, int pktlen, int width, int half_gi,
			     bool is_gf)
{
	...
	if (!half_gi)
		...
		if (!is_gf)
			duration = DIV_ROUND_UP(SYMBOL_TIME_HALFGI(nsymbols), 4)
				   << 2;
		else
...
static u32 ssv6xxx_non_ht_txtime(u8 phy, int kbps, u32 frameLen,
				 bool shortPreamble)
{
				      ...
				      struct ssv_softc *sc)
{
	...
	u32 frame_time = 0, ack_time = 0, rts_cts_nav = 0,
	    frame_consume_time = 0;
	...
	for (nRCParams = 0; (nRCParams < SSV62XX_TX_MAX_RATES); nRCParams++) {
		if ((rc_params == NULL) || (sc == NULL)) {
			...
			ssv_rc = sc->rc;
			mcsidx = (rc_params[nRCParams].drate -
		...
		if (tx_drate->flags & IEEE80211_TX_RC_MCS) {
			frame_time = ssv6xxx_ht_txtime(mcsidx, len, is_ht40,
						       is_sgi, is_gf);
			d_phy = 0;
		} else {
		...
		if (tx_desc->unicast) {
			ack_time = ssv6xxx_non_ht_txtime(
			    c_phy, crate_kbps, ACK_LEN, ctrl_short_preamble);
		}
		if (tx_desc->do_rts_cts & IEEE80211_TX_RC_USE_RTS_CTS) {
			rts_cts_nav = frame_time;
			rts_cts_nav += ack_time;
			rts_cts_nav += ssv6xxx_non_ht_txtime(
			    c_phy, crate_kbps, CTS_LEN, ctrl_short_preamble);
			frame_consume_time = rts_cts_nav;
			frame_consume_time += ssv6xxx_non_ht_txtime(
			    c_phy, crate_kbps, RTS_LEN, ctrl_short_preamble);
		} else if (tx_desc->do_rts_cts &
	...
	if (vif_priv) {
		if ((index != 0) && !list_empty(&vif_priv->sta_list)) {
			...
			list_for_each_entry(sta_priv_iter, &vif_priv->sta_list,
					    list)
			{
				if (((sta_priv_iter->sta_info->s_flags &
				      STA_FLAG_VALID) == 0) ||
				    (sta_priv_iter->sta_info->hw_wsid <
				     SSV_NUM_HW_STA))
					continue;
				hw_update_watch_wsid(
				    sc, sta_priv_iter->sta_info->sta,
...
static int hw_crypto_key_write_wep(struct ieee80211_hw *hw,
				   struct ieee80211_key_conf *key, u8 algorithm,
				   struct ssv_vif_info *vif_info)
{
	struct ssv_softc *sc = hw->priv;
					      ...
					      void *param)
{
	...
	if (wsid >= SSV_NUM_HW_STA) {
		hw_update_watch_wsid(sc, sta_info->sta, sta_info, wsid,
				     SSV6XXX_WSID_SEC_GROUP,
				     SSV6XXX_WSID_OPS_ENABLE_CAPS);
	}
#endif
}
static int _write_pairwise_key_to_hw(struct ssv_softc *sc, int index,
				     u8 algorithm, const u8 *key, int key_len,
...
static int _write_group_key_to_hw(struct ssv_softc *sc, int index, u8 algorithm,
...
#ifndef SSV6200_ECO
	address =
	    sec_key_tbl_base + ((index - 1) * sizeof(struct ssv6xxx_hw_key));
	pointer = (int *)&sramKey->group_key[index - 1];
	for (i = 0; i < (sizeof(struct ssv6xxx_hw_key) / 4); i++)
		SMAC_REG_WRITE(sc->sh, address + (i * 4), *(pointer++));
#endif
	...
	*(u16 *)&sram_key->sta_key[0].reserve[0] = key->keylen;
...
#ifdef USE_LOCAL_WEP_CRYPTO
	{
		...
		if (crypto_data->priv) {
			...
			dev_err(sc->dev, "[Local Crypto]: Failed to initialize "
					 "driver's crypto!\n");
			dev_info(sc->dev,
				       ...
				       struct ieee80211_key_conf *key)
{
	...
	struct ssv_vif_info *vif_info = &sc->vif_info[vif_priv->vif_idx];
	bool tdls_link = false, tdls_use_sw_cipher = false,
	...
	if (vif_info->if_type == NL80211_IFTYPE_STATION) {
		struct ssv_sta_priv_data *first_sta_priv = list_first_entry(
		    &vif_priv->sta_list, struct ssv_sta_priv_data, list);
	...
	if (sta_priv->has_hw_encrypt || sta_priv->has_hw_decrypt) {
        ...
        sta_priv->need_sw_encrypt = true;
#endif
		_write_pairwise_key_to_hw(sc, key->keyidx, cipher, key->key,
					  key->keylen, key, vif_priv, sta_priv);
	}
	if ((vif_priv->has_hw_encrypt || vif_priv->has_hw_decrypt) &&
	    (vif_priv->group_key_idx > 0)) {
		_set_aes_tkip_hw_crypto_group_key(
		    sc, &sc->vif_info[vif_priv->vif_idx], sta_priv->sta_info,
				    ...
				    struct ieee80211_key_conf *key)
{
	...
	if (vif_priv->has_hw_encrypt || vif_priv->has_hw_decrypt) {
		...
		_write_group_key_to_hw(sc, key->keyidx, cipher, key->key,
				       key->keylen, key, vif_priv, sta_priv);
	}
	vif_priv->is_security_valid = true;
	{
		...
		if (another_vif_priv != NULL) {
			if (((SSV6XXX_USE_SW_DECRYPT(vif_priv) &&
				...
				if (((val >> 4) & 0xF) != M_ENG_CPU) {
					SMAC_REG_WRITE(
					    sc->sh, ADR_RX_FLOW_DATA,
					    ((val & 0xf) | (M_ENG_CPU << 4) |
					     (val & 0xfffffff0) << 4));
					dev_info(
					    sc->dev, "orginal Rx_Flow %x , "
						     "modified flow %x \n",
					    val,
					    ((val & 0xf) | (M_ENG_CPU << 4) |
					     (val & 0xfffffff0) << 4));
				} else {
					printk(" doesn't need to change rx "
					       "flow\n");
				}
			}
		}
...
#endif
static int ssv6200_set_key(struct ieee80211_hw *hw, enum set_key_cmd cmd,
			   ...
			   struct ieee80211_key_conf *key)
{
	...
	dev_err(sc->dev, "Set key VIF %d VIF type %d STA %d algorithm = %d, "
			 "key->keyidx = %d, cmd = %d\n",
	...
	case SET_KEY: {
		...
		case SSV_CIPHER_CCMP:
			ret = _set_key_tkip_ccmp(sc, vif_priv, sta_priv, cipher,
						 key);
			break;
#ifdef CONFIG_SSV_WAPI
		case SSV_CIPHER_SMS4:
			ret =
			    _set_key_sms4(sc, vif_priv, sta_priv, cipher, key);
			break;
#endif
		default:
			break;
		...
		if (sta) {
			...
			if (SSV6200_USE_HW_WSID(sta_idx)) {
				if (SSV6XXX_USE_SW_DECRYPT(sta_priv)) {
					u32 cipher_setting;
					cipher_setting =
					    ssv6200_hw_get_pair_type(sc->sh);
				...
				if (sta_priv->has_hw_decrypt) {
					hw_update_watch_wsid(
					    sc, sta, sta_info, sta_idx,
					    SSV6XXX_WSID_SEC_HW,
					    SSV6XXX_WSID_OPS_HWWSID_PAIRWISE_SET_TYPE);
					printk("set hw wsid %d cipher mode to "
					       "HW cipher for pairwise key\n",
					       sta_idx);
				}
			}
		} else {
			if (vif_info->if_type == NL80211_IFTYPE_STATION) {
				...
				if (SSV6200_USE_HW_WSID(
					...
					if (vif_priv->has_hw_decrypt) {
						...
						printk("set hw wsid %d cipher "
						       "mode to HW cipher for "
						       "group  key\n",
						       first_sta_priv->sta_idx);
					}
				}
	...
	case DISABLE_KEY: {
			...
			if ((vif_info->if_type != NL80211_IFTYPE_AP) &&
				...
				if (sta_priv == first_sta_priv) {
					...
					HAL_SET_PAIRWISE_CIPHER_TYPE(
					    sc->sh, ME_NONE, sta_info->hw_wsid);
#else
		...
		if ((cipher == ME_TKIP) || (cipher == ME_CCMP)) {
			...
			hw_crypto_key_clear(hw, key->keyidx, key, vif_priv,
					    sta_priv);
		}
		{
			if ((key->keyidx == 0) && (sta_priv != NULL)) {
				...
				if (crypto_data->ops && crypto_data->priv) {
					...
					spin_lock_irqsave(&sc->crypt_st_lock,
							  flags);
					removed_skb_num =
					    _remove_sta_skb_from_q(
						sc, &sc->preprocess_q,
						sta_addr0_3, sta_addr4_5);
					spin_unlock_irqrestore(
					    &sc->crypt_st_lock, flags);
					dev_err(
					    sc->dev,
	...
	if (vif_priv) {
		printk("vif: hw_en:%d, sw_en:%d, hw_de:%d, sw_de:%d, "
		       "use_mac80211 %d, valid:%d\n",
		       (vif_priv->has_hw_encrypt == true),
		       (vif_priv->need_sw_encrypt == true),
		       (vif_priv->has_hw_decrypt == true),
		       (vif_priv->need_sw_decrypt == true),
...
void ssv6xxx_tx_rate_update(struct sk_buff *skb, void *args)
{
	...
	if (!(info->flags & IEEE80211_TX_CTL_AMPDU)) {
		...
		if ((ieee80211_is_data_qos(hdr->frame_control) ||
			...
			ret = ssv6xxx_rc_hw_rate_update_check(
			    skb, sc, tx_desc->do_rts_cts);
...
#define RTS_CTS_PROTECT(_flg)                                                  \
	((_flg) & IEEE80211_TX_RC_USE_RTS_CTS)                                 \
	    ? 1                                                                \
...
void ssv6xxx_update_txinfo(struct ssv_softc *sc, struct sk_buff *skb)
{
	...
	tx_desc->wsid =
	    (!sta_info || (sta_info->hw_wsid < 0)) ? 0x0F : sta_info->hw_wsid;
	...
	if (tx_desc->aggregation == 1) {
		...
		nav = ssv6xxx_set_frame_duration(info, &ssv_rate,
						 (skb->len + FCS_LEN), tx_desc,
						 &tx_desc->rc_params[0], sc);
#ifdef FW_RC_RETRY_DEBUG
		{
			printk("[FW_RC]:param[0]: drate =%d, count =%d, "
			       "crate=%d, dl_length =%d, frame_consume_time "
			       "=%d, rts_cts_nav=%d\n",
			...
			printk("[FW_RC]:param[1]: drate =%d, count =%d, "
			       "crate=%d, dl_length =%d, frame_consume_time "
			       "=%d, rts_cts_nav=%d\n",
			...
			printk("[FW_RC]:param[2]: drate =%d, count =%d, "
			       "crate=%d, dl_length =%d, frame_consume_time "
			       "=%d, rts_cts_nav=%d\n",
		...
		nav = ssv6xxx_set_frame_duration(
		    info, &ssv_rate, (skb->len + FCS_LEN), tx_desc, NULL, NULL);
	}
...
static void _ssv6xxx_tx(struct ieee80211_hw *hw, struct sk_buff *skb)
{
	...
	do {
		...
		if (
			...
			if ((unicast && (ssv_sta_priv != NULL) &&
			    ...
			    (!unicast && vif_priv->is_security_valid &&
	...
	tx_mpdu:
		...
		if (vif && vif->type == NL80211_IFTYPE_AP && (sc->bq4_dtim) &&
		    info->flags & IEEE80211_TX_CTL_SEND_AFTER_DTIM) {
			...
			if (priv_vif->sta_asleep_mask) {
				buffered = ssv6200_bcast_enqueue(
				    sc, &sc->bcast_txq, skb);
				if (1 == buffered) {
#ifdef BCAST_DEBUG
					printk(
...
bool _is_encrypt_needed(struct sk_buff *skb)
{
	...
	if (!is_unicast_ether_addr(hdr->addr1)) {
		if (vif_priv->is_security_valid && vif_priv->need_sw_encrypt
#ifdef USE_LOCAL_CRYPTO
		...
		if ((skb = __skb_dequeue(&sc->preprocess_q)) != NULL) {
			...
			if (skb_info->crypt_st == PKT_CRYPT_ST_FAIL) {
				...
				dev_err(
				    ...
				    this_task->on_cpu, this_task->pid, cur_prio,
				    skb, cont_crypto_failure, ta->paused);
		...
		list_for_each_entry(ta, &sc->encrypt_task_head, list)
		{
			if (ta->running != 0) {
				...
				if (ta->cpu_no == cpu_id) {
					...
					spin_lock_irqsave(&sc->crypt_st_lock,
							  flags);
					dev_warn(sc->dev, "Crypto running on "
							  "the same core. (%d, "
							  "%d)\n",
						 cpu_id, num_running);
					spin_unlock_irqrestore(
...
int ssv6xxx_tx_task(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		before_timeout = wait_event_interruptible_timeout(
		    sc->tx_wait_q, (skb_queue_len(&sc->tx_skb_q) ||
				    kthread_should_stop() || sc->tx_q_empty),
	...
	while (!kthread_should_stop()) {
		...
		before_timeout = wait_event_interruptible_timeout(
		    ...
		    (skb_queue_len(&sc->rx_skb_q) ||
		     skb_queue_len(&sc->tx_done_q) || kthread_should_stop()),
		    wait_period);
		if (kthread_should_stop()) {
...
struct ssv6xxx_iqk_cfg init_iqk_cfg = {
	...
	{ SSV6XXX_IQK_TEMPERATURE + SSV6XXX_IQK_RXDC + SSV6XXX_IQK_RXRC +
				 ...
				 struct ieee80211_vif *vif)
{
	...
	if ((sc->nvif >= SSV6200_MAX_VIF) ||
	    (((vif->type == NL80211_IFTYPE_AP) || (vif->p2p)) &&
	     (sc->ap_vif != NULL))) {
...
static int ssv6200_config(struct ieee80211_hw *hw, u32 changed)
{
	...
	if (changed & IEEE80211_CONF_CHANGE_CHANNEL) {
		...
		if (hw->conf.flags & IEEE80211_CONF_OFFCHANNEL) {
			if ((sc->ap_vif == NULL) ||
			    list_empty(&((struct ssv_vif_priv_data *)
					 sc->ap_vif->drv_priv)->sta_list)) {
				HCI_PAUSE(sc->sh, (TXQ_EDCA_0 | TXQ_EDCA_1 |
						   TXQ_EDCA_2 | TXQ_EDCA_3));
				sc->sc_flags |= SC_OP_OFFCHAN;
				ssv6xxx_set_channel(sc, chan->hw_value);
				sc->hw_chan = chan->hw_value;
			} else {
				dev_dbg(sc->dev, "Off-channel to %d is ignored "
						 "when AP mode enabled.\n",
					chan->hw_value);
			}
		} else {
			if ((sc->cur_channel == NULL) ||
				...
				ssv6xxx_set_channel(sc, chan->hw_value);
				sc->cur_channel = chan;
				HCI_RESUME(sc->sh, (TXQ_EDCA_0 | TXQ_EDCA_1 |
						    TXQ_EDCA_2 | TXQ_EDCA_3));
				sc->sc_flags &= ~SC_OP_OFFCHAN;
			} else {
				dev_dbg(sc->dev,
...
#endif
			if (0 !=
			    ssv6xxx_beacon_enable(sc, info->enable_beacon)) {
...
static int ssv6200_sta_add(struct ieee80211_hw *hw, struct ieee80211_vif *vif,
			   struct ieee80211_sta *sta)
{
	...
	do {
		...
		if ((sta_priv_dat->has_hw_encrypt ||
			...
			if (sramKey->sta_key[0].pair_key_idx != 0) {
				_set_wep_hw_crypto_group_key(
				    sc, vif_info, sta_info, (void *)sramKey);
			}
		}
#endif
		...
		if (sta_info->hw_wsid >= SSV_NUM_HW_STA) {
			...
			hw_update_watch_wsid(
			    sc, sta, sta_info, sta_priv_dat->sta_idx,
			    SSV6XXX_WSID_SEC_SW,
			    SSV6XXX_WSID_OPS_HWWSID_PAIRWISE_SET_TYPE);
			hw_update_watch_wsid(
			    sc, sta, sta_info, sta_priv_dat->sta_idx,
...
#endif
		printk("Add %02x:%02x:%02x:%02x:%02x:%02x to VIF %d sw_idx=%d, "
		       "wsid=%d\n",
			       ...
			       struct ieee80211_sta *sta)
{
	...
	case STA_NOTIFY_SLEEP:
		if (!prev) {
			...
			if ((vif->type == NL80211_IFTYPE_AP) && sc->bq4_dtim &&
			   ...
			   bool set)
{
	struct ssv_softc *sc = hw->priv;
	struct ssv_sta_info *sta_info =
	    sta ? ((struct ssv_sta_priv_data *)sta->drv_priv)->sta_info : NULL;
...
static int ssv6200_conf_tx(struct ieee80211_hw *hw, u16 queue,
			   const struct ieee80211_tx_queue_params *params)
#else
static int ssv6200_conf_tx(struct ieee80211_hw *hw, struct ieee80211_vif *vif,
			   u16 queue,
			   const struct ieee80211_tx_queue_params *params)
#endif
{
	...
	struct ssv_vif_priv_data *priv_vif =
	    (struct ssv_vif_priv_data *)vif->drv_priv;
	printk("[I] sv6200_conf_tx vif[%d] qos[%d] queue[%d] aifsn[%d] "
	       "cwmin[%d] cwmax[%d] txop[%d] \n",
	       priv_vif->vif_idx, vif->bss_conf.qos, queue, params->aifs,
	       params->cw_min, params->cw_max, params->txop);
#else
	printk("[I] sv6200_conf_tx queue[%d] aifsn[%d] cwmin[%d] cwmax[%d] "
	       "txop[%d] \n",
...
#endif
		printk(KERN_ERR "IEEE80211_AMPDU_RX_START "
...
struct ssv_crypto_data *ssv6xxx_skb_get_tx_cryptops(struct sk_buff *mpdu)
{
	...
	if (unicast) {
		if (sta) {
			...
			printk(KERN_ERR "Unicast to NULL STA frame. "
					"%02X:%02X:%02X:%02X:%02X:%02X -> "
					"%02X:%02X:%02X:%02X:%02X:%02X)\n",
			       hdr->addr1[0], hdr->addr1[1], hdr->addr1[2],
			       hdr->addr1[3], hdr->addr1[4], hdr->addr1[5],
...
int ssv6xxx_skb_pre_encrypt(struct sk_buff *mpdu, struct ssv_softc *sc)
{
	...
	if ((crypto_data->ops != NULL) &&
		...
		ret = crypto_data->ops->encrypt_prepare(mpdu, hdrlen,
							crypto_data->priv);
	} else {
		ret = -EOPNOTSUPP;
	}
			    ...
			    struct ssv_softc *sc)
{
	...
	if ((crypto_data->ops != NULL) &&
		...
		u32 hdrlen = ieee80211_hdrlen(hdr->frame_control);
		ret = crypto_data->ops->decrypt_prepare(mpdu, hdrlen,
							crypto_data->priv);
	} else {
		ret = -EOPNOTSUPP;
	}
...
int ssv6xxx_skb_encrypt(struct sk_buff *mpdu, struct ssv_softc *sc)
{
	...
	if (sta || unicast) {
		...
		struct ssv_vif_priv_data *vif_priv =
		    (struct ssv_vif_priv_data *)tx_info->control.vif->drv_priv;
	...
	if ((crypto_data == NULL) || (crypto_data->ops == NULL) ||
		...
		u32 hdrlen = ieee80211_hdrlen(hdr->frame_control);
		ret = crypto_data->ops->encrypt_mpdu(mpdu, hdrlen,
						     crypto_data->priv);
	}
	END_READ_CRYPTO_DATA(crypto_data);
	return ret;
						    ...
						    struct sk_buff *mpdu)
{
	...
	if ((sta->drv_priv != NULL) &&
	    (vif_info->if_type == NL80211_IFTYPE_AP)) {
		...
		printk("[Local Crypto]: No useful drv_priv, sta = %p, unicast "
		       "= %d, vif_priv = %p",
			...
			struct ssv_softc *sc)
{
	...
	if (crypto_data != NULL) {
		...
		if ((crypto_data->ops != NULL) && (crypto_data->priv != NULL))
			ret = crypto_data->ops->decrypt_mpdu(mpdu, hdrlen,
							     crypto_data->priv);
		END_READ_CRYPTO_DATA(crypto_data);
		return ret;
	}
			...
			if (current_level < input_level) {
				for (i = 0; i < size; i++) {
					if (input_level <=
						...
						SMAC_REG_WRITE(
							...
							.adjust_cca_control);
						SMAC_REG_WRITE(
						    sc->sh, ADR_RX_11B_CCA_1,
						    adjust_cci[i].adjust_cca_1);
#ifdef DEBUG_MITIGATE_CCI
						printk("##Set to the "
						       "0xce0020a0[%x] "
						       "0xce002008[%x]##!!\n",
				...
				for (i = (size - 1); i >= 0; i--) {
					if (input_level >=
						...
						printk(
						    "gate=%d, input_level=%d, "
						    "adjust_cci[%d].down_level="
						    "%d, value=%08x\n",
						    current_gate, input_level,
						    i, adjust_cci[i].down_level,
						    adjust_cci[i]
							.adjust_cca_control);
#endif
						current_level = input_level;
...
static void _proc_data_rx_skb(struct ssv_softc *sc, struct sk_buff *rx_skb)
{
	...
	rxphypad =
	    (struct
	     ssv6200_rxphy_info_padding *)(rx_skb->data + rx_skb->len -
					   sizeof(struct
	...
	if (rxdesc->rate_idx < SSV62XX_G_RATE_INDEX && rxphypad->RSVD == 0) {
		if ((ieee80211_is_beacon(hdr->frame_control)) ||
			...
			if (sta) {
                ...
                mutex_lock(&sc->mutex);
				list_for_each_entry(
				    p_rssi_res, &rssi_res.rssi_list, rssi_list)
				{
					if (!memcmp(p_rssi_res->bssid,
						    hdr->addr2, ETH_ALEN)) {
						{
							p_rssi_res->rssi =
							    ((rxphypad->rpci
...
#ifdef SSV_RSSI_DEBUG
				printk(
				    "gn_beacon %02X:%02X:%02X:%02X:%02X:%02X "
				    "rssi=%d, snr=%d\n",
				    hdr->addr2[0], hdr->addr2[1], hdr->addr2[2],
				    hdr->addr2[3], hdr->addr2[4], hdr->addr2[5],
				    rxphy->rpci, rxphy->snr);
#endif
				if (sta_priv->beacon_rssi) {
					sta_priv->beacon_rssi =
					    ((rxphy->rpci
...
#ifdef SSV_RSSI_DEBUG
		printk("########unicast: %d, b_rssi/snr: %d/%d, gn_rssi/snr: "
		       "%d/%d, rate:%d###############\n",
	...
	if (sc->dbg_rx_frame) {
		_ssv6xxx_hexdump("============================================="
				 "===================\n"
				 "RX frame",
				 (const u8 *)rx_skb->data, rx_skb->len);
	}
	skb_pull(rx_skb, SSV6XXX_RX_DESC_LEN);
	skb_trim(rx_skb, rx_skb->len - sc->sh->rx_pinfo_pad);
#ifdef CONFIG_P2P_NOA
	...
	if ((rx_hw_dec == false) && (do_sw_dec == true)) {
		...
		if (ret < 0) {
			dev_err(sc->dev,
				"[Local Crypto]: Fail to decrypt local: "
		...
		if (ret == 0) {
			...
			ret = ssv6xxx_skb_decrypt(rx_skb, sta, sc);
			if (ret < 0) {
				dev_err(sc->dev, "[Local Crypto]: Fail to "
						 "decrypt local: "
						 "%02X:%02X:%02X:%02X:%02X:%"
						 "02X, ret = %d.\n",
					hdr->addr2[0], hdr->addr2[1],

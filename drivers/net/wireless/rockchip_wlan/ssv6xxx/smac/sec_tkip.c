...
static void *lib80211_tkip_init(int key_idx)
{
	...
	priv->tx_tfm_michael =
	    crypto_alloc_hash("michael_mic", 0, CRYPTO_ALG_ASYNC);
...
static int lib80211_tkip_hdr(struct sk_buff *skb, int hdr_len, u8 *rc4key,
...
static inline int tkip_replay_check(u32 iv32_n, u16 iv16_n, u32 iv32_o,
				    u16 iv16_o)
{
	...
	if (tkip_replay_check(iv32, iv16, tkey->rx_iv32, tkey->rx_iv16)) {
		...
		if (net_ratelimit()) {
			printk(KERN_DEBUG "TKIP: replay detected: STA=%pM"
					  ...
					  "%08x%04x\n",
			       hdr->addr2, tkey->rx_iv32, tkey->rx_iv16, iv32,
			       iv16);
		}
#endif
...
static void michael_mic_hdr(struct sk_buff *skb, u8 *hdr)
{
	...
	if (ieee80211_is_data_qos(hdr11->frame_control)) {
		hdr[12] =
		    le16_to_cpu(*((__le16 *)ieee80211_get_qos_ctl(hdr11))) &
...
static char *lib80211_tkip_print_stats(char *p, void *priv)
{
	...
	p += sprintf(p, "key[%d] alg=TKIP key_set=%d "
			...
			"replays=%d icv_errors=%d local_mic_failures=%d\n",
		     tkip->key_idx, tkip->key_set, (tkip->tx_iv32 >> 24) & 0xff,
		     (tkip->tx_iv32 >> 16) & 0xff, (tkip->tx_iv32 >> 8) & 0xff,
		     tkip->tx_iv32 & 0xff, (tkip->tx_iv16 >> 8) & 0xff,
		     tkip->tx_iv16 & 0xff, (tkip->rx_iv32 >> 24) & 0xff,
		     (tkip->rx_iv32 >> 16) & 0xff, (tkip->rx_iv32 >> 8) & 0xff,
		     tkip->rx_iv32 & 0xff, (tkip->rx_iv16 >> 8) & 0xff,
		     tkip->rx_iv16 & 0xff, tkip->dot11RSNAStatsTKIPReplays,

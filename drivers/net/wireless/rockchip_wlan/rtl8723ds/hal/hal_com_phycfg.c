...
void dump_hal_txpwr_info_2g(void *sel, _adapter *adapter, u8 rf_path)
{
	HAL_DATA_TYPE *hal_data = GET_HAL_DATA(adapter);
	int path, ch_idx, tx_num;
	...
	_RTW_PRINT_SEL(sel, "\n");
	for (path = 0; path < MAX_RF_PATH && path + 1 <= rf_path; path++) {
	...
	for (path = 0; path < MAX_RF_PATH && path + 1 <= rf_path; path++) {
		RTW_PRINT_SEL(sel, "[%c]: ", rf_path_char(path));
		for (tx_num = RF_1TX; tx_num < MAX_TX_COUNT && tx_num + 1 <= rf_path; tx_num++)
			_RTW_PRINT_SEL(sel, "%2d ", hal_data->CCK_24G_Diff[path][tx_num]);
		_RTW_PRINT_SEL(sel, "\n");
	}
	RTW_PRINT_SEL(sel, "\n");
	...
	for (path = 0; path < MAX_RF_PATH && path + 1 <= rf_path; path++) {
		RTW_PRINT_SEL(sel, "[%c]: ", rf_path_char(path));
		for (tx_num = RF_1TX; tx_num < MAX_TX_COUNT && tx_num + 1 <= rf_path; tx_num++)
			_RTW_PRINT_SEL(sel, "%2d ", hal_data->OFDM_24G_Diff[path][tx_num]);
		_RTW_PRINT_SEL(sel, "\n");
	}
	RTW_PRINT_SEL(sel, "\n");
	...
	for (path = 0; path < MAX_RF_PATH && path + 1 <= rf_path; path++) {
		RTW_PRINT_SEL(sel, "[%c]: ", rf_path_char(path));
		for (tx_num = RF_1TX; tx_num < MAX_TX_COUNT && tx_num + 1 <= rf_path; tx_num++)
			_RTW_PRINT_SEL(sel, "%2d ", hal_data->BW20_24G_Diff[path][tx_num]);
		_RTW_PRINT_SEL(sel, "\n");
	}
	RTW_PRINT_SEL(sel, "\n");
	...
	for (path = 0; path < MAX_RF_PATH && path + 1 <= rf_path; path++) {
		RTW_PRINT_SEL(sel, "[%c]: ", rf_path_char(path));
		for (tx_num = RF_1TX; tx_num < MAX_TX_COUNT && tx_num + 1 <= rf_path; tx_num++)
			_RTW_PRINT_SEL(sel, "%2d ", hal_data->BW40_24G_Diff[path][tx_num]);
		_RTW_PRINT_SEL(sel, "\n");
	}
	RTW_PRINT_SEL(sel, "\n");
}

void dump_hal_txpwr_info_5g(void *sel, _adapter *adapter, u8 rf_path)
{
	HAL_DATA_TYPE *hal_data = GET_HAL_DATA(adapter);
	int path, ch_idx, tx_num;
	u8 dump_section = 0;
	...
	for (path = 0; path < MAX_RF_PATH && path + 1 <= rf_path; path++) {
		RTW_PRINT_SEL(sel, "[%c]: ", rf_path_char(path));
		for (tx_num = RF_1TX; tx_num < MAX_TX_COUNT && tx_num + 1 <= rf_path; tx_num++)
			_RTW_PRINT_SEL(sel, "%2d ", hal_data->OFDM_5G_Diff[path][tx_num]);
		_RTW_PRINT_SEL(sel, "\n");
	}
	RTW_PRINT_SEL(sel, "\n");
	...
	for (path = 0; path < MAX_RF_PATH && path + 1 <= rf_path; path++) {
		RTW_PRINT_SEL(sel, "[%c]: ", rf_path_char(path));
		for (tx_num = RF_1TX; tx_num < MAX_TX_COUNT && tx_num + 1 <= rf_path; tx_num++)
			_RTW_PRINT_SEL(sel, "%2d ", hal_data->BW20_5G_Diff[path][tx_num]);
		_RTW_PRINT_SEL(sel, "\n");
	}
	RTW_PRINT_SEL(sel, "\n");
	...
	for (path = 0; path < MAX_RF_PATH && path + 1 <= rf_path; path++) {
		RTW_PRINT_SEL(sel, "[%c]: ", rf_path_char(path));
		for (tx_num = RF_1TX; tx_num < MAX_TX_COUNT && tx_num + 1 <= rf_path; tx_num++)
			_RTW_PRINT_SEL(sel, "%2d ", hal_data->BW40_5G_Diff[path][tx_num]);
		_RTW_PRINT_SEL(sel, "\n");
	}
	RTW_PRINT_SEL(sel, "\n");
	...
	for (path = 0; path < MAX_RF_PATH && path + 1 <= rf_path; path++) {
		RTW_PRINT_SEL(sel, "[%c]: ", rf_path_char(path));
		for (tx_num = RF_1TX; tx_num < MAX_TX_COUNT && tx_num + 1 <= rf_path; tx_num++)
			_RTW_PRINT_SEL(sel, "%2d ", hal_data->BW80_5G_Diff[path][tx_num]);
		_RTW_PRINT_SEL(sel, "\n");
	}
	RTW_PRINT_SEL(sel, "\n");
}

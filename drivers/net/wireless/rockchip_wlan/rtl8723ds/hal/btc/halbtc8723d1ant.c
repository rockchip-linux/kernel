...
u32	glcoex_ver_date_8723d_1ant = 20160510;
u32	glcoex_ver_8723d_1ant = 0x09;
...
void halbtc8723d1ant_action_bt_inquiry(IN struct btc_coexist *btcoexist)
{
	...
	} else if ( ((wifi_scan) || (wifi_busy)) && (coex_sta->bt_create_connection)) {
				    ...
				    IN boolean back_up, IN boolean wifi_only)
{
	...
	if (btcoexist->btc_read_1byte(btcoexist, 0x80) == 0xc6)
		halbtc8723d1ant_post_state_to_bt(btcoexist, BT_8723D_1ANT_SCOREBOARD_ONOFF, true);
	...
	if (wifi_only) {
		...
		halbtc8723d1ant_set_ant_path(btcoexist, BTC_ANT_PATH_WIFI,
					     FORCE_EXEC, BT_8723D_1ANT_PHASE_WLANONLY_INIT);
		...
		halbtc8723d1ant_set_ant_path(btcoexist, BTC_ANT_PATH_AUTO,
					     FORCE_EXEC, BT_8723D_1ANT_PHASE_COEX_INIT);
				     ...
				     IN u32 gen_count)
{
	...
	for (i = psd_scan->psd_start_point;
		...
		if (psd_scan->psd_report[i] > psd_scan->psd_report_max_hold[i])
			psd_scan->psd_report_max_hold[i] = psd_scan->psd_report[i];
		...
		/* search the Max Value between each-freq-point-max-hold value*/
		if (i == psd_scan->psd_start_point) {
			i_max = i;
			val_max = psd_scan->psd_report_max_hold[i];
		} else if (psd_scan->psd_report_max_hold[i] > val_max){ 
			i_max = i;
			val_max = psd_scan->psd_report_max_hold[i];
		}
	...
	do {
		...
		case 3:
			...
			for (j = 1; j <= loopcnt; j++) {
				...
				if (psd_scan->psd_loop_max_value[j-1] >= 4000) {
					psd_sum = psd_sum + psd_scan->psd_loop_max_value[j-1];
					avg_cnt++;
				}

				BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
						"Loop=%d, Max_dB_data = %d\n",
						    j, psd_scan->psd_loop_max_value[j-1]);
				BTC_TRACE(trace_buf);
       ...
       *btcoexist)
{
	...
	do {
...
#if 1
			psd_scan->ant_det_psd_scan_peak_val =
			( (psd_scan->psd_max_value - psd_scan->psd_avg_value) <= 800)? 
		...
		case 4:
			...
			psd_rep2 = psd_scan->ant_det_psd_scan_peak_val - psd_rep1 *
				   100;
			...
			if (bt_resp == false) {
				...
				ant_det_finish = true;
				board_info->btdm_ant_num_by_ant_det = 2;
				coex_sta->isolation_btween_wb = (u8)(85 - psd_scan->ant_det_psd_scan_peak_val / 100) & 0xff;
				BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
		...
		*btcoexist)
{
	...
	} else if (!psd_scan->ant_det_is_ant_det_available) { /* Antenna initial setup is not ready */
		ant_det_finish = false;
...
void ex_halbtc8723d1ant_display_coex_info(IN struct btc_coexist *btcoexist)
{
	...
	CL_SPRINTF(cli_buf, BT_TMP_BUF_SIZE,
		   "\r\n %-35s = %d_%x/ 0x%x/ 0x%x(%d)/ v%d/ %c",
		   "Coex/ WL_FW/ BT_FW/ PHY/ Kt",
		   glcoex_ver_date_8723d_1ant, glcoex_ver_8723d_1ant, fw_ver,
		   bt_patch_ver, bt_patch_ver, phyver, coex_sta->cut_version + 65);
	CL_PRINTF(cli_buf);
	...
	if (bt_link_info->hid_exist) {
		CL_SPRINTF(cli_buf, BT_TMP_BUF_SIZE, "\r\n %-35s = %d/ %d/ %d",
			   "HID BusyNum/PairCnt/Forbid_Slot",
			 coex_sta->hid_busy_num,
	...
	CL_SPRINTF(cli_buf, BT_TMP_BUF_SIZE, "\r\n %-35s = %s / %s/ 0x%x/ 0x%02x",
			   "Role/IgnWlanAct/Feature/Coex_Ver",
...
*/
	...
	CL_SPRINTF(cli_buf, BT_TMP_BUF_SIZE, "\r\n %-35s = %s/ %s",
		   "LTE Coex/Path Owner",
		((lte_coex_on)? "On":"Off") , ((u8tmp[0] & BIT(2)) ? "WL" : "BT"));
	CL_PRINTF(cli_buf);
	...
	CL_SPRINTF(cli_buf, BT_TMP_BUF_SIZE, "\r\n %-35s = %s/ %s/ %s",
		   ...
		   ((u32tmp[0] & BIT(12))? "SW" : "HW"), ((u32tmp[0] & BIT(14))?  "SW" : "HW"),
		   ((u8tmp[0] & BIT(3)) ? "On" : "Off"));
	CL_PRINTF(cli_buf);

	CL_SPRINTF(cli_buf, BT_TMP_BUF_SIZE, "\r\n %-35s = %d/ %d",
		   "GNT_WL/GNT_BT",
		(int)((u32tmp[1] & BIT(2)) >> 2), (int)((u32tmp[1] & BIT(3)) >> 3));
	CL_PRINTF(cli_buf);
	...
	CL_SPRINTF(cli_buf, BT_TMP_BUF_SIZE, "\r\n %-35s = 0x%x/ 0x%x/ 0x%x/ 0x%x",
		...
		(int)((u8tmp[2] & BIT(3)) >> 3), (int)((u8tmp[3] & BIT(7)) >> 7));
	CL_PRINTF(cli_buf);
...
void ex_halbtc8723d1ant_ips_notify(IN struct btc_coexist *btcoexist, IN u8 type)
{
	...
	if (BTC_IPS_ENTER == type) {
		...
		halbtc8723d1ant_post_state_to_bt(btcoexist, BT_8723D_1ANT_SCOREBOARD_ACTIVE, false);
		...
		BTC_TRACE(trace_buf);
		halbtc8723d1ant_post_state_to_bt(btcoexist, BT_8723D_1ANT_SCOREBOARD_ACTIVE, true);
				    ...
				    IN u8 type)
{
	...
	if (BTC_SCAN_START == type) {
		...
		halbtc8723d1ant_post_state_to_bt(btcoexist, BT_8723D_1ANT_SCOREBOARD_SCAN, true);
		halbtc8723d1ant_post_state_to_bt(btcoexist, BT_8723D_1ANT_SCOREBOARD_ACTIVE, true);
		...
		halbtc8723d1ant_post_state_to_bt(btcoexist, BT_8723D_1ANT_SCOREBOARD_SCAN, false);
				       ...
				       IN u8 *tmp_buf, IN u8 length)
{
	...
	if (BT_INFO_SRC_8723D_1ANT_WIFI_FW != rsp_source) {
		...
		coex_sta->bt_whck_test = ( (coex_sta->bt_info == 0xff) ? true:false);

		coex_sta->bt_create_connection = ( (coex_sta->bt_info_c2h[rsp_source][2] & 0x80) ? true:false);

		/* unit: %, value-100 to translate to unit: dBm  */
		coex_sta->bt_rssi = coex_sta->bt_info_c2h[rsp_source][3] * 2 + 10;

		coex_sta->c2h_bt_remote_name_req = ( (coex_sta->bt_info_c2h[rsp_source][2] & 0x20) ? true:false);

		coex_sta->is_A2DP_3M = ( (coex_sta->bt_info_c2h[rsp_source][2] & 0x10) ? true:false);

		coex_sta->acl_busy =  ( (coex_sta->bt_info_c2h[rsp_source][1] & 0x9) ? true:false);

		coex_sta->voice_over_HOGP = ((coex_sta->bt_info_ext & 0x10) ? true: false);

		coex_sta->c2h_bt_inquiry_page = ((coex_sta->bt_info & BT_INFO_8723D_1ANT_B_INQ_PAGE) ? true: false);

		coex_sta->a2dp_bit_pool = (((coex_sta->bt_info_c2h[rsp_source][1] & 0x49) == 0x49)? 
			coex_sta->bt_info_c2h[rsp_source][6] : 0);

		coex_sta->bt_retry_cnt =coex_sta->bt_info_c2h[rsp_source][2] & 0xf;
		...
		if ((!btcoexist->manual_control) && (!btcoexist->stop_coex_dm)) {
			...
			if ((coex_sta->bt_info_ext & BIT(1))) {
				...
				BTC_TRACE(trace_buf);
				if (wifi_connected)
					halbtc8723d1ant_update_wifi_channel_info(btcoexist, BTC_MEDIA_CONNECT);					
				else
					halbtc8723d1ant_update_wifi_channel_info(btcoexist, BTC_MEDIA_DISCONNECT);	
			}
		...
		IN u8 type)
{
	...
	if (BTC_RF_ON == type) {
		...
		halbtc8723d1ant_post_state_to_bt(btcoexist, BT_8723D_1ANT_SCOREBOARD_ACTIVE, FALSE);
		halbtc8723d1ant_post_state_to_bt(btcoexist, BT_8723D_1ANT_SCOREBOARD_ONOFF, FALSE);
...
void ex_halbtc8723d1ant_periodical(IN struct btc_coexist *btcoexist)
{
	...
	if  ( (coex_sta->bt_coex_supported_feature == 0) && (!coex_sta->bt_disabled) )
		...
		IN u32 cent_freq, IN u32 offset, IN u32 span, IN u32 seconds)
{
	...
	if (!board_info->btdm_ant_det_finish) {
		...
		if (psd_scan->ant_det_inteval_count >=
			...
			board_info->btdm_ant_det_finish = halbtc8723d1ant_psd_antenna_detection_check(btcoexist);
			...
			if (board_info->btdm_ant_det_finish) {
				...
				if (board_info->btdm_ant_num_by_ant_det == 2){
					...
					halbtc8723d1ant_set_ant_path(btcoexist, BTC_ANT_PATH_WIFI, FORCE_EXEC,
				     BT_8723D_1ANT_PHASE_2G_RUNTIME);
				} else 
					halbtc8723d1ant_set_ant_path(btcoexist, BTC_ANT_PATH_AUTO, FORCE_EXEC,
				     BT_8723D_1ANT_PHASE_2G_RUNTIME);
				...
				/*for 8723d, btc_set_bt_trx_mask is just used to 
				                   notify BT stop le tx, not set BT RF TRx Mask  */
#if 1				                   
				btcoexist->btc_set_bt_trx_mask(btcoexist, 0);
				BTC_SPRINTF(trace_buf, BT_TMP_BUF_SIZE,
							"xxxxxx AntennaDetect(), Notify BT stop le tx by set_bt_trx_mask!\n");
				BTC_TRACE(trace_buf);
#endif				

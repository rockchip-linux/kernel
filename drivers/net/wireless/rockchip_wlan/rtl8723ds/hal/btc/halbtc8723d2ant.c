...
u32	glcoex_ver_date_8723d_2ant = 20160510;
u32	glcoex_ver_8723d_2ant = 0x09;
	...
	switch (phase)
	{
		...
		case BT_8723D_2ANT_PHASE_COEX_INIT:
				...
				halbtc8723d2ant_ltecoex_set_coex_table(btcoexist,
									       BT_8723D_2ANT_CTT_WL_VS_LTE, 0xffff);

				/* GNT_BT_LTE always = 1 (this should be config if LTE coex is required) */
				halbtc8723d2ant_ltecoex_set_coex_table(btcoexist,
						       BT_8723D_2ANT_CTT_BT_VS_LTE, 0xffff);
				    ...
				    IN boolean wifi_only)
{
	...
	if (btcoexist->btc_read_1byte(btcoexist, 0x80) == 0xc6)
		halbtc8723d2ant_post_state_to_bt(btcoexist, BT_8723D_2ANT_SCOREBOARD_ONOFF, true);
	...
	do {
		...
		case 1: /* stop coex DM & set antenna path */
			...
			/* Set Antenna path, switch WiFi to un-certain antenna port */
 	                /* Set Antenna Path,  both GNT_WL/GNT_BT = 1, and control by SW */
			halbtc8723d2ant_set_ant_path(btcoexist, BTC_ANT_PATH_BT, FORCE_EXEC,
		...
		*btcoexist)
{
	...
	if (scan || roam) {
		...
		ant_det_finish = halbtc8723d2ant_psd_antenna_detection(btcoexist);
...
void ex_halbtc8723d2ant_display_coex_info(IN struct btc_coexist *btcoexist)
{
	...
	CL_SPRINTF(cli_buf, BT_TMP_BUF_SIZE, "\r\n %-35s = %s / %s/ 0x%x/ 0x%02x",
		   ...
		   ((bt_link_info->slave_role) ? "Slave" : "Master"),
			...
			coex_sta->bt_coex_supported_version & 0xff);
	CL_PRINTF(cli_buf);
...
void ex_halbtc8723d2ant_ips_notify(IN struct btc_coexist *btcoexist, IN u8 type)
{
	...
	if (BTC_IPS_ENTER == type) {
		...
		halbtc8723d2ant_post_state_to_bt(btcoexist, BT_8723D_2ANT_SCOREBOARD_ACTIVE, false);
		...
		halbtc8723d2ant_post_state_to_bt(btcoexist, BT_8723D_2ANT_SCOREBOARD_ACTIVE, true);
				    ...
				    IN u8 type)
{
	...
	if (BTC_SCAN_START == type) {
		...
		halbtc8723d2ant_post_state_to_bt(btcoexist, BT_8723D_2ANT_SCOREBOARD_SCAN, true);
		halbtc8723d2ant_post_state_to_bt(btcoexist, BT_8723D_2ANT_SCOREBOARD_ACTIVE, true);
		...
		halbtc8723d2ant_post_state_to_bt(btcoexist, BT_8723D_2ANT_SCOREBOARD_SCAN, false);
				       ...
				       IN u8 *tmp_buf, IN u8 length)
{
	...
	if (BT_INFO_SRC_8723D_2ANT_WIFI_FW != rsp_source) {
		...
		coex_sta->c2h_bt_inquiry_page = ((coex_sta->bt_info & BT_INFO_8723D_2ANT_B_INQ_PAGE) ? true: false);
		...
		IN u8 type)
{
	...
	if (BTC_RF_ON == type) {
		...
		halbtc8723d2ant_post_state_to_bt(btcoexist, BT_8723D_2ANT_SCOREBOARD_ACTIVE, FALSE);
		halbtc8723d2ant_post_state_to_bt(btcoexist, BT_8723D_2ANT_SCOREBOARD_ONOFF, FALSE);
		btcoexist->stop_coex_dm = true;
		...
		IN u32 cent_freq, IN u32 offset, IN u32 span, IN u32 seconds)
{
	...
	if (!board_info->btdm_ant_det_finish) {
		...
		if (psd_scan->ant_det_inteval_count >=
			...
			board_info->btdm_ant_det_finish = halbtc8723d2ant_psd_antenna_detection_check(btcoexist);
			...
			if (board_info->btdm_ant_det_finish) {
				...
				/*for 8723d, btc_set_bt_trx_mask is just used to notify BT stop le tx,
															not set BT RF TRx Mask  */

...
void EXhalbtcoutsrc_rx_rate_change_notify(PBTC_COEXIST	pBtCoexist, u8 btc_rate_id)
{
	if (!halbtcoutsrc_IsBtCoexistAvailable(pBtCoexist))
		return;
	...
	if (IS_HARDWARE_TYPE_8821C(pBtCoexist->Adapter)) {
		if (pBtCoexist->board_info.btdm_ant_num == 1)
			ex_halbtc8821c1ant_rx_rate_change_notify(pBtCoexist, btc_rate_id);
		else if (pBtCoexist->board_info.btdm_ant_num == 2)
			ex_halbtc8821c2ant_rx_rate_change_notify(pBtCoexist, btc_rate_id);
	}
#endif
}
...
void hal_btcoex_rx_rate_change_notify(PADAPTER padapter, u8 btc_rate_id)
{
	EXhalbtcoutsrc_rx_rate_change_notify(&GLBtCoexist, btc_rate_id);
}

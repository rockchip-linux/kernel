...
)
{
	...
	/* struct PHY_DM_STRUCT			*pDM_Odm = &pHalData->dm_out_src; */

	/* check threshold */
	if (threshold <= 0x0)
		threshold = 0x16;

	RTW_INFO("===>phy_SpurCalibration_8723B: Channel = %d\n", ToChannel);
...
)
{
	...
	default:
		RTW_INFO("phy_PostSetBWMode8723B():	unknown Bandwidth: %#X\n", CurrentBW);
		break;
	}
}
...
)
{
	...
	RTW_INFO("===>phy_SwChnl8723B: Channel = %d\n", channelToSW);

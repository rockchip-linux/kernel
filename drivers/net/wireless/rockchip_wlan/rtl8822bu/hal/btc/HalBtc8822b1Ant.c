...
u32	glcoex_ver_date_8822b_1ant = 20160816;
u32	glcoex_ver_8822b_1ant = 0x28;
u32	glcoex_ver_btdesired_8822b_1ant = 0x16;
...
)
{
	...
	if (wifi_active)
		halbtc8822b1ant_write_score_board(btcoexist, (u16) BIT(0),
						  true);
	else
		halbtc8822b1ant_write_score_board(btcoexist, (u16) BIT(0),
						  false);
...
)
{
	...
	if (wifi_on)
		halbtc8822b1ant_write_score_board(btcoexist, (u16) BIT(1),
						  true);
	else
		halbtc8822b1ant_write_score_board(btcoexist, (u16) BIT(1),
						  false);
		     ...
		     IN boolean force_exec, IN boolean turn_on, IN u8 type)
{
	...
	if (turn_on) {
		...
		case 44:/*A2DP+OPP old */
			...
			break;
		case 45:/*OPP new -> WIFI FW check the ncuu_p ack of AP , if has no null_p ack , wifi fw will extended wifi slot */
			halbtc8822b1ant_set_fw_pstdma(btcoexist,
					      0x51, 0x29, 0x3, 0x10, 0x10);
			break;
		case 46:/*A2DP+OPP new-> WIFI FW check the ncuu_p ack of AP , if has no null_p ack , wifi fw will extended wifi slot*/
			halbtc8822b1ant_set_fw_pstdma(btcoexist,
					      0x51, 0x1a, 0x3, 0x10, 0x10);
			break;
		case 47:/*A2DP+OPP new-> WIFI FW check the ncuu_p ack of AP , if has no null_p ack , wifi fw will extended wifi slot*/
			halbtc8822b1ant_set_fw_pstdma(btcoexist,
					      0x51, 0x32, 0x3, 0x10, 0x10);
			break;
		case 48:/*A2DP+OPP new-> WIFI FW check the ncuu_p ack of AP , if has no null_p ack , wifi fw will extended wifi slot*/
			halbtc8822b1ant_set_fw_pstdma(btcoexist,
					      0x51, 0x29, 0x3, 0x10, 0x10);
			break;
...
void ex_halbtc8822b1ant_display_coex_info(IN struct btc_coexist *btcoexist)
{
	...
	if (psd_scan->ant_det_try_count == 0) {
		...
		if (board_info->btdm_ant_det_finish) {
			 if (psd_scan->ant_det_result != 12)
				CL_SPRINTF(cli_buf, BT_TMP_BUF_SIZE, "\r\n %-35s = %s",
				   "Ant Det PSD Value", psd_scan->ant_det_peak_val);
			 else
				CL_SPRINTF(cli_buf, BT_TMP_BUF_SIZE,
					   "\r\n %-35s = %d", "Ant Det PSD Value",
	...
	CL_SPRINTF(cli_buf, BT_TMP_BUF_SIZE, "\r\n %-35s = %s / %s/ 0x%x/ 0x%x",
		   "Role/IgnWlanAct/Feature/BLEScan",
				...
				coex_sta->bt_coex_supported_feature, coex_sta->bt_ble_scan_type);
	CL_PRINTF(cli_buf);
...
*/
	...
	u32tmp[1] = halbtc8822bc1ant_ltecoex_indirect_read_reg(btcoexist, 0x54);
...
void ex_halbtc8822b1ant_lps_notify(IN struct btc_coexist *btcoexist, IN u8 type)
{
	...
	if (BTC_LPS_ENABLE == type) {
		...
		if (coex_sta->force_lps_on == true) { /* LPS No-32K */
			...
			halbtc8822b1ant_post_activestate_to_bt(btcoexist,
							       false);
...
void ex_halbtc8822b1ant_periodical(IN struct btc_coexist *btcoexist)
{
	...
	if (!coex_sta->bt_disabled) {
		if (coex_sta->bt_coex_supported_feature == 0)
			coex_sta->bt_coex_supported_feature =
				btcoexist->btc_get_bt_coex_supported_feature(btcoexist);

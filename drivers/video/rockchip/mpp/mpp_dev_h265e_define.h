...
struct h265e_ctu {
	...
	/*
	 * Start buffer address of ROI map
	 * ROI map holds importance levels for CTUs within a picture.
	 * The memory size is the number of CTUs of picture in bytes.
	 * For example, if there are 64 CTUs within a picture,
	 * the size of ROI map is 64 bytes.
	 * All CTUs have their ROI importance level
	 * (0 ~ 8 ; 1 byte)  in raster order.
	 * A CTU with a high ROI important level
	 * is encoded with a lower QP for higher quality.
	 * It should be given when roiEnable is 1.
	 */
	u32 addr_roi_ctu_map;

	/*
	 * Start buffer address of CTU qp map
	 * The memory size is the number of CTUs of picture in bytes.
	 * For example if there are 64 CTUs within a picture,
	 * the size of CTU map is 64 bytes.
	 * It should be given when ctuQpEnable is 1.
	 * The content of ctuQpMap directly is mapped
	 * to the Qp used to encode the CTU. I.e,

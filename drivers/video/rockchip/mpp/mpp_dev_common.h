...
static inline struct ion_handle *mpp_alloc_ion_buf(struct rockchip_mpp_dev *mpp,
						   size_t size, size_t align)
{
	...
	return ion_alloc(mpp->ion_client, size, align, heap_id_mask, 0);
}

static inline int mpp_get_dma_addr(struct rockchip_mpp_dev *mpp,
				   struct ion_handle *hdl, void *addr, u32 *len)
{
	int ret;

	if (mpp->iommu_enable)
		ret = ion_map_iommu(mpp->dev, mpp->ion_client, hdl,
				    (unsigned long *)addr,
				    (unsigned long *)len);
	else
		ret = ion_phys(mpp->ion_client, hdl,

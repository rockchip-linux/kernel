...
#define H265E_AXI_STATUS 0
#endif
static struct mpp_session *rockchip_mpp_h265e_open(
				struct rockchip_mpp_dev *mpp);
static void rockchip_mpp_h265e_release(
				struct mpp_session *isession);
static int rockchip_mpp_h265e_load_firmware(
				struct rockchip_mpp_dev *mpp);
static int rockchip_mpp_h265e_encode_one_frame(
				struct rockchip_mpp_dev *mpp,
				struct h265e_ctx *ctx, int index);
static int rockchip_mpp_h265e_get_encode_result(
				struct rockchip_mpp_dev *mpp,
				struct h265e_ctx *result);
static int rockchip_mpp_h265e_set_gop_parameter(
				struct rockchip_mpp_dev *mpp,
				int index);
static int rockchip_mpp_h265e_register_frame_buffer(
				struct rockchip_mpp_dev *mpp,
				int index);
static void rockchip_mpp_h265e_enable_clk(
				struct rockchip_mpp_dev *mpp);
static void rockchip_mpp_h265e_disable_clk(
				struct rockchip_mpp_dev *mpp);

static struct ion_handle *rockchip_mpp_h265e_ion(
				struct rockchip_mpp_dev *mpp,
				size_t len,
				size_t align,
				unsigned long *addr)
{
	int ret = 0;
	u32 tmp;
	struct ion_handle *tmp_hdl = NULL;

	tmp_hdl = mpp_alloc_ion_buf(mpp, len, align);
	if (!tmp_hdl) {
		dev_err(mpp->dev, "error: ion_alloc fail\n");
		return NULL;
	}
	ret = mpp_get_dma_addr(mpp, tmp_hdl, addr, &tmp);
	if (ret < 0) {
...
FAIL:
	if (tmp_hdl)
		ion_free(mpp->ion_client, tmp_hdl);
	return NULL;
}

static void rockchip_mpp_h265e_free_frame_buffer(
				struct rockchip_mpp_dev *mpp,
				struct rockchip_h265e_instance *instance)
{
	...
	mpp_debug_enter();
	buf = &instance->mv;
	if (!IS_ERR_OR_NULL(buf->hdl)) {
		ion_free(mpp->ion_client, buf->hdl);
		buf->hdl = NULL;
	}
...
static void rockchip_mpp_h265e_free_instance(
				struct rockchip_mpp_dev *mpp, int index)
{
	struct rockchip_h265e_dev *enc =
		container_of(mpp, struct rockchip_h265e_dev, dev);
...
static int rockchip_mpp_h265e_write_encoder_file(struct rockchip_mpp_dev *mpp)
{
	...
	struct h265e_session *session =
		container_of(ctx->ictx.session, struct h265e_session, isession);
	struct rockchip_h265e_dev *enc =
		container_of(mpp, struct rockchip_h265e_dev, dev);
...
static int rockchip_mpp_h265e_load_firmware(struct rockchip_mpp_dev *mpp)
{
	...
	enc->firmware.hdl = rockchip_mpp_h265e_ion(mpp, firmware->size,
			MPP_ALIGN_SIZE, &enc->firmware.dma_addr);
	if (IS_ERR_OR_NULL(enc->firmware.hdl)) {
		mpp_err("error: alloc firmware buffer error\n");
		goto FAIL;
	}
	enc->firmware.size = ALIGN(firmware->size, MPP_ALIGN_SIZE);
	enc->firmware_cpu_addr = ion_map_kernel(mpp->ion_client,
			enc->firmware.hdl);
	mpp_debug(DEBUG_H265E_INFO, "h265e firmware_size = %d,code_base = %x\n",
		  enc->firmware.size, (u32)enc->firmware.dma_addr);
...
FAIL:
	release_firmware(firmware);
	if (!IS_ERR_OR_NULL(enc->firmware.hdl)) {
		ion_unmap_kernel(mpp->ion_client, enc->firmware.hdl);
		ion_free(mpp->ion_client, enc->firmware.hdl);
		enc->firmware.hdl = NULL;
	}
	return -1;
}

static struct mpp_ctx *rockchip_mpp_h265e_ctx_init(
					struct rockchip_mpp_dev *mpp,
					void __user *src, u32 dwsize)
{
	...
	ctx->bs.hdl = ion_import_dma_buf(mpp->ion_client, ctx->cfg.bs_fd);
	if (IS_ERR(ctx->bs.hdl)) {
		mpp_err("import dma-buf from fd %d failed\n", ctx->cfg.bs_fd);
		return NULL;
	}

	ret = mpp_get_dma_addr(mpp, ctx->bs.hdl,
			       &ctx->bs.dma_addr, (u32 *)&size);
	ctx->bs.size = (u32)size;
#if PRINT_BS_DATA
	ctx->bs_data = ion_map_kernel(mpp->ion_client, ctx->bs.hdl);
#endif
	...
	ctx->src.hdl = ion_import_dma_buf(mpp->ion_client, ctx->cfg.src_fd);
	if (IS_ERR(ctx->src.hdl)) {
		mpp_err("import dma-buf from fd %d failed\n", ctx->cfg.src_fd);
		goto FAIL;
	}
	ret = mpp_get_dma_addr(mpp, ctx->src.hdl,
			       &ctx->src.dma_addr, (u32 *)&size);
...
FAIL:
	if (!IS_ERR_OR_NULL(ctx->bs.hdl)) {
		ion_unmap_kernel(mpp->ion_client, ctx->bs.hdl);
		ion_free(mpp->ion_client, ctx->bs.hdl);
		ctx->bs.hdl = NULL;
	}

	if (!IS_ERR_OR_NULL(ctx->src.hdl)) {
		ion_unmap_kernel(mpp->ion_client, ctx->src.hdl);
		ion_free(mpp->ion_client, ctx->src.hdl);
		ctx->src.hdl = NULL;
	}
...
int rockchip_mpp_h265e_get_stream_header(
				struct rockchip_mpp_dev *mpp,
				int index,
				struct hal_h265e_header *head)
{
	...
	bs_hd = ion_import_dma_buf(mpp->ion_client, address);
	if (IS_ERR(bs_hd)) {
		mpp_err("import dma-buf from fd %d failed\n", address);
		mutex_unlock(&enc->lock);
		return PTR_ERR(bs_hd);
	}
	ret = mpp_get_dma_addr(mpp, bs_hd, &bs_address, (u32 *)&bs_size);
	if (ret < 0) {
		mpp_err("bs fd %d ion map iommu failed\n", address);
		goto FAIL;
	}
	...
	if (!IS_ERR_OR_NULL(bs_hd)) {
		ion_free(mpp->ion_client, bs_hd);
		bs_hd = NULL;
	}
	mutex_unlock(&enc->lock);
	mpp_debug_leave();
 ...
 */
static int rockchip_mpp_h265e_set_common_parameter(
				struct rockchip_mpp_dev *mpp, int index)
{
	u32 value = 0;
	struct mpp_h265e_cfg *cfg = NULL;
...
static int rockchip_mpp_h265e_set_vui_parameter(
				struct rockchip_mpp_dev *mpp, int index)
{
	struct mpp_h265e_cfg *cfg = NULL;
	struct rockchip_h265e_dev *enc =
...
static int rockchip_mpp_h265e_set_parameter(
				struct rockchip_mpp_dev *mpp, int index)
{
	struct rockchip_h265e_dev *enc =
				container_of(mpp,
...
static int rockchip_mpp_h265e_change_parameter(
				struct rockchip_mpp_dev *mpp, int index)
{
	struct rockchip_h265e_dev *enc =
			container_of(mpp, struct rockchip_h265e_dev, dev);
				...
				struct rockchip_mpp_dev *mpp, int index)
{
	...
	if (cfg->map_type == COMPRESSED_FRAME_MAP) {
		...
		instance->mv.hdl =
			rockchip_mpp_h265e_ion(mpp, instance->mv.size,
					       MPP_ALIGN_SIZE,
					       &instance->mv.dma_addr);
		if (IS_ERR_OR_NULL(instance->mv.hdl)) {
			mpp_err("alloc mv buffer fail,index = %d\n", index);
			goto FAIL;
		}

		fbc_y_table_size =
			H265E_FBC_LUMA_TABLE_SIZE(buf_width, buf_height);
		fbc_y_table_size =
			ALIGN(fbc_y_table_size, 16);
		instance->fbc_luma.size =
			ALIGN(fbc_y_table_size * count, 4096) + 4096;
		instance->fbc_luma.hdl =
			rockchip_mpp_h265e_ion(mpp, instance->fbc_luma.size,
					       MPP_ALIGN_SIZE,
					       &instance->fbc_luma.dma_addr);
		if (IS_ERR_OR_NULL(instance->fbc_luma.hdl)) {
		...
		instance->fbc_chroma.size = ALIGN(
				fbc_c_table_size * count,
				4096) + 4096;
		instance->fbc_chroma.hdl =
			rockchip_mpp_h265e_ion(mpp,
					       instance->fbc_chroma.size,
					       MPP_ALIGN_SIZE,
					       &instance->fbc_chroma.dma_addr);
		if (IS_ERR_OR_NULL(instance->fbc_chroma.hdl)) {
	...
	instance->sub_sample.hdl =
		rockchip_mpp_h265e_ion(mpp, instance->sub_sample.size,
				       MPP_ALIGN_SIZE,
				       &instance->sub_sample.dma_addr);
	if (IS_ERR_OR_NULL(instance->sub_sample.hdl)) {
		mpp_err("alloc fbc c buffer fail,index = %d\n", index);
		goto FAIL;
	}
	...
	for (i = 0 ; i < count; i++) {
		...
		buffer->hdl = rockchip_mpp_h265e_ion(mpp, buffer->size,
				MPP_ALIGN_SIZE, &buffer->dma_addr);
		if (IS_ERR_OR_NULL(buffer->hdl)) {
			mpp_err("alloc fbc y buffer fail,index = %d\n", index);
			goto FAIL;
		}
				...
				int index)
{
	...
	if (en_info->stream_end == 1) {
		mpp_debug(DEBUG_H265E_INFO, "%s %d instance %d en_info->stream_end\n",
	...
	mpp_write(mpp, cfg->ctu.addr_roi_ctu_map,
		  H265E_CMD_ENC_ROI_ADDR_CTU_MAP);
	mpp_write(mpp, cfg->ctu.addr_ctu_qp_map,
		  H265E_CMD_ENC_CTU_QP_MAP_ADDR);

	value = ((cfg->ctu.roi_enable) << 0) |
			(cfg->ctu.roi_delta_qp << 1) |
			(cfg->ctu.ctu_qp_enable << 9) |
				     ...
				     unsigned long arg)
{
	...
	struct rockchip_h265e_instance *instance = NULL;
	struct h265e_session *session =
		container_of(isession, struct h265e_session, isession);
				...
				struct rockchip_mpp_dev *mpp)
{
	...
	if (!atomic_read(&enc->load_firmware)) {
		...
		enc->temp.hdl = rockchip_mpp_h265e_ion(mpp, enc->temp.size,
				MPP_ALIGN_SIZE, &enc->temp.dma_addr);
		if (IS_ERR_OR_NULL(enc->temp.hdl)) {
	...
	instance->status = H265E_INSTANCE_STATUS_ERROR;
	mpp_debug(DEBUG_H265E_INFO,
		  "rockchip_mpp_h265e_open instance = %d\n", index);
...
NFREE_FAIL:
	...
	session = NULL;
	mutex_unlock(&enc->lock);
	mpp_err("rockchip_mpp_h265e_open fail\n");
	return NULL;
}
...
static void rockchip_mpp_h265e_remove(struct rockchip_mpp_dev *mpp)
{
	...
	for (i = 0; i < H265E_INSTANCE_NUM; i++) {
		...
		if (atomic_read(&instance->is_used) == 1) {
			buf = &instance->work;
			if (!IS_ERR_OR_NULL(buf->hdl)) {
				ion_unmap_kernel(mpp->ion_client, buf->hdl);
				ion_free(mpp->ion_client, buf->hdl);
			}
			rockchip_mpp_h265e_free_frame_buffer(mpp, instance);
...
const struct rockchip_mpp_dev_variant h265e_variant = {
	...
	.trans_info = NULL,
	.mmu_dev_dts_name = H265E_IOMMU_COMPATIBLE_NAME,

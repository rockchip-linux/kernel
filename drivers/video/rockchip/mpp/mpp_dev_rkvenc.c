...
static void rockchip_mpp_war_init(struct rockchip_mpp_dev *mpp)
{
	...
	if (enc->dummy_ctx == NULL) {
		mpp_err("allocate dummy context for war failed\n");
		return;
	}
						...
						void __user *src, u32 dwsize)
{
	...
	for (i = 0; i < ctx->cfg.tbl_num; i++) {
		if (0 > mpp_reg_address_translate(mpp, ctx->cfg.elem[i].reg,
						  &ctx->ictx, 0)) {
...
static struct mpp_session *mpp_dev_rkvenc_open(struct rockchip_mpp_dev *mpp)
{
	...
	if (session == NULL) {
		mpp_err("can not allocate memory for instance\n");
		return NULL;
	}
...
static int rockchip_mpp_rkvenc_probe(struct rockchip_mpp_dev *mpp)
{
	...
	enc->lkt_hdl = mpp_alloc_ion_buf(mpp, LINK_TABLE_LEN * 4 * 256,
					 MPP_ALIGN_SIZE);
	if (enc->lkt_hdl == NULL) {
...
fail:
	...
	if (enc->war_hdl)
		ion_free(mpp->ion_client, enc->war_hdl);
	if (enc->lkt_cpu_addr)
		ion_unmap_kernel(mpp->ion_client, enc->lkt_hdl);
	if (enc->lkt_hdl)
		ion_free(mpp->ion_client, enc->lkt_hdl);

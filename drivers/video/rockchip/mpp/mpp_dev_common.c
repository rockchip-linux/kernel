...
int mpp_dev_debug;
module_param(mpp_dev_debug, int, S_IRUGO | S_IWUSR);
			     ...
			     int size, u32 *reg, struct mpp_ctx *ctx)
{
	...
	for (i = 0; i < size; i++) {
		...
		hdl = ion_import_dma_buf(mpp->ion_client, usr_fd);
		if (IS_ERR(hdl)) {
		...
		ret = mpp_get_dma_addr(mpp, hdl, (void *)&mem_region->iova,
				       (u32 *)&mem_region->len);
		if (ret < 0) {
...
fail:
	list_for_each_entry_safe(mem_region, n,
		...
		ion_free(mpp->ion_client, mem_region->hdl);
			  ...
			  unsigned long arg)
{
	...
	case MPP_IOC_SET_REG: {
		...
		ctx = ctx_init(mpp, session, (void __user *)req.req, req.size);
		if (NULL == ctx) {
			return -EFAULT;
		} else {
			mpp_srv_lock(mpp->srv);
	...
	default: {
		if (mpp->ops->ioctl)
			...
			mpp_err("unknow mpp ioctl cmd %x\n", cmd);
	} break;
	}
				 ...
				 unsigned long arg)
{
	...
	case MPP_IOC_GET_REG: {
		...
		return mpp_dev_wait_result(session, mpp,
...
{
	struct rockchip_mpp_dev *mpp =
		container_of(inode->i_cdev, struct rockchip_mpp_dev, cdev);
	struct mpp_session *session;
...
static int mpp_dev_probe(struct platform_device *pdev)
{
	...
	mpp->ion_client = rockchip_ion_client_create("mpp");
	if (IS_ERR(mpp->ion_client)) {
		dev_err(dev, "failed to create ion client for mpp ret %ld\n",
			PTR_ERR(mpp->ion_client));
	}
	...
	mpp->mmu_dev = rockchip_get_sysmmu_dev(mpp->variant->mmu_dev_dts_name);

	if (mpp->mmu_dev) {
		platform_set_sysmmu(mpp->mmu_dev, dev);
		rockchip_iovmm_set_fault_handler(dev, mpp_sysmmu_fault_hdl);
	} else {
		dev_err(dev,

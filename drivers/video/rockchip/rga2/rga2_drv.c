...
static void print_info(struct rga2_req *req)
{
	...
	sprintf(string, "render_mode=%d bitblt_mode=%d rotate_mode=%.8x\n",
		req->render_mode, req->bitblt_mode, req->rotate_mode);
	strcat(buf, string, sizeof(string));
	sprintf(string, "src : y=%.lx uv=%.lx v=%.lx format=%d aw=%d ah=%d",
		"vw=%d vh=%d xoff=%d yoff=%d \n",
		req->src.yrgb_addr, req->src.uv_addr, req->src.v_addr,
		req->src.format,
		req->src.act_w, req->src.act_h, req->src.vir_w, req->src.vir_h,
		req->src.x_offset, req->src.y_offset);
	strcat(buf, string, sizeof(string));
	sprintf(string, "dst : y=%lx uv=%lx v=%lx format=%d aw=%d ah=%d",
		"vw=%d vh=%d xoff=%d yoff=%d \n",
...
static void rga2_soft_reset(void)
{
	...
	rga2_write((1 << 3) | (1 << 4), RGA2_SYS_CTRL);

	for(i = 0; i < RGA2_RESET_TIMEOUT; i++) {
		reg = rga2_read(RGA2_SYS_CTRL) & 1;
...
static void rga2_dump(void)
{
	...
	list_for_each_entry_safe(session, session_tmp,
				 &rga2_service.session, list_session) {
...
static int rga2_prepare(rga2_session *session);
...
static int rga2_check_param(const struct rga2_req *req)
{
	if(!((req->render_mode == color_fill_mode))) {
		if (unlikely((req->src.act_w <= 0) || (req->src.act_w > 8191) ||
		    (req->src.act_h <= 0) || (req->src.act_h > 8191))) {
			printk("invalid source w or h act_w = %d,act_h = %d\n",
	...
	if(!((req->render_mode == color_fill_mode))) {
		if (unlikely((req->src.vir_w <= 0) || (req->src.vir_w > 8191) ||
		    (req->src.vir_h <= 0) || (req->src.vir_h > 8191))) {
			printk("invalid source w or h vir_w = %d, vir_h = %d\n",
			       req->src.vir_w, req->src.vir_h);
			return  -EINVAL;
		}
	}

	if (unlikely((req->dst.act_w <= 0) || (req->dst.act_w > 4096) ||
	...
	if (unlikely((req->dst.vir_w <= 0) || (req->dst.vir_w > 4096) ||
...
static struct rga2_reg * rga2_reg_init(rga2_session *session, struct rga2_req *req)
{
	...
	if ((mmu_info->src0_mmu_flag & 1) || (mmu_info->src1_mmu_flag & 1) ||
	    (mmu_info->dst_mmu_flag & 1) || (mmu_info->els_mmu_flag & 1)) {
		ret = rga2_set_mmu_info(reg, req);
		if(ret < 0) {
			printk("%s[%d] mmu error\n", __FUNCTION__, __LINE__);
			if (reg != NULL)
				kfree(reg);
...
static void rga2_try_set_reg(void)
{
	...
	if (list_empty(&rga2_service.running)) {
		if (!list_empty(&rga2_service.waiting)) {
			reg = list_entry(rga2_service.waiting.next,
...
#ifdef CONFIG_ARM
			dmac_flush_range(&rga2_service.cmd_buff[0],
					 &rga2_service.cmd_buff[32]);
			outer_flush_range(virt_to_phys(&rga2_service.cmd_buff[0]),
					  virt_to_phys(&rga2_service.cmd_buff[32]));
#elif defined(CONFIG_ARM64)
			__dma_flush_range(&rga2_service.cmd_buff[0],
					  &rga2_service.cmd_buff[32]);
#endif
			rga2_write(0x0, RGA2_SYS_CTRL);
			rga2_write(virt_to_phys(rga2_service.cmd_buff),
				   RGA2_CMD_BASE);
#if RGA2_TEST
			if (rga2_flag) {
				int32_t i, *p;
				...
				for (i=0; i<8; i++)
					printk("%.8x %.8x %.8x %.8x\n", p[0 + i*4],
						p[1+i*4], p[2 + i*4], p[3 + i*4]);
			}
#endif
			rga2_write((0x1<<1)|(0x1<<2)|(0x1<<5)|(0x1<<6),
				   RGA2_SYS_CTRL);
			rga2_write(rga2_read(RGA2_INT) | (0x1<<10) | (0x1<<9)
				   | (0x1<<8), RGA2_INT);
#if RGA2_TEST_TIME
...
static int rga2_blit_sync(rga2_session *session, struct rga2_req *req)
{
	...
	if (unlikely(ret_timeout< 0)) {
		pr_err("sync pid %d wait task ret %dint[0x%x]status[0x%x]\n",
		       session->pid, ret_timeout, rga2_read(RGA2_INT),
		...
		pr_err("sync pid %d wait %d task timeout:int[%x]status[%x]\n",
...
static long rga_ioctl(struct file *file, uint32_t cmd, unsigned long arg)
{
	...
	case RGA_BLIT_SYNC:
		...
		if (first_RGA2_proc == 0 && req.bitblt_mode == bitblt_mode &&
		    rga2_service.dev_mode == 1) {
		...
		break;
	case RGA_BLIT_ASYNC:
		if (unlikely(copy_from_user(&req_rga, (struct rga_req*)arg,
		...
		break;
	case RGA2_BLIT_SYNC:
		if (unlikely(copy_from_user(&req, (struct rga2_req*)arg,
		    sizeof(struct rga2_req)))) {
			ERR("copy_from_user failed\n");
			ret = -EFAULT;
			break;
		}
	...
	case RGA2_GET_VERSION:
		ret = copy_to_user((void *)arg,
				   RGA2_VERSION, sizeof(RGA2_VERSION));
		break;
	default:
...
static long compat_rga_ioctl(struct file *file, uint32_t cmd, unsigned long arg)
{
	...
	switch (cmd) {
        case RGA_BLIT_SYNC:
		if (unlikely(copy_from_user(&req_rga,
		    compat_ptr((compat_uptr_t)arg), sizeof(struct rga_req_32))))
		{
			ERR("copy_from_user failed\n");
			ret = -EFAULT;
			break;
	...
	case RGA_GET_VERSION:
	case RGA2_GET_VERSION:
		ret = copy_to_user((void *)arg, RGA2_VERSION,
				   sizeof(RGA2_VERSION));
		break;
	default:
		ERR("unknown ioctl cmd!\n");
...
static int rga2_release(struct inode *inode, struct file *file)
{
	...
	if (task_running) {
		pr_err("rga2_service session %d still has %d task running\n",
...
{
	rga2_write(rga2_read(RGA2_INT) | (0x1<<4) | (0x1<<5) | (0x1<<6) | (0x1<<7),
		   RGA2_INT);
...
static int rga2_drv_probe(struct platform_device *pdev)
{
	...
	ret = devm_request_threaded_irq(&pdev->dev, data->irq, rga2_irq,
					rga2_irq_thread, 0, "rga", pdev);
	if (ret) {
...
static int __init rga2_init(void)
{
...

	{
		...
		list_add_tail(&rga2_session_global.list_session,
			      &rga2_service.session);
...
void rga2_test_0(void)
{
	...
	for(j=0; j<320*240*10/8; j++) {
		if (src[j] != dst[j])
			printk("error value dst not equal src j %d,"
			       "s %.2x d %.2x\n", j, src[j], dst[j]);
		}
	}
#endif
...
static int rga2_prepare(rga2_session *session)
{
	...
	memset(&req, 0, sizeof(struct rga2_req));
	src = kmalloc(480*320*4, GFP_KERNEL);
	if (src == NULL)
		goto out;

	dst = kmalloc(480*320*4, GFP_KERNEL);
	if (dst == NULL)
		goto free_src;
	...
	ret = wait_event_timeout(session->wait, atomic_read(&session->done), t);

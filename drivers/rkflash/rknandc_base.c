...
int rknandc_get_reg_addr(unsigned long *p_rknandc_addr)
{
	*p_rknandc_addr = (unsigned long)g_nandc_info.reg_base;
	return 0;
}
...
void rknandc_irq_flag_init(void)
{
	init_completion(&nandc_irq_complete);
}

void rknandc_wait_for_irq_completed(void)
{
	wait_for_completion_timeout(&nandc_irq_complete,
				    msecs_to_jiffies(10));
}
...
static int rknandc_probe(struct platform_device *pdev)
{
	...
	if (unlikely(IS_ERR(g_nandc_info.clk)) ||
		...
		dev_err(&pdev->dev, "rknandc_probe get clk error\n");
		return -1;
	}

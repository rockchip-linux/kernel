...
static u32 flash_read_page_raw(u8 cs, u32 page_addr, u32 *p_data, u32 *p_spare)
{
	...
	if (error_ecc_bits != NAND_STS_ECC_ERR) {
		if (error_ecc_bits >= nand_para.ecc_bits - 3)
...
static u32 flash_prog_page(u8 cs, u32 page_addr, u32 *p_data, u32 *p_spare)
{
	...
	status &= 0x01;
	if (status)
		PRINT_I("flash_prog_page addr=%x status=%x\n",
			page_addr, status);
	return status;
}
...
static u32 flash_erase_block(u8 cs, u32 page_addr)
{
	...
	status &= 0x01;
	if (status)
		PRINT_I("flash_erase_block pageadd=%x status=%x\n",
			page_addr, status);
	return status;
}
...
u32 flash_init(void __iomem *nandc_addr)
{
	u32 cs;
...
void flash_get_id(u8 cs, void *buf)
{
	memcpy(buf, id_byte[cs], 5);
}

void flash_get_info(void *buf)
{
	u8 i;
	struct FLASH_INFO_T *p_info = (struct FLASH_INFO_T *)buf;

	p_info->block_size = nand_para.page_per_blk * nand_para.sec_per_page;
	p_info->ecc_bits = g_nand_flash_ecc_bits;
	p_info->flash_size = nand_para.plane_per_die * p_info->block_size *
			     nand_para.blk_per_plane * nand_para.die_per_chip;
	p_info->page_size = nand_para.sec_per_page;
	p_info->access_time = 32;
	p_info->manufacturer_name = nand_para.vendor;
	p_info->flash_mask = 0;
	for (i = 0; i < g_nand_max_die; i++)
		p_info->flash_mask |= 1 << die_cs_index[i];
}

u32 flash_deinit(void)
{
	return 0;
}

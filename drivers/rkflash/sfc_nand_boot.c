...
#define SNAND_VENDOR_PART_START	8
#define SNAND_VENDOR_PART_SIZE	8
#define SNAND_VENDOR_PART_NUM	4
#define SNAND_VENDOR_TAG		0x524B5644
struct tag_vendor_info {
	...
	u8	data[SNAND_VENDOR_PART_SIZE * 512 - 512 - 8];
	u32	hash;
	u32	version2;
};
...
u32 snand_vendor_init(void)
{
	...
	max_ver = 0;
	max_index = 0;
	for (i = 0; i < SNAND_VENDOR_PART_NUM; i++) {
		snand_read(SNAND_VENDOR_PART_START +
				SNAND_VENDOR_PART_SIZE * i,
				SNAND_VENDOR_PART_SIZE,
				g_vendor);
		if (g_vendor->tag == SNAND_VENDOR_TAG &&
	...
	if (max_ver) {
		snand_read(SNAND_VENDOR_PART_START +
				SNAND_VENDOR_PART_SIZE * max_index,
				SNAND_VENDOR_PART_SIZE,
		g_vendor);
	} else {
		memset(g_vendor, 0, sizeof(*g_vendor));
		g_vendor->version = 1;
		g_vendor->tag = SNAND_VENDOR_TAG;
...
int snand_vendor_read(u32 id, void *pbuf, u32 size)
{
	u32 i;
...
EXPORT_SYMBOL(snand_vendor_read);

int snand_vendor_write(u32 id, void *pbuf, u32 size)
{
	...
	for (i = 0; i < g_vendor->item_num; i++) {
		if (g_vendor->item[i].id == id) {
			...
			g_vendor->version2 = g_vendor->version;
			g_vendor->next_index++;
			if (g_vendor->next_index >= SNAND_VENDOR_PART_NUM)
				g_vendor->next_index = 0;
			rkflash_device_lock();
			snand_write(SNAND_VENDOR_PART_START +
					SNAND_VENDOR_PART_SIZE * next_index,
					SNAND_VENDOR_PART_SIZE,
					g_vendor);
...
EXPORT_SYMBOL(snand_vendor_write);

#if (SNAND_VENDOR_TEST)
void snand_vendor_test(void)
{
	u32 i;
	u8 test_buf[512];
	...
	for (i = 0; i < 62; i++) {
		...
		snand_vendor_write(i, test_buf, i + 1);
	}
	memset(test_buf, 0, 512);
	for (i = 0; i < 62; i++) {
		snand_vendor_read(i, test_buf, i + 1);
		PRINT_E("id = %d ,size = %d\n", i, i + 1);

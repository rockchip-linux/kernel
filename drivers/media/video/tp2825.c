...
static int tp2825_check_cvstd(struct i2c_client *client)
{
	unsigned char cvstd;
	int i, j;
...
continue_state:
	sensor_read(client, 0x01, &status);
	SENSOR_DG("%s(%d): state %d, read 0x01:0x%x\n", __func__, __LINE__, state, status);

	if (status & FLAG_LOSS) {
		if (VIDEO_UNPLUG == state) {
			count++;
		} else {
			state = VIDEO_UNPLUG;
			count = 0;
		}
	} else if (status & FLAG_LOCKED) {
		if (VIDEO_UNPLUG == state) {
			count++;
		} else {
			state = VIDEO_LOCKED;
		...
		if (VIDEO_UNPLUG == state) {
			state = VIDEO_IN;
			count = 0;
		} else if (VIDEO_LOCKED == state) {
			state = VIDEO_UNLOCK;
			count = 0;
		} else {
			count++;
			if (VIDEO_UNLOCK == state && count > 2) {
				state = VIDEO_IN;
				count = 0;
			}
		}
	...
	loop_index--;
	if ((VIDEO_IN != state) && (VIDEO_LOCKED != state) && loop_index) {
		mdelay(10);
		goto continue_state;
	}
	SENSOR_TR("%s(%d): state %s\n", __func__, __LINE__,
		  (VIDEO_UNPLUG == state) ? "UNPLUG" : (VIDEO_IN == state) ? "IN" : (VIDEO_LOCKED == state) ? "LOCKED" : "UNLOCKED");
	if (!loop_index) {
		SENSOR_TR("%s(%d): check sensor statue failed!\n", __func__, __LINE__);
		return -1;
	}

	if (VIDEO_IN == state) {
		sensor_read(client, 0x03, &cvstd);
		SENSOR_TR("%s(%d): read 0x03 return 0x%x\n", __func__, __LINE__, cvstd);

		cvstd &= 0x07;
		if (cvstd == cvstd_old)
			return 0;

		if (CVSTD_720P60 == cvstd) {
			SENSOR_TR("%s(%d): 720P60\n", __func__, __LINE__);
		} else if (CVSTD_720P50 == cvstd) {
		...
		} else if (CVSTD_1080P30 == cvstd) {
			SENSOR_TR("%s(%d): 1080P30\n", __func__, __LINE__);
		} else if (CVSTD_1080P25 == cvstd) {
			SENSOR_TR("%s(%d): 1080P25\n", __func__, __LINE__);
		} else if (CVSTD_720P30 == cvstd) {
		...
		tp2825_reinit_parameter(cvstd, sensor);
		cvstd_old = cvstd;
	} else {
		SENSOR_TR("%s(%d): check sensor statue failed!\n", __func__, __LINE__);
	...
	for (i = 0; i < ARRAY_SIZE(sensor_preview_data); i++) {
		...
		if (sensor_preview_data[i].reg == 0xB8)
			sensor_preview_data[i].val = 0x01;/*0x03*/
...
static int sensor_activate_cb(struct i2c_client *client)
{
	...
	if ((cvstd_old == cvstd_sd) && (sensor->state_check_work.state_check_wq)) {
		SENSOR_TR("%s(%d): queue_delayed_work 1000ms", __func__, __LINE__);
...
static int sensor_deactivate_cb(struct i2c_client *client)
{
	...
	SENSOR_TR("%s(%d): cancel_delayed_work_sync, ret %d.", __func__, __LINE__, ret);
	return ret;
}
/*
...
static int tp2825_debugfs_reg_show(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < ARRAY_SIZE(sensor_preview_data); i++) {
		ret = sensor_read(client, sensor_preview_data[i].reg, &val);
		if (IS_ERR_VALUE(ret))
...
static void tp2825_check_state_work(struct work_struct *work)
{
	...
	unsigned int state = VIDEO_UNPLUG;
	unsigned char status;
	...
	if (status & FLAG_LOSS) {
		...
		if (status & FLAG_V_LOCKED && status & FLAG_H_LOCKED) {
			state = VIDEO_IN;
		} else {
			cvstd_sd = (CVSTD_NTSC == cvstd_sd) ? CVSTD_PAL : CVSTD_NTSC;
			SENSOR_TR("tp2825_check_state_func: cvstd_sd is %s\n", (cvstd_sd == CVSTD_PAL) ? "PAL" : "NTSC");
			cvstd_old = cvstd_sd;
			tp2825_check_cvstd(client);
			tp2825_reinit_parameter(cvstd_old, sensor);
			tp2825_send_uevent(sensor);
			state = VIDEO_UNLOCK;
		}
	}

	SENSOR_DG("%s(%d): read 0x01 0x%x, state %d\n", __func__, __LINE__, status, state);

	queue_delayed_work(sensor->state_check_work.state_check_wq, &sensor->state_check_work.work, 100);
}

	...
	if (state == VIDEO_IN) {
		...
		if (cvstd == CVSTD_720P30) {
			...
			for (i = 0; i < ARRAY_SIZE(sensor_preview_data); i++)
				for (j = 0; j < ARRAY_SIZE(sensor_preview_data_720p_30hz); j++)
					if (sensor_preview_data[i].reg == sensor_preview_data_720p_30hz[j].reg)
						sensor_preview_data[i].val = sensor_preview_data_720p_30hz[j].val;
		} else if (cvstd == CVSTD_720P25) {
			cvstd_mode = CVSTD_720P25;
			SENSOR_TR("%s(%d): 720P25\n", __func__, __LINE__);
			for (i = 0; i < ARRAY_SIZE(sensor_preview_data); i++)
				for (j = 0; j < ARRAY_SIZE(sensor_preview_data_720p_25hz); j++)
					if (sensor_preview_data[i].reg == sensor_preview_data_720p_25hz[j].reg)
						sensor_preview_data[i].val = sensor_preview_data_720p_25hz[j].val;
			...
			for (i = 0; i < ARRAY_SIZE(sensor_preview_data); i++)
				for (j = 0; j < ARRAY_SIZE(sensor_preview_data_720p_50hz); j++)
					if (sensor_preview_data[i].reg == sensor_preview_data_720p_50hz[j].reg)
						sensor_preview_data[i].val = sensor_preview_data_720p_50hz[j].val;
			...
			if (!activate_check || is_first)
				cvstd_sd = (CVSTD_NTSC == cvstd_sd) ? CVSTD_PAL : CVSTD_NTSC;
			...
			if (cvstd_mode == CVSTD_PAL) {
				for (i = 0; i < ARRAY_SIZE(sensor_preview_data); i++)
					for (j = 0; j < ARRAY_SIZE(sensor_preview_data_pal); j++)
						if (sensor_preview_data[i].reg == sensor_preview_data_pal[j].reg)
							sensor_preview_data[i].val = sensor_preview_data_pal[j].val;
			} else {
				for (i = 0; i < ARRAY_SIZE(sensor_preview_data); i++)
					for (j = 0; j < ARRAY_SIZE(sensor_preview_data_ntsc); j++)
						if (sensor_preview_data[i].reg == sensor_preview_data_ntsc[j].reg)
							sensor_preview_data[i].val = sensor_preview_data_ntsc[j].val;
			}
		}
...
/*static irqreturn_t tp2825_irq(int irq, void *dev_id)
{
	...
	if (IS_ERR_VALUE(ret)) {
		SENSOR_TR("%s(%d): sensor_read failed", __func__, __LINE__);
		return IRQ_HANDLED;
	}
	SENSOR_DG("%s(%d): read 0xB5 val 0x%x\n", __func__, __LINE__, val);

	tp2825_check_cvstd(client);
	tp2825_send_uevent(&spsensor->common_sensor);

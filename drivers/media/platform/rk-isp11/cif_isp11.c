	...
	bool start_mi_y12)
{
	...
	if (start_mi_sp) {
		cif_isp11_stream_metadata_reset(&dev->sp_stream);
		dev->config.mi_config.sp.next_buff_addr =
			CIF_ISP11_INVALID_BUFF_ADDR;
		dev->config.mi_config.sp.curr_buff_addr =
			CIF_ISP11_INVALID_BUFF_ADDR;
	...
	if (start_mi_mp) {
		cif_isp11_stream_metadata_reset(&dev->mp_stream);
		dev->config.mi_config.mp.next_buff_addr =
			CIF_ISP11_INVALID_BUFF_ADDR;
		dev->config.mi_config.mp.curr_buff_addr =
			CIF_ISP11_INVALID_BUFF_ADDR;
	...
	if (start_mi_y12) {
		cif_isp11_stream_metadata_reset(&dev->y12_stream);
		dev->config.mi_config.y12.next_buff_addr =
			CIF_ISP11_INVALID_BUFF_ADDR;
		dev->config.mi_config.y12.curr_buff_addr =
			CIF_ISP11_INVALID_BUFF_ADDR;
...
static void cif_isp11_vs_work(struct work_struct *work)
{
	...
	case CIF_ISP11_VS_EXP: {
		...
		if (dev->img_src != NULL)
			cif_isp11_img_src_s_ext_ctrls(dev->img_src, exp_ctrl);
		else
...
static void cif_isp11_vcm_work(struct work_struct *work)
{
	struct cif_isp11_isp_vcm_work *vcm_wk =
		container_of(work, struct cif_isp11_isp_vcm_work, work);
	struct cif_isp11_device *dev = vcm_wk->dev;

	switch (vcm_wk->cmd) {
	case CIF_ISP11_VCM_VCM: {
		struct cif_isp11_img_src_vcm *vcm =
			(struct cif_isp11_img_src_vcm *)vcm_wk->param;

		if (dev->img_src != NULL)
			cif_isp11_img_src_ioctl(dev->img_src,
				PLTFRM_CIFCAM_SET_VCM_POS, &vcm->val);
		else
			cif_isp11_pltfrm_pr_err(dev->dev, "dev->img_src is NULL\n");
...
static int cif_isp11_vcm_work_cmd(
	struct cif_isp11_device *dev,
	...
	void *para)
{
	...
	vcm_wk = kmalloc(sizeof(struct cif_isp11_isp_vcm_work),
		GFP_ATOMIC);

	if (vcm_wk) {
		vcm_wk->cmd = cmd;
		vcm_wk->dev = dev;
		vcm_wk->param = (void *)para;
		INIT_WORK((struct work_struct *)&vcm_wk->work,
			cif_isp11_vcm_work);
		if (!queue_work(dev->vcm_wq,
			(struct work_struct *)&vcm_wk->work)) {
			cif_isp11_pltfrm_pr_err(dev->dev,
				"%s: queue work failed\n", __func__);
			kfree(vcm_wk);
		}
...
{
	struct cif_isp11_img_src_data *last_data;
	struct cif_isp11_img_src_data *new_data;
	...
	u32 stream_ids)
{
	...
	if (dev->sp_stream.state == CIF_ISP11_STATE_STREAMING) {
		if (streamoff_sp)
			streamoff_all |= CIF_ISP11_STREAM_SP;
	} else
		streamoff_all |= CIF_ISP11_STREAM_SP;

	if (dev->mp_stream.state == CIF_ISP11_STATE_STREAMING) {
		if (streamoff_mp)
			streamoff_all |= CIF_ISP11_STREAM_MP;
	} else
		streamoff_all |= CIF_ISP11_STREAM_MP;

	if (dev->y12_stream.state == CIF_ISP11_STATE_STREAMING) {
		if (streamoff_y12)
			streamoff_all |= CIF_ISP11_STREAM_Y12;
	} else
		streamoff_all |= CIF_ISP11_STREAM_Y12;
	...
	if (streamoff_all ==
		...
		if (!list_empty(&dev->img_src_vcms.list)) {
			vcm = list_first_entry(&dev->img_src_vcms.list,
	...
	struct pltfrm_soc_cfg *soc_cfg)
{
	...
	dev->vs_wq = alloc_workqueue("cif isp11 vs workqueue",
			WQ_UNBOUND | WQ_MEM_RECLAIM, 1);
	spin_lock_init(&dev->img_src_vcms.lock);
	INIT_LIST_HEAD(&dev->img_src_vcms.list);
	dev->vcm_wq = alloc_workqueue("cif isp11 vcm workqueue",
			WQ_UNBOUND | WQ_MEM_RECLAIM, 1);
	...
	struct vm_area_struct *vma)
{
	...
	if (mem_size > strm_dev->metadata.cnt*CAMERA_METADATA_LEN) {
		...
		cif_isp11_pltfrm_pr_err(dev->dev,
			...
			mem_size,
			(unsigned long)(strm_dev->metadata.cnt*CAMERA_METADATA_LEN));
		goto done;
	}
	...
	if (!mem_vaddr) {
		cif_isp11_pltfrm_pr_err(dev->dev,
			...
			pages,
			(stream_id == CIF_ISP11_STREAM_MP) ? "mp" : "sp");
		retval = -ENOMEM;
		goto done;
	}
	...
	int val)
{
	...
	case CIF_ISP11_CID_FOCUS_ABSOLUTE:
		return cif_isp11_s_vcm(dev, id, val);
	default:
		cif_isp11_pltfrm_pr_err(dev->dev,
...
int cif_isp11_isp_isr(unsigned int isp_mis, void *cntxt)
{
	...
	if (isp_mis & CIF_ISP_V_START) {
		...
		spin_lock(&dev->img_src_vcms.lock);
		if (!list_empty(&dev->img_src_vcms.list)) {
			vcm = list_first_entry(&dev->img_src_vcms.list,
				struct cif_isp11_img_src_vcm,
				list);
		...
		spin_unlock(&dev->img_src_vcms.lock);

		if (vcm)
			cif_isp11_vcm_work_cmd(dev, CIF_ISP11_VCM_VCM, (void *)vcm);
	}

...
static struct cif_isp11_fmt cif_isp11_output_format[] = {
/* ************* YUV422 ************* */
{
...
static struct cif_isp11_hw_error cif_isp11_hw_errors[] = {
	{
	 .name = "isp_data_loss",
	 .count = 0,
...
#define CIF_ISP11_INVALID_BUFF_ADDR ((u32)~0)
#define CIF_ISP11_MI_IS_BUSY(dev)\
	(dev->config.mi_config.mp.busy ||\
	dev->config.mi_config.sp.busy ||\
	dev->config.mi_config.dma.busy ||\
	dev->config.mi_config.y12.busy)
enum {
	CIF_ISP11_ASYNC_JPEG = 0x1,
	CIF_ISP11_ASYNC_YCFLT = 0x2,
	CIF_ISP11_ASYNC_ISM = 0x4,
	CIF_ISP11_ASYNC_DMA = 0x8
};
#define CIF_ISP11_ALWAYS_ASYNC 0x00
#define CIF_ISP11_ALWAYS_STALL_ON_NO_BUFS (false)
...
/**Static Functions***********************************************************/
static const char *cif_isp11_interface_string(
...
static const char *cif_isp11_img_src_state_string(
	enum cif_isp11_img_src_state state)
{
	switch (state) {
	case CIF_ISP11_IMG_SRC_STATE_OFF:
		return "OFF";
	case CIF_ISP11_IMG_SRC_STATE_SW_STNDBY:
		return "SW_STNDBY";
	case CIF_ISP11_IMG_SRC_STATE_STREAMING:
		return "STREAMING";
	default:
		return "UNKNOWN/UNSUPPORTED";
	}
...
static const char *cif_isp11_state_string(
	enum cif_isp11_state state)
{
	switch (state) {
	case CIF_ISP11_STATE_DISABLED:
		return "DISABLED";
	case CIF_ISP11_STATE_INACTIVE:
		return "INACTIVE";
	case CIF_ISP11_STATE_READY:
		return "READY";
	case CIF_ISP11_STATE_STREAMING:
		return "STREAMING";
	default:
		return "UNKNOWN/UNSUPPORTED";
	}
...
static const char *cif_isp11_pm_state_string(
	enum cif_isp11_pm_state pm_state)
{
	switch (pm_state) {
	case CIF_ISP11_PM_STATE_OFF:
		return "OFF";
	case CIF_ISP11_PM_STATE_SW_STNDBY:
		return "STANDBY";
	case CIF_ISP11_PM_STATE_SUSPENDED:
		return "SUSPENDED";
	case CIF_ISP11_PM_STATE_STREAMING:
		return "STREAMING";
	default:
		return "UNKNOWN/UNSUPPORTED";
	}
...
static const char *cif_isp11_stream_id_string(
...
static const char *cif_isp11_inp_string(
	enum cif_isp11_inp inp)
{
	switch (inp) {
	case CIF_ISP11_INP_CSI:
		return "CSI";
	case CIF_ISP11_INP_CPI:
		return "CPI";
	case CIF_ISP11_INP_DMA:
		return "DMA(ISP)";
	case CIF_ISP11_INP_DMA_IE:
		return "DMA(Image Effects)";
	case CIF_ISP11_INP_DMA_SP:
		return "DMA(SP)";
	case CIF_ISP11_INP_DMA_Y12:
		return "DMA(Y12)";
	default:
		return "UNKNOWN/UNSUPPORTED";
	}
...
enum cif_isp11_inp cif_isp11_input_index2inp(
	...
	unsigned int input)
{
	...
	if (input >= dev->img_src_cnt) {
		return input - dev->img_src_cnt + CIF_ISP11_INP_DMA;
	} else {
		cif_isp11_pltfrm_g_interface_config(
			dev->img_src_array[input],
			&itf);
		if (PLTFRM_CAM_ITF_IS_MIPI(itf.type))
			return CIF_ISP11_INP_CSI;
		if (PLTFRM_CAM_ITF_IS_DVP(itf.type))
			return CIF_ISP11_INP_CPI;
	}

	return -EINVAL;
}
static const char *cif_isp11_pix_fmt_string(int pixfmt)
{
	switch (pixfmt) {
	case CIF_YUV400:
		return "YUV400";
...
static void cif_isp11_debug_print_mi_sp(struct cif_isp11_device *dev)
{
	cif_isp11_pltfrm_pr_info(dev->dev,
...
static void cif_isp11_debug_print_mi_mp(struct cif_isp11_device *dev)
{
	cif_isp11_pltfrm_pr_info(dev->dev,
		"\n  MI_CTRL 0x%08x/0x%08x\n"
		"  MI_STATUS 0x%08x\n"
...
static void cif_isp11_debug_print_mi_y12(struct cif_isp11_device *dev)
{
	cif_isp11_pltfrm_pr_info(dev->dev,
		"\n  MI_CTRL 0x%08x/0x%08x\n"
		"  MI_STATUS 0x%08x\n"
...
static void cif_isp11_debug_print_srsz(struct cif_isp11_device *dev)
{
	cif_isp11_pltfrm_pr_info(dev->dev,
...
static void cif_isp11_debug_print_mrsz(struct cif_isp11_device *dev)
{
	cif_isp11_pltfrm_pr_info(dev->dev,
...
static void cif_isp11_debug_print_block(
	struct cif_isp11_device *dev,
	const char *block_name)
{
	if (!strncmp(block_name, "all", 3)) {
		cif_isp11_debug_print_srsz(dev);
		cif_isp11_debug_print_mrsz(dev);
		cif_isp11_debug_print_mi_sp(dev);
		cif_isp11_debug_print_mi_mp(dev);
		cif_isp11_debug_print_mi_y12(dev);
...
static u32 cif_isp11_calc_llength(
	u32 width,
	u32 stride,
	enum cif_isp11_pix_fmt pix_fmt)
{
	if (stride == 0)
		return width;

	if (CIF_ISP11_PIX_FMT_IS_YUV(pix_fmt)) {
		u32 num_cplanes =
			CIF_ISP11_PIX_FMT_YUV_GET_NUM_CPLANES(pix_fmt);
		if (num_cplanes == 0)
			return 8 * stride / CIF_ISP11_PIX_FMT_GET_BPP(pix_fmt);
		else
			return stride;
	} else if (CIF_ISP11_PIX_FMT_IS_RGB(pix_fmt))
		return 8 * stride / CIF_ISP11_PIX_FMT_GET_BPP(pix_fmt);
	else
		return width;
}

static int cif_isp11_set_pm_state(
	struct cif_isp11_device *dev,
	enum cif_isp11_pm_state pm_state)
{
	cif_isp11_pltfrm_pr_dbg(dev->dev, "%s -> %s\n",
		cif_isp11_pm_state_string(dev->pm_state),
		cif_isp11_pm_state_string(pm_state));
	...
	case CIF_ISP11_PM_STATE_OFF:
	case CIF_ISP11_PM_STATE_SUSPENDED:
		if ((dev->pm_state == CIF_ISP11_PM_STATE_SW_STNDBY) ||
			(dev->pm_state == CIF_ISP11_PM_STATE_STREAMING)) {
			pm_runtime_put_sync(dev->dev);
		}
		break;
	case CIF_ISP11_PM_STATE_SW_STNDBY:
	case CIF_ISP11_PM_STATE_STREAMING:
		if ((dev->pm_state == CIF_ISP11_PM_STATE_OFF) ||
			(dev->pm_state == CIF_ISP11_PM_STATE_SUSPENDED)) {
...
static int cif_isp11_img_src_set_state(
	struct cif_isp11_device *dev,
	enum cif_isp11_img_src_state state)
{
	int ret = 0;
	...
	cif_isp11_pltfrm_pr_dbg(dev->dev, "%s -> %s\n",
		cif_isp11_img_src_state_string(dev->img_src_state),
		cif_isp11_img_src_state_string(state));
	...
	switch (state) {
	case CIF_ISP11_IMG_SRC_STATE_OFF:
		ret = cif_isp11_img_src_s_power(dev->img_src, false);
		break;
	case CIF_ISP11_IMG_SRC_STATE_SW_STNDBY:
		if (dev->img_src_state == CIF_ISP11_IMG_SRC_STATE_STREAMING) {
			ret = cif_isp11_img_src_s_streaming(
				dev->img_src, false);
		} else
			ret = cif_isp11_img_src_s_power(dev->img_src, true);
		break;
	case CIF_ISP11_IMG_SRC_STATE_STREAMING:
		if (dev->config.flash_mode !=
			CIF_ISP11_FLASH_MODE_OFF)
			cif_isp11_img_src_s_ctrl(dev->img_src,
				CIF_ISP11_CID_FLASH_MODE,
				dev->config.flash_mode);
		ret = cif_isp11_img_src_s_streaming(dev->img_src, true);
		break;
	default:
		break;
	}

	if ((dev->config.flash_mode != CIF_ISP11_FLASH_MODE_OFF) &&
		(IS_ERR_VALUE(ret) ||
		(state == CIF_ISP11_IMG_SRC_STATE_OFF)))
		cif_isp11_img_src_s_ctrl(dev->img_src,
			CIF_ISP11_CID_FLASH_MODE,
			CIF_ISP11_FLASH_MODE_OFF);
...
static int cif_isp11_img_srcs_init(
	struct cif_isp11_device *dev)
{
	int ret = 0;

	memset(dev->img_src_array, 0x00, sizeof(dev->img_src_array));
	dev->img_src_cnt = cif_isp11_pltfrm_get_img_src_device(dev->dev,
		dev->img_src_array, CIF_ISP11_NUM_INPUTS);
...
static int cif_isp11_img_src_select_strm_fmt(
	struct cif_isp11_device *dev)
{
	int ret = 0;
	u32 index;
	struct cif_isp11_strm_fmt_desc strm_fmt_desc;
	struct cif_isp11_strm_fmt request_strm_fmt;
	...
	ret = cif_isp11_get_target_frm_size(dev,
	...
	cif_isp11_pltfrm_pr_dbg(
		...
		request_strm_fmt.frm_intrvl.denominator,
		request_strm_fmt.frm_intrvl.numerator,
		cif_isp11_img_src_g_name(dev->img_src));

	ret = cif_isp11_img_src_s_strm_fmt(dev->img_src, &request_strm_fmt);
	if (IS_ERR_VALUE(ret))
		goto err;

	dev->config.img_src_output = request_strm_fmt;

	ret = cif_isp11_img_src_g_ctrl(dev->img_src,
		CIF_ISP11_CID_VBLANKING, &vblanking);
	if (IS_ERR_VALUE(ret)) {
		cif_isp11_pltfrm_pr_dbg(
			dev->dev,
...
static void cif_isp11_config_ism(struct cif_isp11_device *dev, bool async)
{
	const struct cif_isp11_ism_config *pconfig =
		&(dev->config.isp_config.ism_config);

	if (pconfig->ism_en) {
		cif_isp11_pltfrm_pr_dbg(dev->dev, "%dx%d -> %dx%d@(%d,%d)\n",
...
static void cif_isp11_program_jpeg_tables(
	struct cif_isp11_device *dev)
{
	...
	cif_isp11_pltfrm_pr_dbg(NULL, "ratio %d\n", ratio);
...
static void cif_isp11_select_jpeg_tables(
	struct cif_isp11_device *dev)
{
...
static int cif_isp11_config_img_src(
	struct cif_isp11_device *dev)
{
	int ret = 0;
	...
	ret = cif_isp11_img_src_set_state(dev,
	...
	if (!dev->sp_stream.updt_cfg &&
		...
		!dev->y12_stream.updt_cfg)
		return 0;

	ret = cif_isp11_pltfrm_g_interface_config(dev->img_src,
			&dev->config.cam_itf);
	if (IS_ERR_VALUE(ret))
		goto err;
...
static int cif_isp11_config_isp(
	struct cif_isp11_device *dev)
{
	int ret = 0;
	u32 input_width;
	...
	if (CIF_ISP11_PIX_FMT_IS_RAW_BAYER(in_pix_fmt)) {
		if (!dev->config.mi_config.raw_enable) {
			...
			if ((dev->mp_stream.state == CIF_ISP11_STATE_READY) &&
				(dev->sp_stream.state == CIF_ISP11_STATE_READY) &&
				(dev->y12_stream.state == CIF_ISP11_STATE_READY)) {

				if (dev->config.mi_config.mp.output.quantization !=
					dev->config.mi_config.sp.output.quantization) {
					cif_isp11_pltfrm_pr_err(dev->dev,
						"colorspace quantization"
						"(mp: %d, sp: %d) is not support!\n",
			...
			if (PLTFRM_CAM_ITF_IS_BT656(dev->config.cam_itf.type)) {
				cif_iowrite32(CIF_ISP_CTRL_ISP_MODE_RAW_PICT_ITU656,
					dev->config.base_addr + CIF_ISP_CTRL);
			} else {
		...
		output->pix_fmt = in_pix_fmt;
		acq_mult = 2;
		if (dev->config.input_sel == CIF_ISP11_INP_DMA) {
			bpp = CIF_ISP11_PIX_FMT_GET_BPP(in_pix_fmt);
			bpp =
				bpp * 4
				/
				(4 + (CIF_ISP11_PIX_FMT_YUV_GET_X_SUBS(
					in_pix_fmt) *
				CIF_ISP11_PIX_FMT_YUV_GET_Y_SUBS(
					in_pix_fmt) / 2));
			if (bpp == 8)
				isp_input_sel = CIF_ISP_ACQ_PROP_IN_SEL_8B_MSB;
			else if (bpp == 10)
				...
				cif_isp11_pltfrm_pr_err(dev->dev,
					...
					cif_isp11_pix_fmt_string(in_pix_fmt),
					bpp);
				ret = -EINVAL;
				goto err;
			}
			...
			if (PLTFRM_CAM_ITF_IS_MIPI(dev->config.cam_itf.type)) {
				...
				if (PLTFRM_CAM_ITF_IS_BT656(dev->config.cam_itf.type)) {
					cif_iowrite32(CIF_ISP_CTRL_ISP_MODE_ITU656,
						      dev->config.base_addr + CIF_ISP_CTRL);
				} else {
					cif_iowrite32(CIF_ISP_CTRL_ISP_MODE_ITU601,
						      dev->config.base_addr + CIF_ISP_CTRL);
				...
				default:
					cif_isp11_pltfrm_pr_err(dev->dev,
						"%s isn't support for cif isp11\n",
						cif_isp11_interface_string(dev->config.cam_itf.type));
					break;
				}
			} else {
				cif_isp11_pltfrm_pr_err(dev->dev,
		...
		if (CIF_ISP11_PIX_FMT_YUV_IS_YC_SWAPPED(in_pix_fmt)) {
			yuv_seq = CIF_ISP_ACQ_PROP_CBYCRY;
			cif_isp11_pix_fmt_set_yc_swapped(output->pix_fmt, 0);
		} else if (CIF_ISP11_PIX_FMT_YUV_IS_UV_SWAPPED(in_pix_fmt))
	...
	if (PLTFRM_CAM_ITF_IS_DVP(cam_itf->type)) {
		...
		if (PLTFRM_CAM_ITF_IS_BT601(cam_itf->type)) {
			signal |=
				...
				CIF_ISP_ACQ_PROP_VSYNC_HIGH : CIF_ISP_ACQ_PROP_VSYNC_LOW;

			signal |=
				(cam_itf->cfg.dvp.hsync == PLTFRM_CAM_SIGNAL_HIGH_LEVEL) ?
				CIF_ISP_ACQ_PROP_HSYNC_HIGH : CIF_ISP_ACQ_PROP_HSYNC_LOW;
...
static int cif_isp11_config_mipi(
	struct cif_isp11_device *dev)
{
	int ret = 0;
	u32 data_type;
	...
	if (!CIF_ISP11_INP_IS_MIPI(dev->config.input_sel)) {
	...
	cif_isp11_pltfrm_pr_dbg(dev->dev,
		...
		cif_isp11_inp_string(dev->config.input_sel),
		dev->config.cam_itf.cfg.mipi.vc,
		dev->config.cam_itf.cfg.mipi.nb_lanes);
	...
	/* Shutdown lanes bits must be 0x0f */
	shutdown_lanes = 0xf;
	...
	if (CIF_ISP11_PIX_FMT_IS_YUV(in_pix_fmt)) {
		if ((CIF_ISP11_PIX_FMT_YUV_GET_X_SUBS(in_pix_fmt) == 2) &&
			(CIF_ISP11_PIX_FMT_YUV_GET_Y_SUBS(in_pix_fmt) == 2) &&
			(CIF_ISP11_PIX_FMT_GET_BPP(in_pix_fmt) == 12))
			data_type = CSI2_DT_YUV420_8b;
		else if ((CIF_ISP11_PIX_FMT_YUV_GET_X_SUBS(in_pix_fmt) == 2) &&
			(CIF_ISP11_PIX_FMT_YUV_GET_Y_SUBS(in_pix_fmt) == 2) &&
			(CIF_ISP11_PIX_FMT_GET_BPP(in_pix_fmt) == 15))
			data_type = CSI2_DT_YUV420_10b;
		else if ((CIF_ISP11_PIX_FMT_YUV_GET_X_SUBS(in_pix_fmt) == 2) &&
			(CIF_ISP11_PIX_FMT_YUV_GET_Y_SUBS(in_pix_fmt) == 4) &&
			(CIF_ISP11_PIX_FMT_GET_BPP(in_pix_fmt) == 16))
	...
	} else if (CIF_ISP11_PIX_FMT_IS_RAW_BAYER(in_pix_fmt)) {
		if (CIF_ISP11_PIX_FMT_GET_BPP(in_pix_fmt) == 8)
			data_type = CSI2_DT_RAW8;
		else if (CIF_ISP11_PIX_FMT_GET_BPP(in_pix_fmt) == 10)
			data_type = CSI2_DT_RAW10;
		else if (CIF_ISP11_PIX_FMT_GET_BPP(in_pix_fmt) == 12)
...
static int cif_isp11_config_mi_mp(
	struct cif_isp11_device *dev)
{
	enum cif_isp11_pix_fmt out_pix_fmt =
	...
	u32 writeformat = CIF_ISP11_BUFF_FMT_PLANAR;
	u32 swap_cb_cr = 0;
	u32 bpp = CIF_ISP11_PIX_FMT_GET_BPP(out_pix_fmt);
	...
	cif_isp11_pltfrm_pr_dbg(dev->dev,
		"%s %dx%d, llength = %d\n",
		cif_isp11_pix_fmt_string(out_pix_fmt),
		width,
		height,
		llength);
	...
	if (CIF_ISP11_PIX_FMT_IS_YUV(out_pix_fmt)) {
		u32 num_cplanes =
		...
		if (num_cplanes == 0) {
			...
			dev->config.mi_config.mp.cb_size =
				...
				dev->config.mi_config.mp.y_size;
			if (num_cplanes == 1)
				writeformat = CIF_ISP11_BUFF_FMT_SEMIPLANAR;
		...
		if (CIF_ISP11_PIX_FMT_YUV_IS_UV_SWAPPED(out_pix_fmt))
			swap_cb_cr = CIF_MI_XTD_FMT_CTRL_MP_CB_CR_SWAP;

		if (writeformat == CIF_ISP11_BUFF_FMT_SEMIPLANAR) {
			dev->config.mi_config.mp.cb_offs =
			    dev->config.mi_config.mp.y_size;
			dev->config.mi_config.mp.cr_offs =
			    dev->config.mi_config.mp.cb_offs;
		} else if (writeformat == CIF_ISP11_BUFF_FMT_PLANAR) {
			if (swap_cb_cr) {
				swap_cb_cr = 0;
				dev->config.mi_config.mp.cr_offs =
					dev->config.mi_config.mp.y_size;
	...
	} else if (CIF_ISP11_PIX_FMT_IS_RAW_BAYER(out_pix_fmt)) {
		if (CIF_ISP11_PIX_FMT_GET_BPP(out_pix_fmt) > 8) {
			writeformat = CIF_ISP11_BUFF_FMT_RAW12;
			dev->config.mi_config.mp.y_size = width * height*2;
		} else {
			writeformat = CIF_ISP11_BUFF_FMT_RAW8;
	...
	cif_iowrite32_verify(dev->config.mi_config.mp.y_size,
		dev->config.base_addr + CIF_MI_MP_Y_SIZE_INIT, CIF_MI_ADDR_SIZE_ALIGN_MASK);
	cif_iowrite32_verify(dev->config.mi_config.mp.cb_size,
		dev->config.base_addr + CIF_MI_MP_CB_SIZE_INIT, CIF_MI_ADDR_SIZE_ALIGN_MASK);
	cif_iowrite32_verify(dev->config.mi_config.mp.cr_size,
		dev->config.base_addr + CIF_MI_MP_CR_SIZE_INIT, CIF_MI_ADDR_SIZE_ALIGN_MASK);
...
static int cif_isp11_config_mi_y12(
	struct cif_isp11_device *dev)
{
	enum cif_isp11_pix_fmt out_pix_fmt =
		dev->config.mi_config.y12.output.pix_fmt;
	u32 llength =
		dev->config.mi_config.y12.llength;
	u32 width =
		dev->config.mi_config.y12.output.width;
	u32 height =
		dev->config.mi_config.y12.output.height;
	u32 swap_cb_cr = 0;
	u32 bpp = CIF_ISP11_PIX_FMT_GET_BPP(out_pix_fmt);
	u32 size = llength * height * bpp / 8;
	u32 mi_ctrl;

	dev->config.mi_config.y12.input =
		&dev->config.isp_config.output;
	...
	dev->config.mi_config.y12.y_size = size;
	dev->config.mi_config.y12.cb_size = 0;
	dev->config.mi_config.y12.cr_size = 0;
	if (CIF_ISP11_PIX_FMT_IS_YUV(out_pix_fmt)) {
		u32 num_cplanes =
			CIF_ISP11_PIX_FMT_YUV_GET_NUM_CPLANES(out_pix_fmt);
		if (num_cplanes <= 1) {
			dev->config.mi_config.y12.y_size =
				(dev->config.mi_config.y12.y_size * 4)
				/
				(4 + (CIF_ISP11_PIX_FMT_YUV_GET_X_SUBS(
					out_pix_fmt) *
				CIF_ISP11_PIX_FMT_YUV_GET_Y_SUBS(
					out_pix_fmt) / 2));
			dev->config.mi_config.y12.cb_size =
				size -
				dev->config.mi_config.y12.y_size;
			dev->config.mi_config.y12.cb_offs =
			    dev->config.mi_config.y12.y_size;
		} else {
			cif_isp11_pltfrm_pr_err(dev->dev,
			"wrong num_cplanes %s for y12 path\n",
			cif_isp11_pix_fmt_string(out_pix_fmt));
			return -EINVAL;
		}

		if (CIF_ISP11_PIX_FMT_YUV_IS_UV_SWAPPED(out_pix_fmt))
			swap_cb_cr = CIF_MI_XTD_FMT_CTRL_Y12_CB_CR_SWAP;
	} else if (CIF_ISP11_PIX_FMT_IS_RAW_BAYER(out_pix_fmt)) {
		cif_isp11_pltfrm_pr_err(dev->dev,
		"wrong format %s for y12 path\n",
		cif_isp11_pix_fmt_string(out_pix_fmt));
		return -EINVAL;
	}
	if (out_pix_fmt == CIF_Y12) {
		cif_iowrite32_verify(dev->config.mi_config.y12.y_size,
			dev->config.base_addr + CIF_MI_MP_CR_SIZE_INIT, CIF_MI_ADDR_SIZE_ALIGN_MASK);
		cif_iowrite32OR_verify(CIF_MI_Y12_Y_FRAME|CIF_MI_ALL_FRAME, dev->config.base_addr + CIF_MI_IMSC, ~0);
		cif_iowrite32OR_verify(CIF_MI_INIT_Y12_Y_EN,
		...
		cif_iowrite32_verify(dev->config.mi_config.y12.y_size,
			dev->config.base_addr + CIF_MI_MP_CR_SIZE_INIT, CIF_MI_ADDR_SIZE_ALIGN_MASK);
		cif_iowrite32_verify(dev->config.mi_config.y12.cb_size,
			dev->config.base_addr + CIF_MI_SP_CR_SIZE_INIT, CIF_MI_ADDR_SIZE_ALIGN_MASK);
		cif_iowrite32OR_verify(CIF_MI_Y12_Y_FRAME|CIF_MI_Y12_UV_FRAME|CIF_MI_ALL_FRAME,
			dev->config.base_addr +
			CIF_MI_IMSC, ~0);
		cif_iowrite32OR_verify((CIF_MI_INIT_Y12_Y_EN|CIF_MI_INIT_Y12_UV_EN),
			dev->config.base_addr + CIF_MI_INIT, ~0);
		cif_isp11_pltfrm_pr_dbg(NULL, "CIF_MI_INIT 0x%x\n",
			cif_ioread32(dev->config.base_addr + CIF_MI_INIT));
	}
...
static int cif_isp11_config_mi_sp(
	struct cif_isp11_device *dev)
{
	int ret = 0;
	enum cif_isp11_pix_fmt out_pix_fmt =
	...
	if (CIF_ISP11_PIX_FMT_IS_YUV(out_pix_fmt)) {
		...
		if ((CIF_ISP11_PIX_FMT_YUV_GET_X_SUBS(out_pix_fmt) == 0) &&
			(CIF_ISP11_PIX_FMT_YUV_GET_Y_SUBS(out_pix_fmt) == 0))
			output_format = CIF_MI_CTRL_SP_OUTPUT_FMT_YUV400;
		else if ((CIF_ISP11_PIX_FMT_YUV_GET_X_SUBS(out_pix_fmt) == 2) &&
			(CIF_ISP11_PIX_FMT_YUV_GET_Y_SUBS(out_pix_fmt) == 2))
			output_format = CIF_MI_CTRL_SP_OUTPUT_FMT_YUV420;
		else if ((CIF_ISP11_PIX_FMT_YUV_GET_X_SUBS(out_pix_fmt) == 2) &&
			(CIF_ISP11_PIX_FMT_YUV_GET_Y_SUBS(out_pix_fmt) == 4))
			output_format = CIF_MI_CTRL_SP_OUTPUT_FMT_YUV422;
		else if ((CIF_ISP11_PIX_FMT_YUV_GET_X_SUBS(out_pix_fmt) == 4) &&
			(CIF_ISP11_PIX_FMT_YUV_GET_Y_SUBS(out_pix_fmt) == 4))
	...
	} else if (CIF_ISP11_PIX_FMT_IS_RGB(out_pix_fmt)) {
	...
	if ((CIF_ISP11_PIX_FMT_YUV_GET_X_SUBS(in_pix_fmt) == 0) &&
		(CIF_ISP11_PIX_FMT_YUV_GET_Y_SUBS(in_pix_fmt) == 0))
	...
	else if ((CIF_ISP11_PIX_FMT_YUV_GET_X_SUBS(in_pix_fmt) == 4) &&
		(CIF_ISP11_PIX_FMT_YUV_GET_Y_SUBS(in_pix_fmt) == 4))
	...
	cif_iowrite32_verify(dev->config.mi_config.sp.y_size,
		dev->config.base_addr + CIF_MI_SP_Y_SIZE_INIT, CIF_MI_ADDR_SIZE_ALIGN_MASK);
	cif_iowrite32_verify(dev->config.mi_config.sp.y_size,
		dev->config.base_addr + CIF_MI_SP_Y_PIC_SIZE, CIF_MI_ADDR_SIZE_ALIGN_MASK);
	cif_iowrite32_verify(dev->config.mi_config.sp.cb_size,
		dev->config.base_addr + CIF_MI_SP_CB_SIZE_INIT, CIF_MI_ADDR_SIZE_ALIGN_MASK);
...
static int cif_isp11_config_mi_dma(
	struct cif_isp11_device *dev)
{
	int ret = 0;
	enum cif_isp11_pix_fmt out_pix_fmt =
	...
	u32 readformat = CIF_ISP11_BUFF_FMT_PLANAR;
	u32 bpp = CIF_ISP11_PIX_FMT_GET_BPP(out_pix_fmt);
	u32 size = llength * height * bpp / 8;
	u32 output_format;
	u32 mi_ctrl;
	...
	if (CIF_ISP11_PIX_FMT_IS_YUV(out_pix_fmt)) {
		u32 num_cplanes =
			CIF_ISP11_PIX_FMT_YUV_GET_NUM_CPLANES(out_pix_fmt);
		if (num_cplanes == 0) {
			readformat = CIF_ISP11_BUFF_FMT_INTERLEAVED;
		} else {
			...
			dev->config.mi_config.dma.cb_size =
				...
				dev->config.mi_config.dma.y_size;
			if (num_cplanes == 1)
				readformat = CIF_ISP11_BUFF_FMT_SEMIPLANAR;
			else if (num_cplanes == 2) {
				readformat = CIF_ISP11_BUFF_FMT_PLANAR;
	...
	if (readformat == CIF_ISP11_BUFF_FMT_SEMIPLANAR) {
		dev->config.mi_config.dma.cb_offs =
		    dev->config.mi_config.dma.y_size;
		dev->config.mi_config.dma.cr_offs =
		    dev->config.mi_config.dma.cb_offs;
	} else if (readformat == CIF_ISP11_BUFF_FMT_PLANAR) {
		dev->config.mi_config.dma.cb_offs =
			dev->config.mi_config.dma.y_size;
		dev->config.mi_config.dma.cr_offs =
			dev->config.mi_config.dma.cb_offs +
...
static int cif_isp11_config_jpeg_enc(
	struct cif_isp11_device *dev)
{
	struct cif_isp11_frm_fmt *inp_fmt =
		&dev->config.mp_config.rsz_config.output;
	dev->config.jpeg_config.input = inp_fmt;

	cif_isp11_pltfrm_pr_dbg(NULL,
		"%s %dx%d\n",
		cif_isp11_pix_fmt_string(inp_fmt->pix_fmt),
	...
	if (CIF_ISP11_INP_IS_DMA(dev->config.input_sel) ||
		!CIF_ISP11_PIX_FMT_IS_RAW_BAYER(
	...
	/* Program JPEG tables */
	cif_isp11_program_jpeg_tables(dev);
	/* Select JPEG tables */
	cif_isp11_select_jpeg_tables(dev);

	switch (dev->config.jpeg_config.header) {
	case CIF_ISP11_JPEG_HEADER_JFIF:
		cif_isp11_pltfrm_pr_dbg(NULL,
		...
		cif_iowrite32(CIF_JPE_HEADER_MODE_JFIF,
			...
			CIF_JPE_HEADER_MODE);
		break;
	case CIF_ISP11_JPEG_HEADER_NONE:
		cif_isp11_pltfrm_pr_dbg(NULL,
...
static int cif_isp11_config_path(
	struct cif_isp11_device *dev,
	u32 stream_ids)
{
	u32 dpcl = 0;
	...
	else if (dev->config.input_sel == CIF_ISP11_INP_DMA_IE)
	...
	if ((stream_ids & CIF_ISP11_STREAM_MP) &&
		!(dev->config.input_sel == CIF_ISP11_INP_DMA_SP)) {
	...
	cif_isp11_pltfrm_pr_dbg(dev->dev, "CIF_DPCL 0x%08x\n", dpcl);

	return 0;
}

int cif_isp11_config_dcrop(
	struct cif_isp11_device *dev,
	enum cif_isp11_stream_id stream_id,
	bool async)
{
	...
	if (stream_id == CIF_ISP11_STREAM_MP) {
		cif_iowrite32(0, dev->config.base_addr + CIF_DUAL_CROP_M_H_OFFS);
		cif_iowrite32(0, dev->config.base_addr + CIF_DUAL_CROP_M_V_OFFS);
		cif_iowrite32(0, dev->config.base_addr + CIF_DUAL_CROP_M_H_SIZE);
		cif_iowrite32(0, dev->config.base_addr + CIF_DUAL_CROP_M_V_SIZE);
		...
		cif_iowrite32(0, dev->config.base_addr + CIF_DUAL_CROP_S_H_OFFS);
		cif_iowrite32(0, dev->config.base_addr + CIF_DUAL_CROP_S_V_OFFS);
		cif_iowrite32(0, dev->config.base_addr + CIF_DUAL_CROP_S_H_SIZE);
		cif_iowrite32(0, dev->config.base_addr + CIF_DUAL_CROP_S_V_SIZE);
	...
	bool async)
{
	int ret;
	u32 i;
	CIF_ISP11_PLTFRM_MEM_IO_ADDR scale_h_y_addr =
		dev->config.base_addr;
	CIF_ISP11_PLTFRM_MEM_IO_ADDR scale_h_cr_addr =
		dev->config.base_addr;
	CIF_ISP11_PLTFRM_MEM_IO_ADDR scale_h_cb_addr =
		dev->config.base_addr;
	CIF_ISP11_PLTFRM_MEM_IO_ADDR scale_v_y_addr =
		dev->config.base_addr;
	CIF_ISP11_PLTFRM_MEM_IO_ADDR scale_v_c_addr =
		dev->config.base_addr;
	CIF_ISP11_PLTFRM_MEM_IO_ADDR rsz_ctrl_addr =
		dev->config.base_addr;
	struct cif_isp11_frm_fmt *rsz_input;
	struct cif_isp11_frm_fmt *rsz_output;
	struct cif_isp11_frm_fmt *mi_output;
	u32 rsz_ctrl;
	u32 input_width_y;
	u32 output_width_y;
	u32 input_height_y;
	...
	if (stream_id == CIF_ISP11_STREAM_MP) {
		...
		if (dev->config.input_sel == CIF_ISP11_INP_DMA_SP)
	...
	if (CIF_ISP11_PIX_FMT_IS_YUV(mi_output->pix_fmt)) {
		cif_isp11_pix_fmt_set_y_subs(
			rsz_output->pix_fmt,
			CIF_ISP11_PIX_FMT_YUV_GET_Y_SUBS(mi_output->pix_fmt));
		cif_isp11_pix_fmt_set_x_subs(
			rsz_output->pix_fmt,
			CIF_ISP11_PIX_FMT_YUV_GET_X_SUBS(mi_output->pix_fmt));
		cif_isp11_pix_fmt_set_bpp(
			rsz_output->pix_fmt,
			CIF_ISP11_PIX_FMT_GET_BPP(mi_output->pix_fmt));
	} else if (CIF_ISP11_PIX_FMT_IS_JPEG(mi_output->pix_fmt)) {
		cif_isp11_pix_fmt_set_y_subs(
	...
	cif_isp11_pltfrm_pr_dbg(dev->dev,
		...
		cif_isp11_pix_fmt_string(rsz_input->pix_fmt),
		rsz_input->width,
		rsz_input->height,
		cif_isp11_pix_fmt_string(rsz_output->pix_fmt),
		rsz_output->width,
		rsz_output->height);
	...
	if (CIF_ISP11_PIX_FMT_IS_YUV(rsz_output->pix_fmt)) {
		input_width_c = (input_width_c *
			CIF_ISP11_PIX_FMT_YUV_GET_X_SUBS(
				rsz_input->pix_fmt)) / 4;
	...
	if (input_width_y < output_width_y) {
		...
		cif_iowrite32(
			DIV_TRUNCATE((input_width_y - 1) * CIF_RSZ_SCALER_BYPASS,
				output_width_y - 1),
			scale_h_y_addr);
	} else if (input_width_y > output_width_y) {
		rsz_ctrl |= CIF_RSZ_CTRL_SCALE_HY_ENABLE;
		cif_iowrite32(
			DIV_TRUNCATE((output_width_y - 1) * CIF_RSZ_SCALER_BYPASS,
				input_width_y - 1) + 1,
			scale_h_y_addr);
	}
	if (input_width_c < output_width_c) {
		...
		scale_h_c = DIV_TRUNCATE((input_width_c - 1) * CIF_RSZ_SCALER_BYPASS,
			output_width_c - 1);
		cif_iowrite32(scale_h_c, scale_h_cb_addr);
		cif_iowrite32(scale_h_c, scale_h_cr_addr);
	} else if (input_width_c > output_width_c) {
		rsz_ctrl |= CIF_RSZ_CTRL_SCALE_HC_ENABLE;
		scale_h_c = DIV_TRUNCATE((output_width_c - 1) * CIF_RSZ_SCALER_BYPASS,
			input_width_c - 1) + 1;
		cif_iowrite32(scale_h_c, scale_h_cb_addr);
		cif_iowrite32(scale_h_c, scale_h_cr_addr);
	}
	...
	if (input_height_y < output_height_y) {
		...
		cif_iowrite32(
			DIV_TRUNCATE((input_height_y - 1) * CIF_RSZ_SCALER_BYPASS,
				output_height_y - 1),
			scale_v_y_addr);
	} else if (input_height_y > output_height_y) {
		rsz_ctrl |= CIF_RSZ_CTRL_SCALE_VY_ENABLE;
		cif_iowrite32(
			DIV_TRUNCATE((output_height_y - 1) * CIF_RSZ_SCALER_BYPASS,
				input_height_y - 1) + 1,
			scale_v_y_addr);
	}
	...
	if (input_height_c < output_height_c) {
		...
		cif_iowrite32(
			DIV_TRUNCATE((input_height_c - 1) * CIF_RSZ_SCALER_BYPASS,
				output_height_c - 1),
			scale_v_c_addr);
	} else if (input_height_c > output_height_c) {
		rsz_ctrl |= CIF_RSZ_CTRL_SCALE_VC_ENABLE;
		cif_iowrite32(
			DIV_TRUNCATE((output_height_c - 1) * CIF_RSZ_SCALER_BYPASS,
				input_height_c - 1) + 1,
			scale_v_c_addr);
	}
...
static int cif_isp11_config_sp(
	struct cif_isp11_device *dev)
{
	int ret = 0;
	...
	ret = cif_isp11_config_rsz(dev, CIF_ISP11_STREAM_SP, true);
	if (IS_ERR_VALUE(ret))
		goto err;

	ret = cif_isp11_config_dcrop(dev, CIF_ISP11_STREAM_SP, true);
	if (IS_ERR_VALUE(ret))
		goto err;

	ret = cif_isp11_config_mi_sp(dev);
	if (IS_ERR_VALUE(ret))
		goto err;
...
static int cif_isp11_config_mp(
	struct cif_isp11_device *dev)
{
	int ret = 0;
	...
	ret = cif_isp11_config_rsz(dev, CIF_ISP11_STREAM_MP, true);
	if (IS_ERR_VALUE(ret))
		goto err;

	ret = cif_isp11_config_dcrop(dev, CIF_ISP11_STREAM_MP, true);
	if (IS_ERR_VALUE(ret))
		goto err;

	ret = cif_isp11_config_mi_mp(dev);
	if (IS_ERR_VALUE(ret))
		goto err;
	if (dev->config.jpeg_config.enable) {
		ret = cif_isp11_config_jpeg_enc(dev);
		if (IS_ERR_VALUE(ret))
			goto err;
...
static int cif_isp11_config_y12(
	struct cif_isp11_device *dev)
{
	int ret = 0;
    ...
    ret = cif_isp11_config_mi_y12(dev);
	if (IS_ERR_VALUE(ret))
		goto err;
	/*
	if (dev->config.jpeg_config.enable) {
...
static void cif_isp11_config_clk(
	struct cif_isp11_device *dev)
{
...
static int cif_isp11_config_cif(
	struct cif_isp11_device *dev,
	u32 stream_ids)
{
	int ret = 0;
	...
	cif_isp11_pltfrm_pr_dbg(dev->dev,
		"config MP = %d, config SP = %d, config Y12 = %d,img_src state = %s,"
		" PM state = %s, SP state = %s, MP state = %s, Y12 state = %s\n",
		(stream_ids & CIF_ISP11_STREAM_MP) == CIF_ISP11_STREAM_MP,
		(stream_ids & CIF_ISP11_STREAM_SP) == CIF_ISP11_STREAM_SP,
		(stream_ids & CIF_ISP11_STREAM_Y12) == CIF_ISP11_STREAM_Y12,
		cif_isp11_img_src_state_string(dev->img_src_state),
		cif_isp11_pm_state_string(dev->pm_state),
		cif_isp11_state_string(dev->sp_stream.state),
		cif_isp11_state_string(dev->mp_stream.state),
		cif_isp11_state_string(dev->y12_stream.state));

	cif_isp11_pltfrm_rtrace_printf(NULL,
		"start configuring CIF...\n");

	if ((stream_ids & CIF_ISP11_STREAM_MP) ||
		(stream_ids & CIF_ISP11_STREAM_SP) ||
		(stream_ids & CIF_ISP11_STREAM_Y12)) {

		ret = cif_isp11_set_pm_state(dev,
			CIF_ISP11_PM_STATE_SW_STNDBY);
		if (IS_ERR_VALUE(ret))
			goto err;

		if (!CIF_ISP11_INP_IS_DMA(dev->config.input_sel)) {
			/* configure sensor */
			ret = cif_isp11_config_img_src(dev);
			if (IS_ERR_VALUE(ret))
				goto err;
		}
		...
		dev->config.out_of_buffer_stall =
				CIF_ISP11_ALWAYS_STALL_ON_NO_BUFS;
		...
		dev->config.mi_config.async_updt = CIF_ISP11_ALWAYS_ASYNC;
		if (CIF_ISP11_INP_IS_DMA(dev->config.input_sel)) {
			dev->config.mi_config.async_updt |= CIF_ISP11_ASYNC_DMA;
			ret = cif_isp11_config_mi_dma(dev);
			if (IS_ERR_VALUE(ret))
				goto err;
		}
		if ((stream_ids & CIF_ISP11_STREAM_MP) &&
			...
			dev->config.mi_config.async_updt |=
				CIF_ISP11_ASYNC_JPEG;
		if (dev->config.isp_config.ism_config.ism_en)
			dev->config.mi_config.async_updt |=
				CIF_ISP11_ASYNC_ISM;

		if (PLTFRM_CAM_ITF_IS_MIPI(dev->config.cam_itf.type)) {
			ret = cif_isp11_config_mipi(dev);
			if (IS_ERR_VALUE(ret))
				goto err;
		}

		ret = cif_isp11_config_isp(dev);
		if (IS_ERR_VALUE(ret))
			goto err;
		...
		if (stream_ids & CIF_ISP11_STREAM_SP) {
			ret = cif_isp11_config_sp(dev);
			if (IS_ERR_VALUE(ret))
				goto err;
		}

		if (stream_ids & CIF_ISP11_STREAM_MP) {
			ret = cif_isp11_config_mp(dev);
			if (IS_ERR_VALUE(ret))
				goto err;
		}
		if (stream_ids & CIF_ISP11_STREAM_Y12) {
			ret = cif_isp11_config_y12(dev);
			if (IS_ERR_VALUE(ret))
				goto err;
		}

		ret = cif_isp11_config_path(dev, stream_ids);
		if (IS_ERR_VALUE(ret))
			goto err;
	}
	...
	if (dev->config.sp_config.rsz_config.ycflt_adjust ||
		...
		if (dev->sp_stream.state == CIF_ISP11_STATE_READY) {
			ret = cif_isp11_config_rsz(dev,
				CIF_ISP11_STREAM_SP, true);
			if (IS_ERR_VALUE(ret))
				goto err;
		} else {
...
static void cif_isp11_init_stream(
	struct cif_isp11_device *dev,
	enum cif_isp11_stream_id stream_id)
{
...
static int cif_isp11_jpeg_gen_header(
	struct cif_isp11_device *dev)
{
...
#ifdef CIF_ISP11_VERIFY_JPEG_HEADER
	{
...
static void cif_isp11_mi_update_buff_addr(
	struct cif_isp11_device *dev,
	enum cif_isp11_stream_id strm_id)
{
	if (strm_id == CIF_ISP11_STREAM_SP) {
		...
		cif_iowrite32_verify(0,
			dev->config.base_addr +
			CIF_MI_SP_Y_OFFS_CNT_INIT, CIF_MI_ADDR_SIZE_ALIGN_MASK);
		cif_iowrite32_verify(0,
			dev->config.base_addr +
			CIF_MI_SP_CB_OFFS_CNT_INIT, CIF_MI_ADDR_SIZE_ALIGN_MASK);
		cif_iowrite32_verify(0,
			dev->config.base_addr +
			CIF_MI_SP_CR_OFFS_CNT_INIT, CIF_MI_ADDR_SIZE_ALIGN_MASK);
		cif_isp11_pltfrm_pr_dbg(dev->dev,
			"\n  MI_SP_Y_BASE_AD 0x%08x/0x%08x\n"
	...
	} else if (strm_id == CIF_ISP11_STREAM_MP) {
		...
		cif_iowrite32_verify(0,
			dev->config.base_addr +
			CIF_MI_MP_CB_OFFS_CNT_INIT, CIF_MI_ADDR_SIZE_ALIGN_MASK);
		cif_iowrite32_verify(0,
			dev->config.base_addr +
			CIF_MI_MP_CR_OFFS_CNT_INIT, CIF_MI_ADDR_SIZE_ALIGN_MASK);
		cif_isp11_pltfrm_pr_dbg(dev->dev,
			"\n  MI_MP_Y_BASE_AD 0x%08x/0x%08x\n"
	...
	} else if (strm_id == CIF_ISP11_STREAM_Y12) {
		if (dev->config.mi_config.y12.output.pix_fmt == CIF_Y12) {
			cif_iowrite32_verify(dev->config.mi_config.y12.next_buff_addr,
			dev->config.base_addr +
			CIF_MI_MP_CR_BASE_AD_INIT, CIF_MI_ADDR_SIZE_ALIGN_MASK);
		} else {
			cif_iowrite32_verify(dev->config.mi_config.y12.next_buff_addr,
		...
		cif_iowrite32_verify(dev->config.mi_config.dma.next_buff_addr +
			dev->config.mi_config.dma.cb_offs,
			dev->config.base_addr +
			CIF_MI_DMA_CB_PIC_START_AD, CIF_MI_ADDR_SIZE_ALIGN_MASK);
		cif_iowrite32_verify(dev->config.mi_config.dma.next_buff_addr +
			dev->config.mi_config.dma.cr_offs,
			dev->config.base_addr +
			CIF_MI_DMA_CR_PIC_START_AD, CIF_MI_ADDR_SIZE_ALIGN_MASK);
		cif_isp11_pltfrm_pr_dbg(dev->dev,
...
static int cif_isp11_update_mi_mp(
	struct cif_isp11_device *dev)
{
	int ret = 0;
	enum cif_isp11_pix_fmt out_pix_fmt =
	...
	if (dev->config.jpeg_config.enable) {
		...
		if (!dev->config.jpeg_config.busy) {
			if ((dev->config.mi_config.mp.curr_buff_addr !=
				...
				ret = cif_isp11_jpeg_gen_header(dev);
				if (IS_ERR_VALUE(ret))
					goto err;
				cif_isp11_pltfrm_pr_dbg(NULL,
			...
			if (dev->config.mi_config.mp.next_buff_addr !=
				CIF_ISP11_INVALID_BUFF_ADDR)
				cif_isp11_mi_update_buff_addr(dev,
					CIF_ISP11_STREAM_MP);
			dev->config.mi_config.mp.curr_buff_addr =
				dev->config.mi_config.mp.next_buff_addr;
		}
		...
		if (dev->config.mi_config.mp.next_buff_addr !=
			...
			if (dev->config.mi_config.mp.next_buff_addr ==
				...
				cif_iowrite32AND_verify(
						...
						dev->config.base_addr + CIF_MI_CTRL,
						~0);
				if (CIF_ISP11_PIX_FMT_IS_RAW_BAYER(out_pix_fmt)) {
					cif_iowrite32OR_verify(
						CIF_MI_CTRL_RAW_ENABLE,
						dev->config.base_addr + CIF_MI_CTRL,
						~0);
				} else if (CIF_ISP11_PIX_FMT_IS_YUV(out_pix_fmt)) {
					cif_iowrite32OR_verify(
						CIF_MI_CTRL_MP_ENABLE_IN,
						dev->config.base_addr + CIF_MI_CTRL,
						~0);
			...
			cif_isp11_mi_update_buff_addr(dev, CIF_ISP11_STREAM_MP);
			dev->config.mi_config.mp.curr_buff_addr =
				dev->config.mi_config.mp.next_buff_addr;
		}
	}
...
static int cif_isp11_update_mi_y12(
	struct cif_isp11_device *dev)
{
	cif_isp11_pltfrm_pr_dbg(NULL,
		"curr 0x%08x next 0x%08x\n",
		dev->config.mi_config.y12.curr_buff_addr,
		dev->config.mi_config.y12.next_buff_addr);

	if (dev->config.mi_config.y12.next_buff_addr !=
		dev->config.mi_config.y12.curr_buff_addr) {
		if (dev->config.mi_config.y12.next_buff_addr ==
			CIF_ISP11_INVALID_BUFF_ADDR) {
			/* disable MI Y12 */
			...
			CIF_ISP11_INVALID_BUFF_ADDR) {
			/* re-enable MI Y12 */
			cif_isp11_pltfrm_pr_dbg(NULL, "enabling Y12 MI\n");
			if (dev->config.mi_config.y12.output.pix_fmt == CIF_Y12) {
		...
		cif_isp11_mi_update_buff_addr(dev, CIF_ISP11_STREAM_Y12);
		dev->config.mi_config.y12.curr_buff_addr =
			dev->config.mi_config.y12.next_buff_addr;
	}
...
static int cif_isp11_update_mi_sp(
	struct cif_isp11_device *dev)
{
	cif_isp11_pltfrm_pr_dbg(NULL,
		"curr 0x%08x next 0x%08x\n",
	...
	if (dev->config.mi_config.sp.next_buff_addr !=
		...
		if (dev->config.mi_config.sp.next_buff_addr ==
			CIF_ISP11_INVALID_BUFF_ADDR) {
			/* disable MI SP */
			cif_isp11_pltfrm_pr_dbg(NULL, "disabling SP MI\n");
			/* 'switch off' MI interface */
			...
			CIF_ISP11_INVALID_BUFF_ADDR) {
			/* re-enable MI SP */
			cif_isp11_pltfrm_pr_dbg(NULL, "enabling SP MI\n");
			cif_iowrite32(CIF_MI_SP_FRAME,
				dev->config.base_addr + CIF_MI_ICR);
			cif_iowrite32OR_verify(CIF_MI_CTRL_SP_ENABLE,
				dev->config.base_addr + CIF_MI_CTRL, ~0);
		}
		cif_isp11_mi_update_buff_addr(dev, CIF_ISP11_STREAM_SP);
		dev->config.mi_config.sp.curr_buff_addr =
			dev->config.mi_config.sp.next_buff_addr;
	}
	...
	u32 stride)
{
	...
	else if (CIF_ISP11_PIX_FMT_IS_RAW_BAYER(strm_fmt->frm_fmt.pix_fmt)) {
		if ((dev->sp_stream.state == CIF_ISP11_STATE_READY) ||
	...
	dev->mp_stream.state = CIF_ISP11_STATE_READY;

	if (!CIF_ISP11_INP_IS_DMA(dev->config.input_sel)) {
		ret = cif_isp11_img_src_select_strm_fmt(dev);
		if (IS_ERR_VALUE(ret)) {
			dev->mp_stream.updt_cfg = false;
			dev->mp_stream.state = CIF_ISP11_STATE_INACTIVE;
			goto err;
		}
	}
...
static int cif_isp11_s_fmt_y12(
	struct cif_isp11_device *dev,
	struct cif_isp11_strm_fmt *strm_fmt,
	u32 stride)
{
	...
	dev->config.mi_config.y12.output = strm_fmt->frm_fmt;
	dev->config.mi_config.y12.output.stride = stride;
	...
	dev->config.mi_config.y12.llength =
		cif_isp11_calc_llength(
			...
			stride,
			strm_fmt->frm_fmt.pix_fmt);
	cif_isp11_pltfrm_pr_dbg(dev->dev, "mi_config.y12.llength=%d",
								dev->config.mi_config.y12.llength);

	dev->y12_stream.updt_cfg = true;
	dev->y12_stream.state = CIF_ISP11_STATE_READY;
	...
	if (!CIF_ISP11_INP_IS_DMA(dev->config.input_sel)) {
		...
		if (IS_ERR_VALUE(ret)) {
			dev->y12_stream.updt_cfg = false;
			dev->y12_stream.state = CIF_ISP11_STATE_INACTIVE;
			goto err;
		}
	}
	...
	u32 stride)
{
	...
	if (!CIF_ISP11_PIX_FMT_IS_YUV(strm_fmt->frm_fmt.pix_fmt) &&
		!CIF_ISP11_PIX_FMT_IS_RGB(strm_fmt->frm_fmt.pix_fmt)) {
		cif_isp11_pltfrm_pr_err(dev->dev,
	...
	if (!CIF_ISP11_INP_IS_DMA(dev->config.input_sel)) {
		...
		if (IS_ERR_VALUE(ret)) {
			dev->sp_stream.updt_cfg = false;
			dev->sp_stream.state = CIF_ISP11_STATE_INACTIVE;
			goto err;
		}
	}
...
static int cif_isp11_s_fmt_dma(
	struct cif_isp11_device *dev,
	struct cif_isp11_strm_fmt *strm_fmt,
	u32 stride)
{
	...
	if (!CIF_ISP11_PIX_FMT_IS_YUV(strm_fmt->frm_fmt.pix_fmt) &&
		!CIF_ISP11_PIX_FMT_IS_RAW_BAYER(strm_fmt->frm_fmt.pix_fmt)) {
		cif_isp11_pltfrm_pr_err(dev->dev,
	...
	dev->dma_stream.state = CIF_ISP11_STATE_READY;
...
static void cif_isp11_dma_next_buff(
	struct cif_isp11_device *dev)
{
	cif_isp11_pltfrm_pr_dbg(NULL, "\n");
	...
	if (!list_empty(&dev->dma_stream.buf_queue) &&
		...
		if ((dev->y12_stream.state == CIF_ISP11_STATE_STREAMING) &&
			dev->y12_stream.curr_buf)
			dev->config.mi_config.y12.busy = true;
...
static void cif_isp11_dma_ready(
	struct cif_isp11_device *dev)
{
	...
	dev->config.mi_config.dma.busy = false;
	cif_isp11_pltfrm_event_signal(dev->dev, &dev->dma_stream.done);
}

static int cif_isp11_mi_frame_end(
	struct cif_isp11_device *dev,
	enum cif_isp11_stream_id stream_id)
{
	struct cif_isp11_stream *stream;
	...
	if (!stream->stall) {
		if (stream->curr_buf != NULL) {
			...
			if (stream->frame.frame_t) {
				frame_t = stream->frame.frame_t +
					stream->curr_buf->i;
				frame_t->fi_t = tv;
			if (stream_id == CIF_ISP11_STREAM_MP || stream_id == CIF_ISP11_STREAM_Y12)
				cif_isp11_pltfrm_pr_dbg(
					dev->dev,
					"v4l2buffer: %d  fi_t: (%ld, %ld)\n",
					stream->curr_buf->i,
					frame_t->fi_t.tv_sec,
					frame_t->fi_t.tv_usec);
			}
...
static void cif_isp11_start_mi(
	struct cif_isp11_device *dev,
	bool start_mi_sp,
	bool start_mi_mp,
	bool start_mi_y12)
{
	...
	if (start_mi_y12 &&
		(dev->y12_stream.state == CIF_ISP11_STATE_STREAMING))
		start_mi_y12 = false;
	if (!start_mi_sp && !start_mi_mp && !start_mi_y12)
		return;
	...
	if (start_mi_sp) {
		...
		cif_isp11_mi_frame_end(dev, CIF_ISP11_STREAM_SP);
	...
	if (start_mi_mp) {
		...
		dev->config.mi_config.mp.curr_buff_addr =
			CIF_ISP11_INVALID_BUFF_ADDR;
		spin_lock(&dev->vbq_lock);
		cif_isp11_mi_frame_end(dev, CIF_ISP11_STREAM_MP);
		spin_unlock(&dev->vbq_lock);
	...
	if (start_mi_y12) {
		...
		dev->config.mi_config.y12.curr_buff_addr =
			CIF_ISP11_INVALID_BUFF_ADDR;
		spin_lock(&dev->vbq_lock);
		cif_isp11_mi_frame_end(dev, CIF_ISP11_STREAM_Y12);
		spin_unlock(&dev->vbq_lock);
	...
	if (start_mi_sp) {
		...
		if (dev->sp_stream.curr_buf &&
			(!CIF_ISP11_INP_IS_DMA(dev->config.input_sel)))
			dev->config.mi_config.sp.busy = true;
	}
	...
	bool stop_mi_y12)
{
	...
	if (stop_mi_sp && stop_mi_mp && stop_mi_y12) {
		...
		cif_iowrite32AND_verify(~(CIF_MI_Y12_Y_FRAME |
			...
			dev->config.base_addr + CIF_MI_IMSC, ~0);
		cif_iowrite32(CIF_MI_Y12_Y_FRAME |
			CIF_MI_Y12_UV_FRAME|
			CIF_MI_ALL_FRAME,
			dev->config.base_addr + CIF_MI_ICR);
		cif_iowrite32AND_verify(~CIF_MI_CTRL_SP_ENABLE,
...
static void cif_isp11_requeue_bufs(
	struct cif_isp11_device *dev,
	struct cif_isp11_stream *stream)
{
	INIT_LIST_HEAD(&stream->buf_queue);
...
static void cif_isp11_stop_sp(
	struct cif_isp11_device *dev)
{
	int ret;
	if (dev->sp_stream.state ==
		CIF_ISP11_STATE_STREAMING) {
		dev->sp_stream.stop = true;
		ret = cif_isp11_pltfrm_event_wait_timeout(dev->dev,
			&dev->sp_stream.done,
			dev->sp_stream.state !=
		CIF_ISP11_STATE_STREAMING,
			1000000);
...
static void cif_isp11_stop_y12(
	struct cif_isp11_device *dev)
{
	int ret;
	if (dev->y12_stream.state ==
		CIF_ISP11_STATE_STREAMING) {
		dev->y12_stream.stop = true;
		ret = cif_isp11_pltfrm_event_wait_timeout(dev->dev,
			&dev->y12_stream.done,
			dev->y12_stream.state !=
		CIF_ISP11_STATE_STREAMING,
			1000000);
		dev->y12_stream.stop = false;
		if (IS_ERR_VALUE(ret)) {
			cif_isp11_pltfrm_pr_warn(NULL,
				"waiting on event returned with error %d\n",
				ret);
		}
		if (dev->config.mi_config.y12.busy)
			cif_isp11_pltfrm_pr_warn(NULL,
				"Y12 path still active while stopping it\n");
	}
}

static void cif_isp11_stop_dma(
	struct cif_isp11_device *dev)
{
	...
	if (dev->dma_stream.state ==
		...
		(void)cif_isp11_pltfrm_event_wait_timeout(dev->dev,
			&dev->dma_stream.done,
		...
		cif_isp11_requeue_bufs(dev, &dev->dma_stream);
		spin_unlock_irqrestore(&dev->vbq_lock, flags);
	}
}

static int cif_isp11_stop(
	struct cif_isp11_device *dev,
	bool stop_sp,
	bool stop_mp,
	bool stop_y12)
{
	...
	if ((stop_mp && stop_sp && stop_y12) ||
		...
		if (IS_ERR_VALUE(cif_isp11_set_pm_state(dev,
			CIF_ISP11_PM_STATE_SW_STNDBY)))
			cif_isp11_pltfrm_pr_dbg(dev->dev,
		...
		if (!dev->config.mi_config.async_updt) {
			local_irq_save(flags);
			cif_isp11_stop_mi(dev, true, false, false);
			local_irq_restore(flags);
		}
		...
		cif_iowrite32AND_verify(~(CIF_MI_Y12_Y_FRAME |
			...
			CIF_MI_ALL_FRAME),
			dev->config.base_addr + CIF_MI_IMSC, ~0);
		cif_iowrite32AND_verify(~(CIF_ISP_CTRL_ISP_Y12_Y_OUT_EN |
			CIF_ISP_CTRL_ISP_Y12_UV_OUT_EN),
			dev->config.base_addr + CIF_ISP_CTRL, ~0);
	} else /* stop_mp */ {
		if (!dev->config.mi_config.async_updt) {
			local_irq_save(flags);
			cif_isp11_stop_mi(dev, false, true, false);
			local_irq_restore(flags);
		}
	...
	if (stop_mp && (dev->mp_stream.state == CIF_ISP11_STATE_STREAMING))
		dev->mp_stream.state = CIF_ISP11_STATE_READY;

	if (stop_y12 && (dev->y12_stream.state == CIF_ISP11_STATE_STREAMING))
		dev->y12_stream.state = CIF_ISP11_STATE_READY;

	if (stop_sp && (dev->sp_stream.state == CIF_ISP11_STATE_STREAMING))
		dev->sp_stream.state = CIF_ISP11_STATE_READY;

	spin_lock(&dev->vbq_lock);
	if (stop_sp) {
		dev->config.mi_config.sp.busy = false;
		cif_isp11_requeue_bufs(dev, &dev->sp_stream);
	}
	if (stop_mp) {
		dev->config.mi_config.mp.busy = false;
		cif_isp11_requeue_bufs(dev, &dev->mp_stream);
	}

	if (stop_y12) {
		dev->config.mi_config.y12.busy = false;
		cif_isp11_requeue_bufs(dev, &dev->y12_stream);
	}
	...
	cif_isp11_pltfrm_pr_dbg(dev->dev,
		...
		cif_isp11_state_string(dev->sp_stream.state),
		cif_isp11_state_string(dev->mp_stream.state),
		cif_isp11_state_string(dev->dma_stream.state),
		cif_isp11_img_src_state_string(dev->img_src_state),
		cif_ioread32(dev->config.base_addr + CIF_MI_CTRL),
		cif_ioread32(dev->config.base_addr + CIF_ISP_CTRL),
		cif_ioread32(dev->config.base_addr + CIF_MIPI_CTRL));
	...
	bool start_y12)
{
	...
	cif_isp11_pltfrm_pr_dbg(dev->dev,
		...
		" start_sp = %d, start_mp = %d, start_y12 = %d\n"
		"  MI_INIT = 0x%08x\n",
		cif_isp11_state_string(dev->sp_stream.state),
		cif_isp11_state_string(dev->mp_stream.state),
		cif_isp11_state_string(dev->y12_stream.state),
	...
	/* Activate MI */
	cif_isp11_start_mi(dev, start_sp, start_mp, start_y12);
	...
	if (!CIF_ISP11_INP_IS_DMA(dev->config.input_sel)) {
		...
		ret = cif_isp11_img_src_set_state(dev,
			CIF_ISP11_IMG_SRC_STATE_STREAMING);
		if (IS_ERR_VALUE(ret))
			goto err;
	} else {
		cif_isp11_pltfrm_rtrace_printf(dev->dev,
			"starting DMA...\n");
		dev->dma_stream.state = CIF_ISP11_STATE_STREAMING;
...
static int cif_isp11_update_ism_dcr_rsz(
	struct cif_isp11_device *dev)
{
	int ret = 0;
	...
	/* Update ISM, cif_isp11_config_ism() changes the output size of isp,
	so it must be called before cif_isp11_config_rsz() */
	if (dev->config.isp_config.ism_config.ism_update_needed) {
		cif_isp11_config_ism(dev, false);
		if (dev->mp_stream.state == CIF_ISP11_STATE_STREAMING)
			dev->config.mp_config.rsz_config.ism_adjust = true;
		if (dev->sp_stream.state == CIF_ISP11_STATE_STREAMING)
			dev->config.sp_config.rsz_config.ism_adjust = true;
		...
		if (dev->config.isp_config.ism_config.ism_en)
			dev->config.mi_config.async_updt |= CIF_ISP11_ASYNC_ISM;
	}
...
static int cif_isp11_mi_isr(unsigned int mi_mis, void *cntxt)
{
	struct cif_isp11_device *dev = cntxt;
	unsigned int mi_mis_tmp;
...
*/
	...
	if (!CIF_ISP11_MI_IS_BUSY(dev) &&
		...
		if (dev->mp_stream.stop &&
			...
			cif_isp11_pltfrm_pr_dbg(NULL,
				"MP has stopped\n");
			cif_isp11_pltfrm_event_signal(dev->dev,
				&dev->mp_stream.done);
		}
		if (dev->y12_stream.stop &&
			(dev->y12_stream.state == CIF_ISP11_STATE_STREAMING)) {
		...
		if (dev->dma_stream.state == CIF_ISP11_STATE_STREAMING) {
			cif_isp11_dma_next_buff(dev);
		} else {
			if ((dev->sp_stream.state ==
				CIF_ISP11_STATE_STREAMING) &&
		...
		((dev->mp_stream.state == CIF_ISP11_STATE_STREAMING) ||
		(dev->sp_stream.state == CIF_ISP11_STATE_STREAMING)))
			cif_isp11_update_ism_dcr_rsz(dev);
...
static int cif_isp11_register_isrs(struct cif_isp11_device *dev)
{
	int ret = 0;

	cif_isp11_pltfrm_irq_register_isr(
		dev->dev,
		CIF_ISP_MIS,
		cif_isp11_isp_isr,
		dev);
	...
	ret = cif_isp11_pltfrm_irq_register_isr(
		dev->dev,
		CIF_MI_MIS,
		cif_isp11_mi_isr,
		dev);
...
err:
	cif_isp11_pltfrm_pr_err(dev->dev, "failed with error %d", ret);
	return ret;
}

/**Public Functions***********************************************************/

int cif_isp11_streamon(
	struct cif_isp11_device *dev,
	u32 stream_ids)
{
	int ret = 0;
	bool streamon_sp = stream_ids & CIF_ISP11_STREAM_SP;
	bool streamon_mp = stream_ids & CIF_ISP11_STREAM_MP;
	bool streamon_dma = stream_ids & CIF_ISP11_STREAM_DMA;
	bool streamon_y12 = stream_ids & CIF_ISP11_STREAM_Y12;

	cif_isp11_pltfrm_pr_dbg(dev->dev,
		"SP state = %s, MP state = %s, DMA state = %s, Y12 state = %s,"
		"streamon SP = %d, streamon MP = %d, streamon DMA = %d, streamon Y12 = %d\n",
		cif_isp11_state_string(dev->sp_stream.state),
		cif_isp11_state_string(dev->mp_stream.state),
		cif_isp11_state_string(dev->dma_stream.state),
		cif_isp11_state_string(dev->y12_stream.state),
		streamon_sp, streamon_mp, streamon_dma, streamon_y12);
	...
	if (streamon_mp && (dev->mp_stream.state != CIF_ISP11_STATE_READY)) {
		cif_isp11_pltfrm_pr_err(dev->dev,
	...
	if (streamon_y12 && (dev->y12_stream.state != CIF_ISP11_STATE_READY)) {
		cif_isp11_pltfrm_pr_err(dev->dev,
			"cannot start streaming on Y12 path, path not yet enabled\n");
		ret = -EFAULT;
		goto err;
	}
	...
	if (streamon_sp && dev->config.mi_config.raw_enable &&
		...
		(dev->mp_stream.state == CIF_ISP11_STATE_STREAMING))) {
		cif_isp11_pltfrm_pr_err(dev->dev,
			"cannot start streaming on SP path when MP is"
			"active and set to RAW output\n");
		ret = -EBUSY;
		goto err;
	}
	if (streamon_y12 && dev->config.mi_config.raw_enable &&
		(streamon_mp ||
		(dev->mp_stream.state == CIF_ISP11_STATE_STREAMING))) {
		cif_isp11_pltfrm_pr_err(dev->dev,
			"cannot start streaming on Y12 path when MP is"
			"active and set to RAW output\n");
		ret = -EBUSY;
		goto err;
	}
	...
	if (streamon_sp && dev->sp_stream.updt_cfg && (
		...
		(dev->y12_stream.state == CIF_ISP11_STATE_STREAMING))) {
			if (dev->mp_stream.state == CIF_ISP11_STATE_STREAMING) {
				ret = cif_isp11_stop(dev, false, true, false);
				if (IS_ERR_VALUE(ret))
					goto err;
				streamon_mp = true;
				dev->mp_stream.updt_cfg = true;
	...
	if (streamon_mp && dev->mp_stream.updt_cfg &&
		...
		(dev->y12_stream.state == CIF_ISP11_STATE_STREAMING))) {
		if (dev->sp_stream.state == CIF_ISP11_STATE_STREAMING) {
			ret = cif_isp11_stop(dev, true, false, false);
			if (IS_ERR_VALUE(ret))
				goto err;
	...
	if (streamon_y12 && dev->y12_stream.updt_cfg &&
		((dev->sp_stream.state == CIF_ISP11_STATE_STREAMING) ||
		(dev->mp_stream.state == CIF_ISP11_STATE_STREAMING))) {
		if (dev->sp_stream.state == CIF_ISP11_STATE_STREAMING) {
			ret = cif_isp11_stop(dev, true, false, false);
	...
	stream_ids = 0;
	if (streamon_mp && dev->mp_stream.updt_cfg)
		stream_ids |= CIF_ISP11_STREAM_MP;
	if (streamon_sp && dev->sp_stream.updt_cfg)
		stream_ids |= CIF_ISP11_STREAM_SP;
	if (streamon_y12 && dev->y12_stream.updt_cfg)
		stream_ids |= CIF_ISP11_STREAM_Y12;

	ret = cif_isp11_config_cif(dev, stream_ids);
	if (IS_ERR_VALUE(ret))
		goto err;

	ret = cif_isp11_start(dev, streamon_sp, streamon_mp, streamon_y12);
	if (IS_ERR_VALUE(ret))
		goto err;
...
int cif_isp11_streamoff(
	struct cif_isp11_device *dev,
	u32 stream_ids)
{
	int ret = 0;
	bool streamoff_sp = stream_ids & CIF_ISP11_STREAM_SP;
	bool streamoff_mp = stream_ids & CIF_ISP11_STREAM_MP;
	bool streamoff_dma = stream_ids & CIF_ISP11_STREAM_DMA;
	bool streamoff_y12 = stream_ids & CIF_ISP11_STREAM_Y12;

	cif_isp11_pltfrm_pr_dbg(dev->dev,
		"SP state = %s, MP state = %s, DMA state = %s, Y12 state = %s,"
		"streamoff SP = %d, streamoff MP = %d, streamoff DMA = %d, streamoff Y12 = %d\n",
		cif_isp11_state_string(dev->sp_stream.state),
		cif_isp11_state_string(dev->mp_stream.state),
		cif_isp11_state_string(dev->dma_stream.state),
		cif_isp11_state_string(dev->y12_stream.state),
	...
	if (streamoff_y12) {
		dev->config.jpeg_config.enable = false;
		dev->config.mi_config.raw_enable = false;
		dev->config.mi_config.y12.output.width = 0;
		dev->config.mi_config.y12.output.height = 0;
		dev->config.mi_config.y12.output.pix_fmt =
			CIF_UNKNOWN_FORMAT;
		if (dev->y12_stream.state == CIF_ISP11_STATE_READY)
			dev->y12_stream.state = CIF_ISP11_STATE_INACTIVE;
	...
	if (streamoff_dma) {
		cif_isp11_stop_dma(dev);
		if (dev->dma_stream.state == CIF_ISP11_STATE_READY)
			dev->dma_stream.state = CIF_ISP11_STATE_INACTIVE;
	}
	if ((dev->dma_stream.state <= CIF_ISP11_STATE_INACTIVE) &&
		(dev->mp_stream.state <= CIF_ISP11_STATE_INACTIVE) &&
		(dev->sp_stream.state <= CIF_ISP11_STATE_INACTIVE) &&
		(dev->y12_stream.state <= CIF_ISP11_STATE_INACTIVE)) {
	...
	cif_isp11_pltfrm_pr_dbg(dev->dev,
		"SP state = %s, MP state = %s, DMA state = %s, Y12 state = %s, # frames received = %d\n",
		cif_isp11_state_string(dev->sp_stream.state),
		cif_isp11_state_string(dev->mp_stream.state),
		cif_isp11_state_string(dev->dma_stream.state),
		cif_isp11_state_string(dev->y12_stream.state),
...
int cif_isp11_suspend(
	struct cif_isp11_device *dev)
{
	int ret = 0;

	cif_isp11_pltfrm_pr_dbg(dev->dev,
		"SP state = %s, MP state = %s, Y12 state = %s\n",
		cif_isp11_state_string(dev->sp_stream.state),
		cif_isp11_state_string(dev->mp_stream.state),
		cif_isp11_state_string(dev->y12_stream.state));

	if ((dev->pm_state == CIF_ISP11_PM_STATE_SUSPENDED) ||
		(dev->pm_state == CIF_ISP11_PM_STATE_OFF))
		return 0;

	dev->sp_stream.saved_state = dev->sp_stream.state;
	dev->mp_stream.saved_state = dev->mp_stream.state;
	dev->y12_stream.saved_state = dev->y12_stream.state;
	ret = cif_isp11_stop(dev, true, true, true);
	if (IS_ERR_VALUE(ret))
		goto err;
	ret = cif_isp11_set_pm_state(dev, CIF_ISP11_PM_STATE_SUSPENDED);
	if (IS_ERR_VALUE(ret))
		goto err;
	ret = cif_isp11_img_src_set_state(dev, CIF_ISP11_IMG_SRC_STATE_OFF);
	if (IS_ERR_VALUE(ret))
		goto err;
	...
	struct cif_isp11_device *dev)
{
	...
	if ((dev->sp_stream.saved_state == CIF_ISP11_STATE_READY) ||
		(dev->sp_stream.saved_state == CIF_ISP11_STATE_STREAMING)) {
		dev->sp_stream.updt_cfg = true;
		dev->sp_stream.state = CIF_ISP11_STATE_READY;
		if (dev->sp_stream.saved_state == CIF_ISP11_STATE_STREAMING)
			stream_ids |= CIF_ISP11_STREAM_SP;
	}
	if ((dev->mp_stream.saved_state == CIF_ISP11_STATE_READY) ||
		(dev->mp_stream.saved_state == CIF_ISP11_STATE_STREAMING)) {
		dev->mp_stream.updt_cfg = true;
		dev->mp_stream.state = CIF_ISP11_STATE_READY;
		if (dev->mp_stream.saved_state == CIF_ISP11_STATE_STREAMING)
			stream_ids |= CIF_ISP11_STREAM_MP;
	}
	if ((dev->y12_stream.saved_state == CIF_ISP11_STATE_READY) ||
		(dev->y12_stream.saved_state == CIF_ISP11_STATE_STREAMING)) {
		dev->y12_stream.updt_cfg = true;
		dev->y12_stream.state = CIF_ISP11_STATE_READY;
		if (dev->y12_stream.saved_state == CIF_ISP11_STATE_STREAMING)
			stream_ids |= CIF_ISP11_STREAM_Y12;
	}

	if ((dev->dma_stream.saved_state == CIF_ISP11_STATE_READY) ||
		(dev->dma_stream.saved_state == CIF_ISP11_STATE_STREAMING)) {
		dev->dma_stream.state = CIF_ISP11_STATE_READY;
		if (dev->dma_stream.saved_state == CIF_ISP11_STATE_STREAMING)
			stream_ids |= CIF_ISP11_STREAM_DMA;
	}

	return cif_isp11_streamon(dev, stream_ids);
}
	...
	u32 stride)
{
	...
	switch (stream_id) {
	case CIF_ISP11_STREAM_SP:
		return cif_isp11_s_fmt_sp(dev, strm_fmt, stride);
	case CIF_ISP11_STREAM_MP:
		return cif_isp11_s_fmt_mp(dev, strm_fmt, stride);
	case CIF_ISP11_STREAM_DMA:
		return cif_isp11_s_fmt_dma(dev, strm_fmt, stride);
	case CIF_ISP11_STREAM_Y12:
		return cif_isp11_s_fmt_y12(dev, strm_fmt, stride);
	default:
		cif_isp11_pltfrm_pr_err(NULL,
			"unknown/unsupported stream ID %d\n", stream_id);
		ret = -EINVAL;
	...
	u32 stream_ids)
{
	...
	if (stream_ids & ~(CIF_ISP11_ALL_STREAMS)) {
		cif_isp11_pltfrm_pr_err(NULL,
	...
	if ((dev->sp_stream.state == CIF_ISP11_STATE_DISABLED) &&
		(dev->mp_stream.state == CIF_ISP11_STATE_DISABLED) &&
		(dev->y12_stream.state == CIF_ISP11_STATE_DISABLED)) {
		(void)cif_isp11_s_input(dev, 0);
	...
	if (stream_ids & CIF_ISP11_STREAM_SP)
		cif_isp11_init_stream(dev, CIF_ISP11_STREAM_SP);
	if (stream_ids & CIF_ISP11_STREAM_MP)
		cif_isp11_init_stream(dev, CIF_ISP11_STREAM_MP);
	if (stream_ids & CIF_ISP11_STREAM_DMA)
		cif_isp11_init_stream(dev, CIF_ISP11_STREAM_DMA);
	if (stream_ids & CIF_ISP11_STREAM_Y12)
		cif_isp11_init_stream(dev, CIF_ISP11_STREAM_Y12);
	...
	int stream_ids)
{
	...
	if ((dev->sp_stream.state == CIF_ISP11_STATE_DISABLED) &&
		(dev->mp_stream.state == CIF_ISP11_STATE_DISABLED) &&
		(dev->dma_stream.state == CIF_ISP11_STATE_DISABLED) &&
		(dev->y12_stream.state == CIF_ISP11_STATE_DISABLED))
		return 0;
	...
	else if (stream_ids == CIF_ISP11_STREAM_SP)
		strm_dev = &dev->sp_stream;
	else if (stream_ids == CIF_ISP11_STREAM_Y12)
		strm_dev = &dev->y12_stream;
	else
		strm_dev = NULL;

	if (strm_dev) {
		if (strm_dev->frame.frame_t != NULL) {
			kfree(strm_dev->frame.frame_t);
			strm_dev->frame.frame_t = NULL;
			strm_dev->frame.cnt = 0;
		}
	}
	...
	if (stream_ids & CIF_ISP11_STREAM_SP) {
		if (dev->sp_stream.state == CIF_ISP11_STATE_STREAMING) {
			cif_isp11_pltfrm_pr_warn(dev->dev,
				"CIF SP in streaming state,"
				"should be stopped before release, trying to stop it\n");
			ret = cif_isp11_stop(dev, true, false, false);
			if (IS_ERR_VALUE(ret))
				goto err;
		}
		dev->sp_stream.state = CIF_ISP11_STATE_DISABLED;
	}
	if (stream_ids & CIF_ISP11_STREAM_MP) {
		if (dev->mp_stream.state == CIF_ISP11_STATE_STREAMING) {
			cif_isp11_pltfrm_pr_warn(dev->dev,
				"CIF MP in streaming state,"
				"should be stopped before release, trying to stop it\n");
			ret = cif_isp11_stop(dev, false, true, false);
			if (IS_ERR_VALUE(ret))
				goto err;
	...
	if ((dev->sp_stream.state == CIF_ISP11_STATE_DISABLED) &&
		...
		(dev->y12_stream.state == CIF_ISP11_STATE_DISABLED)) {
		if (IS_ERR_VALUE(cif_isp11_set_pm_state(dev,
			CIF_ISP11_PM_STATE_OFF)))
			cif_isp11_pltfrm_pr_warn(dev->dev,
			"CIF power off failed\n");
		if (dev->img_src != NULL) {
			if (IS_ERR_VALUE(cif_isp11_img_src_set_state(dev,
				CIF_ISP11_IMG_SRC_STATE_OFF)))
				cif_isp11_pltfrm_pr_warn(dev->dev,
...
struct cif_isp11_device *cif_isp11_create(
	...
	struct pltfrm_soc_cfg *soc_cfg)
{
	...
	dev = kzalloc(sizeof(struct cif_isp11_device), GFP_KERNEL);
	if (NULL == dev) {
		cif_isp11_pltfrm_pr_err(dev->dev,
	...
	ret = cif_isp11_pltfrm_dev_init(dev,
		&pdev, &dev->config.base_addr);
	if (IS_ERR_VALUE(ret))
		goto err;
	cif_isp11_pltfrm_debug_register_print_cb(
		dev->dev,
		(void (*)(void *, const char *))cif_isp11_debug_print_block,
		dev);

	cif_isp11_pltfrm_soc_init (dev, soc_cfg);

	ret = cif_isp11_img_srcs_init(dev);
	if (IS_ERR_VALUE(ret))
		goto err;

	ret = cif_isp11_register_isrs(dev);
	if (IS_ERR_VALUE(ret))
		goto err;

	(void)cif_isp11_init(dev, CIF_ISP11_ALL_STREAMS);
	dev->pm_state = CIF_ISP11_PM_STATE_OFF;
	dev->sp_stream.state = CIF_ISP11_STATE_DISABLED;
	dev->sp_stream.id = CIF_ISP11_STREAM_SP;
	dev->mp_stream.state = CIF_ISP11_STATE_DISABLED;
	dev->mp_stream.id = CIF_ISP11_STREAM_MP;
	dev->dma_stream.state = CIF_ISP11_STATE_DISABLED;
	dev->dma_stream.id = CIF_ISP11_STREAM_DMA;
	dev->y12_stream.state = CIF_ISP11_STATE_DISABLED;
	dev->y12_stream.id = CIF_ISP11_STREAM_Y12;
	...
	unsigned int input)
{
	...
	cif_isp11_pltfrm_pr_dbg(dev->dev,
		"setting input to %s\n",
		cif_isp11_inp_string(
		cif_isp11_input_index2inp(dev, input)));

	if (input >= dev->img_src_cnt + CIF_ISP11_INP_DMA_CNT()) {
		cif_isp11_pltfrm_pr_err(NULL,
			"invalid input %d\n", input);
		ret = -EINVAL;
		goto err;
	...
	inp = cif_isp11_input_index2inp(dev, input);

	/* DMA -> ISP or DMA -> IE */
	if ((inp == CIF_ISP11_INP_DMA) || (inp == CIF_ISP11_INP_DMA_IE))
...
const char *cif_isp11_g_input_name(
	struct cif_isp11_device *dev,
	unsigned int input_index)
{
	if (input_index >=
		dev->img_src_cnt + CIF_ISP11_INP_DMA_CNT()) {
		cif_isp11_pltfrm_pr_dbg(NULL,
	...
	if (input_index < dev->img_src_cnt)
		return cif_isp11_img_src_g_name(
			dev->img_src_array[input_index]);
	else
		return cif_isp11_inp_string(CIF_ISP11_INP_DMA +
			((input_index - dev->img_src_cnt) << 24));
}

int cif_isp11_qbuf(
	struct cif_isp11_device *dev,
	enum cif_isp11_stream_id stream,
	struct cif_isp11_buffer *buf)
{
	int ret = 0;

	cif_isp11_pltfrm_pr_dbg(dev->dev,
		"%s\n",
		cif_isp11_stream_id_string(stream));
	...
	struct v4l2_requestbuffers *req)
{
	...
	if (strm == CIF_ISP11_STREAM_MP)
		strm_dev = &dev->mp_stream;
	else if (strm == CIF_ISP11_STREAM_Y12)
		strm_dev = &dev->y12_stream;
	else if (strm == CIF_ISP11_STREAM_SP)
		strm_dev = &dev->sp_stream;
	else
		return 0;
	...
	strm_dev->frame.frame_t = kzalloc(
		req->count * sizeof(struct frame_timeinfo_s),
		GFP_KERNEL);
	strm_dev->frame.cnt = req->count;
	if (strm_dev->frame.frame_t == NULL) {
		cif_isp11_pltfrm_pr_err(dev->dev,
			"kzalloc frame_timeinfo_s failed for %s\n",
			(strm == CIF_ISP11_STREAM_MP) ? "mp" : ((strm == CIF_ISP11_STREAM_SP) ? "sp" : "y12"));
		return -ENOMEM;
	}
	...
	struct v4l2_buffer_timeinfo_s *p_v4l2buffer_t)
{
	...
	if (strm == CIF_ISP11_STREAM_MP)
		...
		cif_isp11_pltfrm_pr_err(dev->dev,
			"%s isn't support v4l2 buffer timeinfo\n",
			cif_isp11_stream_id_string(strm));
		return -EINVAL;
	}
	...
	if (dev->isp_dev.frame_id_setexp > 0) {
		if (p_v4l2buffer_t->frame.frame_id >=
		    dev->isp_dev.frame_id_setexp)
			p_v4l2buffer_t->frame.exposure_active =
				true;
	} else {
		if (dev->isp_dev.frame_id_setexp == 0) {
			if (p_v4l2buffer_t->frame.frame_id >=
	...
	cif_isp11_pltfrm_pr_dbg(
		dev->dev,
		"v4l2buffer: index: %d,  frame: ( %ld.%ld -> %ld.%ld),"
		" flash(%ld.%ld -> %ld.%ld))  exposure_active: %d\n",
		p_v4l2buffer_t->index,
		p_v4l2buffer_t->frame.vs_t.tv_sec,
		p_v4l2buffer_t->frame.vs_t.tv_usec,
		p_v4l2buffer_t->frame.fi_t.tv_sec,
		p_v4l2buffer_t->frame.fi_t.tv_usec,
		p_v4l2buffer_t->flash.mainflash_start_t.tv_sec,
		p_v4l2buffer_t->flash.mainflash_start_t.tv_usec,
		p_v4l2buffer_t->flash.mainflash_end_t.tv_sec,
		p_v4l2buffer_t->flash.mainflash_end_t.tv_usec,
		p_v4l2buffer_t->frame.exposure_active);
	...
	u32 *target_height)
{
	if (dev->sp_stream.state >= CIF_ISP11_STATE_READY) {
		...
		if ((dev->mp_stream.state >= CIF_ISP11_STATE_READY) &&
			...
			*target_height =
				dev->config.mi_config.sp.output.height;
	} else if (dev->mp_stream.state >= CIF_ISP11_STATE_READY) {
		*target_width = dev->config.mi_config.mp.output.width;
		*target_height = dev->config.mi_config.mp.output.height;
	} else if (dev->y12_stream.state >= CIF_ISP11_STATE_READY) {
		*target_width = dev->config.mi_config.y12.output.width;
		*target_height = dev->config.mi_config.y12.output.height;
	} else {
		cif_isp11_pltfrm_pr_err(dev->dev,
...
int cif_isp11_calc_isp_cropping(
	struct cif_isp11_device *dev,
	u32 *width,
	u32 *height,
	u32 *h_offs,
...
int cif_isp11_calc_min_out_buff_size(
	struct cif_isp11_device *dev,
	enum cif_isp11_stream_id stream_id,
	u32 *size)
{
	...
	u32 height;
	u32 bpp;
	struct cif_isp11_mi_path_config *mi_path;
	struct cif_isp11_stream *stream;
	...
	if (stream_id == CIF_ISP11_STREAM_SP) {
		mi_path = &dev->config.mi_config.sp;
		stream = &dev->sp_stream;
	} else if (stream_id == CIF_ISP11_STREAM_MP) {
		mi_path = &dev->config.mi_config.mp;
		stream = &dev->mp_stream;
	} else if (stream_id == CIF_ISP11_STREAM_Y12) {
		mi_path = &dev->config.mi_config.y12;
		stream = &dev->y12_stream;
	} else if (stream_id == CIF_ISP11_STREAM_DMA) {
	...
	if (stream->state < CIF_ISP11_STATE_READY) {
		cif_isp11_pltfrm_pr_err(NULL,
	...
	if (CIF_ISP11_PIX_FMT_IS_RAW_BAYER(pix_fmt) &&
		CIF_ISP11_PIX_FMT_GET_BPP(pix_fmt) > 8)
		/* RAW input > 8BPP is stored with 16BPP by MI */
		bpp = 16;
	else
		bpp = CIF_ISP11_PIX_FMT_GET_BPP(pix_fmt);
...
int cif_isp11_s_ctrl(
	struct cif_isp11_device *dev,
	const enum cif_isp11_cid id,
	int val)
{
	cif_isp11_pltfrm_pr_dbg(NULL,
		"id %d, val %d\n",
	...
	switch (id) {
	case CIF_ISP11_CID_SUPER_IMPOSE:
		dev->config.isp_config.si_enable = val;
		break;
	case CIF_ISP11_CID_IMAGE_EFFECT:
		if ((u32)val > CIF_ISP11_IE_NONE) {
			cif_isp11_pltfrm_pr_err(NULL,
		...
		break;
	case CIF_ISP11_CID_JPEG_QUALITY:
		if ((u32)val > 100) {
			cif_isp11_pltfrm_pr_err(NULL,
	...
	case CIF_ISP11_CID_FLASH_MODE:
		if ((u32)val > CIF_ISP11_FLASH_MODE_TORCH) {
			cif_isp11_pltfrm_pr_err(NULL,
				"unknown/unsupported flash mode (%d)\n", val);
			return -EINVAL;
		}
		dev->config.flash_mode = val;
		cif_isp11_img_src_s_ctrl(
			dev->img_src,
			CIF_ISP11_CID_FLASH_MODE,
			dev->config.flash_mode);
		if (dev->config.flash_mode == CIF_ISP11_FLASH_MODE_FLASH) {
		...
		} else if (dev->config.flash_mode == CIF_ISP11_FLASH_MODE_OFF) {
...
static void cif_isp11_hw_restart(struct cif_isp11_device *dev)
{
	cif_isp11_pltfrm_pr_dbg(NULL, "\n");

	cif_isp11_hw_errors[isp_pic_size_err].count = 0;
	cif_isp11_hw_errors[isp_data_loss].count = 0;
	cif_isp11_hw_errors[csi_err_protocol].count = 0;
	cif_isp11_hw_errors[csi_ecc1_err].count = 0;
	cif_isp11_hw_errors[csi_ecc2_err].count = 0;
	cif_isp11_hw_errors[csi_cs_err].count = 0;
...
int cif_isp11_mipi_isr(unsigned int mipi_mis, void *cntxt)
{
	struct cif_isp11_device *dev =
...
/* ======================================================================== */
int cif_isp11_isp_isr(unsigned int isp_mis, void *cntxt)
{
	struct cif_isp11_device *dev =
	    (struct cif_isp11_device *)cntxt;
	...
	if (isp_mis & CIF_ISP_V_START) {
		...
		if (dev->config.mi_config.sp.busy && dev->sp_stream.curr_buf) {
			if (dev->sp_stream.frame.frame_t) {
				frame_t = dev->sp_stream.frame.frame_t +
					dev->sp_stream.curr_buf->i;
				frame_t->vs_t = dev->curr_frame_time;
				frame_t->frame_id = dev->isp_dev.frame_id;
			}
		}
		if (dev->config.mi_config.mp.busy && dev->mp_stream.curr_buf) {
			if (dev->mp_stream.frame.frame_t) {
				frame_t = dev->mp_stream.frame.frame_t +
					dev->mp_stream.curr_buf->i;
				frame_t->vs_t = dev->curr_frame_time;
				frame_t->frame_id = dev->isp_dev.frame_id;
				cif_isp11_pltfrm_pr_dbg(
					dev->dev,
					"frame_id: %d,	v4l2buffer: %d	vs_t: (%ld, %ld)\n",
					dev->isp_dev.frame_id,
					dev->mp_stream.curr_buf->i,
					frame_t->vs_t.tv_sec,
					frame_t->vs_t.tv_usec);
			}
		}
		/*//not neccesary for y12 path
		if (dev->config.mi_config.y12.busy && dev->y12_stream.curr_buf) {
			if (dev->y12_stream.frame.frame_t) {
				frame_t = dev->y12_stream.frame.frame_t +
					dev->y12_stream.curr_buf->i;
				frame_t->vs_t = dev->curr_frame_time;
				frame_t->frame_id = dev->isp_dev.frame_id;
				cif_isp11_pltfrm_pr_dbg(
					dev->dev,
	...
	if ((isp_mis & (CIF_ISP_DATA_LOSS | CIF_ISP_PIC_SIZE_ERROR))) {
		...
		dev->y12_stream.stall = true;
		if ((isp_mis & CIF_ISP_PIC_SIZE_ERROR)) {
			/* Clear pic_size_error */
			cif_iowrite32(CIF_ISP_PIC_SIZE_ERROR,
				dev->config.base_addr +
				CIF_ISP_ICR);
			cif_isp11_hw_errors[isp_pic_size_err].count++;
			isp_err =
			    cif_ioread32(dev->config.base_addr +
					 CIF_ISP_ERR);
			dev_err(dev->dev,
			...
			cif_iowrite32(CIF_ISP_DATA_LOSS,
				dev->config.base_addr +
				CIF_ISP_ICR);
			cif_isp11_hw_errors[isp_data_loss].count++;
			dev_err(dev->dev,
				"CIF_ISP_DATA_LOSS\n");
			cif_iowrite32(CIF_ISP_DATA_LOSS,
				      dev->config.base_addr +
	...
	if (isp_mis & CIF_ISP_FRAME_IN) {
		...
		if (!CIF_ISP11_INP_IS_DMA(dev->config.input_sel) &&
			...
			if (dev->y12_stream.state == CIF_ISP11_STATE_STREAMING)
				mi_isr |= CIF_MI_Y12_Y_FRAME|CIF_MI_Y12_UV_FRAME|
						CIF_MI_ALL_FRAME;
...
void init_output_formats(void)
{
	...
	xgold_num_format =
	    (sizeof(cif_isp11_output_format) / sizeof(struct cif_isp11_fmt));
	...
	for (i = 0; i < xgold_num_format; i++) {
		...
		strlcpy((&fmtdesc)->description,
			cif_isp11_output_format[(&fmtdesc)->index].name,
			sizeof((&fmtdesc)->description));
		(&fmtdesc)->pixelformat =
		    cif_isp11_output_format[(&fmtdesc)->index].fourcc;
		(&fmtdesc)->flags =
		    cif_isp11_output_format[(&fmtdesc)->index].flags;
...
int get_cif_isp11_output_format_size(void)
{
	return sizeof(cif_isp11_output_format) / sizeof(struct cif_isp11_fmt);
}

struct cif_isp11_fmt *get_cif_isp11_output_format(int index)
{
	struct cif_isp11_fmt *fmt = NULL;

	if ((index >= 0) && (index < get_cif_isp11_output_format_size()))
		fmt = &cif_isp11_output_format[index];

	return fmt;
}

struct v4l2_fmtdesc *get_cif_isp11_output_format_desc(int index)
{
	struct v4l2_fmtdesc *desc = NULL;

	if ((index >= 0) && (index < get_cif_isp11_output_format_desc_size()))
		desc = &output_formats[index];

	return desc;
}

int get_cif_isp11_output_format_desc_size(void)
{
	return ARRAY_SIZE(cif_isp11_output_format);
}

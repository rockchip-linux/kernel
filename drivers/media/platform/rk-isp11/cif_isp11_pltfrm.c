...
struct cif_isp11_pltfrm_csi_config {
	struct list_head list;
	u32 pps;
	struct cif_isp11_csi_config csi_config;
};

struct cif_isp11_pltfrm_data {
...
#ifdef FPGA_TEST
void __iomem *cru_base_addr;
#endif
void cif_isp11_pltfrm_debug_register_print_cb(
	struct device *dev,
	void (*print)(void *cntxt, const char *block),
	void *cntxt) {
#ifndef CONFIG_DEBUG_FS
	struct cif_isp11_pltfrm_data *pdata = dev->platform_data;
...
#ifndef CONFIG_DEBUG_FS
#define CIF_ISP11_DBGFS_BUF_SIZE 1024
static char cif_isp11_dbgfs_buf[CIF_ISP11_DBGFS_BUF_SIZE];

static int cif_isp11_dbgfs_fill_csi_config_from_string(
	struct device *dev,
	struct cif_isp11_csi_config *csi_config,
	char *strp)
{
...
static int cif_isp11_dbgfs_csi_configs_init(
	struct device *dev,
	...
	struct list_head *csi_configs)
{
	...
	struct cif_isp11_pltfrm_csi_config *cfg = NULL;
	u32 pps;

	img_src_dev = cif_isp11_pltfrm_get_img_src_device(dev, inp);
	...
	while (!IS_ERR_OR_NULL(child_node =
		...
		if (!strncasecmp(child_node->name,
			...
			cfg = kmalloc(
				sizeof(struct cif_isp11_pltfrm_csi_config),
			GFP_KERNEL);
			if (cfg == NULL) {
				cif_isp11_pltfrm_pr_err(dev,
			...
			cfg->pps = pps;
			ret = cif_isp11_pltfrm_fill_csi_config_from_node(
...
static ssize_t cif_isp11_dbgfs_csi_read(
	struct file *f,
	char __user *out,
	size_t count,
	loff_t *pos)
{
	u32 out_size = 0;
	u32 str_len;
	struct cif_isp11_pltfrm_csi_config *cfg;
	u32 index = 0;
	...
	if (f->f_inode == pdata->dbgfs.csi0_file->d_inode) {
		...
		inp = CIF_ISP11_INP_CSI_0;
	} else if (f->f_inode == pdata->dbgfs.csi1_file->d_inode) {
		csi_configs = &pdata->csi1_configs;
		inp = CIF_ISP11_INP_CSI_1;
	} else {
		cif_isp11_pltfrm_pr_err(dev, "wrong file handle\n");
		return -EINVAL;
	}

	if (list_empty(csi_configs))
		if (IS_ERR_VALUE(cif_isp11_dbgfs_csi_configs_init(
				dev, inp, csi_configs)))
				return -EFAULT;
	...
	list_for_each(list_pos, csi_configs) {
		cfg = list_entry(list_pos,
			struct cif_isp11_pltfrm_csi_config, list);
		sprintf(cif_isp11_dbgfs_buf,
			"csi-config-%d:\n"
			"   pps = %d\n"
			"   vc = %d\n"
			"   nb_lanes = %d\n"
		...
		index++;
		str_len = strnlen(cif_isp11_dbgfs_buf,
			CIF_ISP11_DBGFS_BUF_SIZE);
		if (str_len > count) {
			*pos += out_size;
			return 0;
		}
		*pos = 0;
		if (IS_ERR_VALUE(simple_read_from_buffer(
			...
			cif_isp11_dbgfs_buf, str_len)))
			break;
		out_size += strnlen(cif_isp11_dbgfs_buf,
			CIF_ISP11_DBGFS_BUF_SIZE);
		count -= str_len;
	}
...
static ssize_t cif_isp11_dbgfs_csi_write(
	struct file *f,
	...
	loff_t *pos)
{
	...
	if (count > CIF_ISP11_DBGFS_BUF_SIZE) {
		cif_isp11_pltfrm_pr_err(dev, "command line too large\n");
		return -EINVAL;
	}
	...
	memset(cif_isp11_dbgfs_buf, 0, CIF_ISP11_DBGFS_BUF_SIZE);
	ret = simple_write_to_buffer(strp,
		CIF_ISP11_DBGFS_BUF_SIZE, pos, in, count);
	if (IS_ERR_VALUE(ret))
		return ret;
	...
	if (!strcmp(token, "push")) {
		...
		ret = cif_isp11_dbgfs_fill_csi_config_from_string(
			dev, &cfg.csi_config, strp);
		if (IS_ERR_VALUE(ret))
			return ret;
		ret = cif_isp11_pltfrm_l_s_csi_config(
		...
		cif_isp11_pltfrm_pr_err(dev, "unkown command %s\n", token);
		return -EINVAL;
	}
...
void cif_isp11_dbgfs_fill_sensor_aec_para(
	...
	u16 gain)
{
	struct cif_isp11_pltfrm_data *pdata;

	pdata = (struct cif_isp11_pltfrm_data *)
			cif_isp11_dev->dev->platform_data;
	pdata->exp_time = exp_time;
	pdata->gain = gain;
}

static ssize_t cif_isp11_dbgfs_sensor_read(
	struct file *f,
	char __user *out,
	size_t count,
	loff_t *pos)
...
static ssize_t cif_isp11_dbgfs_sensor_write(
	struct file *f,
	const char __user *in,
	size_t count,
	loff_t *pos)
...
static ssize_t cif_isp11_dbgfs_write(
	struct file *f,
	const char __user *in,
	size_t count,
	loff_t *pos)
{
	...
	if (!strncmp(token, "print", 5)) {
		...
		if (!strncmp(token, "on", 2)) {
			if (IS_ERR_VALUE(cif_isp11_pltfrm_pm_set_state(dev,
				CIF_ISP11_PM_STATE_SW_STNDBY, NULL)))
				cif_isp11_pltfrm_pr_err(dev,
					"power on failed\n");
			else
...
static const struct file_operations cif_isp11_dbgfs_csi_fops = {
	.read = cif_isp11_dbgfs_csi_read,
	.write = cif_isp11_dbgfs_csi_write
};

static const struct file_operations cif_isp11_dbgfs_fops = {
	.write = cif_isp11_dbgfs_write
};

static const struct file_operations cif_isp11_dbgfs_sensor_fops = {
	.read = cif_isp11_dbgfs_sensor_read,
	.write = cif_isp11_dbgfs_sensor_write
};

#ifdef CONFIG_CIF_ISP11_REG_TRACE

static inline int cif_isp11_pltfrm_trace_printf(
	struct device *dev,
	const char *fmt,
	va_list args)
{
	...
	if (!in_irq())
		spin_lock_irqsave(&cif_isp11_reg_trace.lock, flags);
	cif_isp11_reg_trace.internal = true;

	rem_size = cif_isp11_reg_trace.reg_trace_max_size -
		cif_isp11_reg_trace.reg_trace_write_pos;

	if (rem_size <= 0) {
		if (!in_irq())
			spin_unlock_irqrestore(
				&cif_isp11_reg_trace.lock, flags);
		cif_isp11_reg_trace.internal = false;
		return 0;
	}

	i = vsnprintf(cif_isp11_reg_trace.reg_trace +
		cif_isp11_reg_trace.reg_trace_write_pos,
		rem_size,
		fmt, args);
	if (i == rem_size) /* buffer full */
		...
		cif_isp11_reg_trace.reg_trace_write_pos += i;
	cif_isp11_reg_trace.internal = false;
	if (!in_irq())
		spin_unlock_irqrestore(&cif_isp11_reg_trace.lock, flags);

	return i;
}

inline int cif_isp11_pltfrm_rtrace_printf(
	struct device *dev,
	const char *fmt,
	...)
{
	...
	va_start(args, fmt);
	i = cif_isp11_pltfrm_trace_printf(dev, fmt, args);
	va_end(args);

	return i;
}

inline int cif_isp11_pltfrm_ftrace_printf(
	struct device *dev,
	const char *fmt,
	...)
{
	...
	if (!cif_isp11_reg_trace.ftrace ||
		cif_isp11_reg_trace.internal)
		return 0;
...
static void cif_isp11_dbgfs_reg_trace_clear(
	struct device *dev)
{
	cif_isp11_reg_trace.reg_trace_write_pos = 0;
	cif_isp11_reg_trace.reg_trace_read_pos = 0;
}

static ssize_t cif_isp11_dbgfs_reg_trace_read(
	struct file *f,
	char __user *out,
	size_t count,
	loff_t *pos)
{
	ssize_t bytes;
	size_t available = cif_isp11_reg_trace.reg_trace_write_pos -
		cif_isp11_reg_trace.reg_trace_read_pos;
	size_t rem = count;
	...
	while (rem && available) {
		...
		rem -= bytes;
		available = cif_isp11_reg_trace.reg_trace_write_pos -
			cif_isp11_reg_trace.reg_trace_read_pos;
	}
...
static ssize_t cif_isp11_dbgfs_reg_trace_write(
	struct file *f,
	const char __user *in,
	size_t count,
	loff_t *pos)
{
	...
	if (count > CIF_ISP11_DBGFS_BUF_SIZE) {
		cif_isp11_pltfrm_pr_err(dev, "command line too long\n");
		return -EINVAL;
	}
	...
	if (!strncmp(token, "clear", 5)) {
		cif_isp11_dbgfs_reg_trace_clear(dev);
		cif_isp11_pltfrm_pr_info(dev,
		...
		if (cif_isp11_reg_trace.reg_trace) {
			devm_kfree(dev, cif_isp11_reg_trace.reg_trace);
			cif_isp11_reg_trace.reg_trace = NULL;
		}
		cif_isp11_dbgfs_reg_trace_clear(dev);
		if (max_size > 0) {
			cif_isp11_reg_trace.reg_trace = devm_kzalloc(dev,
				max_size, GFP_KERNEL);
			if (!cif_isp11_reg_trace.reg_trace) {
				cif_isp11_pltfrm_pr_err(dev,
					"memory allocation failed\n");
			...
			cif_isp11_reg_trace.reg_trace_max_size = max_size;
			cif_isp11_pltfrm_pr_info(dev,
		...
		if (!strncmp(token, "on", 2)) {
			cif_isp11_reg_trace.rtrace = true;
			cif_isp11_pltfrm_pr_info(dev,
				"register trace enabled\n");
		} else if (!strncmp(token, "off", 3)) {
			cif_isp11_reg_trace.rtrace = false;
			cif_isp11_pltfrm_pr_info(dev,
		...
		if (!strncmp(token, "on", 2)) {
			cif_isp11_reg_trace.ftrace = true;
			cif_isp11_pltfrm_pr_info(dev,
				"function trace enabled\n");
		} else if (!strncmp(token, "off", 3)) {
			cif_isp11_reg_trace.ftrace = false;
			cif_isp11_pltfrm_pr_info(dev,
...
cif_isp11_dbgfs_reg_trace_fops = {
	.read = cif_isp11_dbgfs_reg_trace_read,
	.write = cif_isp11_dbgfs_reg_trace_write
};

#endif
#endif

static irqreturn_t cif_isp11_pltfrm_irq_handler(int irq, void *cntxt)
{
	...
	void *cif_isp11_dev = dev_get_drvdata(dev);
	...
	for (i = 0; i < ARRAY_SIZE(pdata->irq_handlers); i++) {
		...
		ret = pdata->irq_handlers[i].isr(mis_val, cif_isp11_dev);
		if (IS_ERR_VALUE(ret)) {
			cif_isp11_pltfrm_pr_err(NULL,
...
const char *cif_isp11_pltfrm_pm_state_string(
	enum cif_isp11_pm_state pm_state)
{
	switch (pm_state) {
	case CIF_ISP11_PM_STATE_OFF:
		return "CIF_ISP11_PM_STATE_OFF";
	case CIF_ISP11_PM_STATE_SUSPENDED:
		return "CIF_ISP11_PM_STATE_SUSPENDED";
	case CIF_ISP11_PM_STATE_SW_STNDBY:
		return "CIF_ISP11_PM_STATE_SW_STNDBY";
	case CIF_ISP11_PM_STATE_STREAMING:
		return "CIF_ISP11_PM_STATE_STREAMING";
	default:
		return "PM_STATE_UNKNOWN";
	}
}

inline void cif_isp11_pltfrm_write_reg(
	struct device *dev,
	u32 data,
	CIF_ISP11_PLTFRM_MEM_IO_ADDR addr)
{
...
#ifdef CONFIG_CIF_ISP11_REG_TRACE
	{
		...
		if (!in_irq())
			spin_lock_irqsave(&cif_isp11_reg_trace.lock, flags);
		cif_isp11_reg_trace.internal = true;
		if (((cif_isp11_reg_trace.reg_trace_write_pos +
			(20 * sizeof(char))) <
			cif_isp11_reg_trace.reg_trace_max_size) &&
			cif_isp11_reg_trace.rtrace) {
			int bytes =
				sprintf(cif_isp11_reg_trace.reg_trace +
					cif_isp11_reg_trace.reg_trace_write_pos,
					"%04x %08x\n",
					addr - cif_isp11_reg_trace.base_addr,
					data);
			if (bytes > 0)
				cif_isp11_reg_trace.reg_trace_write_pos +=
					bytes;
...
inline void cif_isp11_pltfrm_write_reg_OR(
	struct device *dev,
	u32 data,
	CIF_ISP11_PLTFRM_MEM_IO_ADDR addr)
{
	cif_isp11_pltfrm_write_reg(dev,
		(ioread32(addr) | data), addr);
}

inline void cif_isp11_pltfrm_write_reg_AND(
	struct device *dev,
	u32 data,
	CIF_ISP11_PLTFRM_MEM_IO_ADDR addr)
{
...
inline u32 cif_isp11_pltfrm_read_reg(
	struct device *dev,
	CIF_ISP11_PLTFRM_MEM_IO_ADDR addr)
{
	...
	void __iomem **reg_base_addr)
{
	...
	dev_set_drvdata(dev, cif_isp11_dev);
	cif_isp11_dev->dev = dev;
	...
	if (IS_ERR_OR_NULL(base_addr)) {
		cif_isp11_pltfrm_pr_err(NULL, "devm_ioremap_resource failed\n");
		if (IS_ERR(base_addr))
	...
	*reg_base_addr = base_addr;
...
#ifdef FPGA_TEST
	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "cru_base");
	if (res == NULL) {
		cif_isp11_pltfrm_pr_err(NULL,
			"cru_base platform_get_resource_byname failed\n");
		ret = -ENODEV;
		goto err;
	}
	cru_base_addr = devm_ioremap_resource(&pdev->dev, res);
	if (IS_ERR_OR_NULL(cru_base_addr)) {
		cif_isp11_pltfrm_pr_err(NULL, "cru_base devm_ioremap_resource failed\n");
		if (IS_ERR(cru_base_addr))
			ret = PTR_ERR(cru_base_addr);
		else
			ret = -ENODEV;
	}
#endif
	irq = platform_get_irq_byname(pdev, "cif_isp11_irq");
	if (IS_ERR_VALUE(irq)) {
		cif_isp11_pltfrm_pr_err(NULL, "get cif_isp11_irq failed\n");
		ret = irq;
		goto err;
	}
	...
	ret = devm_request_threaded_irq(dev,
			irq,
			cif_isp11_pltfrm_irq_handler,
			NULL,
			0,
			dev_driver_string(dev),
			dev);
	if (IS_ERR_VALUE(ret)) {
		cif_isp11_pltfrm_pr_err(NULL, "devm_request_threaded_irq failed\n");
		goto err;
	}
	pdata->irq = irq;
...
#ifndef CONFIG_DEBUG_FS
	pdata->dbgfs.dir = debugfs_create_dir("cif_isp11", NULL);
	pdata->dbgfs.csi0_file = debugfs_create_file(
		"csi-0",
		0644,
		pdata->dbgfs.dir,
		dev,
		&cif_isp11_dbgfs_csi_fops);
	pdata->dbgfs.csi1_file = debugfs_create_file(
		...
		dev,
		&cif_isp11_dbgfs_csi_fops);
	pdata->dbgfs.cif_isp11_file = debugfs_create_file(
		"cif_isp20",
		0200,
		pdata->dbgfs.dir,
		dev,
		&cif_isp11_dbgfs_fops);
	pdata->dbgfs.cif_isp11_file = debugfs_create_file(
		"sensor",
		0644,
		pdata->dbgfs.dir,
		dev,
		&cif_isp11_dbgfs_sensor_fops);
#ifdef CONFIG_CIF_ISP11_REG_TRACE
	pdata->dbgfs.reg_trace_file = debugfs_create_file(
		...
		dev,
		&cif_isp11_dbgfs_reg_trace_fops);
	spin_lock_init(&cif_isp11_reg_trace.lock);
	cif_isp11_reg_trace.reg_trace = NULL;
	cif_isp11_dbgfs_reg_trace_clear(dev);
	cif_isp11_reg_trace.reg_trace_max_size = 0;
	cif_isp11_reg_trace.base_addr = base_addr;
	cif_isp11_reg_trace.rtrace = true;
	cif_isp11_reg_trace.ftrace = false;
	cif_isp11_reg_trace.internal = false;
#endif
	...
	struct pltfrm_soc_cfg *soc_cfg)
{
	...
	struct device *dev = cif_isp11_dev->dev;
	struct platform_device *pdev =
		container_of(dev, struct platform_device, dev);
	...
	if (!IS_ERR_OR_NULL(soc_cfg) && !IS_ERR_OR_NULL(soc_cfg->soc_cfg)) {
		...
		init_para.pdev = pdev;
		init_para.isp_base = cif_isp11_dev->config.base_addr;
		ret = (soc_cfg->soc_cfg)(&cfg_para);
		if (ret == 0)
			cif_isp11_dev->soc_cfg = soc_cfg;
	}
...
{
	int ret;
	struct cif_isp11_device *cif_isp11_dev = dev_get_drvdata(dev);
	struct pltfrm_soc_cfg *soc_cfg = cif_isp11_dev->soc_cfg;
	struct pltfrm_soc_cfg_para cfg_para;
	...
	if (IS_ERR_VALUE(ret))
		cif_isp11_pltfrm_pr_err(dev,
			...
			cif_isp11_pltfrm_pm_state_string(pm_state), ret);
	else
		cif_isp11_pltfrm_pr_dbg(dev,
			"successfully changed pm state to %s\n",
			cif_isp11_pltfrm_pm_state_string(pm_state));
	return ret;
}
	...
	struct pltfrm_cam_itf *cam_itf)
{
	...
	ret = cif_isp11_img_src_ioctl(img_src,
			PLTFRM_CIFCAM_G_ITF_CFG, (void *)cam_itf);
	if (IS_ERR_VALUE(ret)) {
		cif_isp11_pltfrm_pr_err(
			dev,
			"cif_isp11_img_src_ioctl PLTFRM_CIFCAM_G_ITF_CFG failed!\n");
		return ret;
	}
	return 0;
}
...
{
	int ret = 0;
	struct cif_isp11_pltfrm_data *pdata = dev_get_platdata(dev);
	...
	case CIF_ISP11_PINCTRL_STATE_SLEEP:
		if (!IS_ERR_OR_NULL(pdata->pins_sleep))
			ret = pinctrl_select_state(pdata->pinctrl,
				pdata->pins_sleep);
		break;
	case CIF_ISP11_PINCTRL_STATE_ACTIVE:
	case CIF_ISP11_PINCTRL_STATE_DEFAULT:
		if (!IS_ERR_OR_NULL(pdata->pins_default))
			ret = pinctrl_select_state(pdata->pinctrl,
				pdata->pins_default);
		break;
	case CIF_ISP11_PINCTRL_STATE_INACTIVE:
	...
	return 0;
err:
	cif_isp11_pltfrm_pr_err(dev, "failed with error %d\n", ret);
	return ret;
}
	...
	void *cntxt)
{
	...
	switch (mis) {
	case CIF_MIPI_MIS:
		irq_name = "CIF_ISP11_MIPI_IRQ";
		break;
	case CIF_ISP_MIS:
		irq_name = "CIF_ISP11_ISP_IRQ";
		break;
	case CIF_MI_MIS:
		irq_name = "CIF_ISP11_MI_IRQ";
		break;
	default:
		cif_isp11_pltfrm_pr_err(dev,
	...
	unsigned int array_len)
{
	...
	for (index = 0; index < size/sizeof(*phandle); index++) {
		...
		if (!IS_ERR_OR_NULL(img_src_array[num_cameras])) {
			cif_isp11_pltfrm_pr_info(dev,
				"%s attach to cif isp11 img_src_array[%d]\n",
				cif_isp11_img_src_g_name(
					img_src_array[num_cameras]),
				num_cameras);
			num_cameras++;
			if (num_cameras >= array_len) {
				cif_isp11_pltfrm_pr_err(dev,
					"cif isp11 isn't support > %d"

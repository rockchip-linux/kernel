...
struct cif_isp11_v4l2_fh {
	enum cif_isp11_stream_id stream_id;
	struct v4l2_fh fh;
};

/* One structure per video node */
struct cif_isp11_v4l2_node {
	struct videobuf_queue buf_queue;
	struct video_device vdev;
	int users;
	struct cif_isp11_v4l2_fh *owner;
};

/* One structure per device */
struct cif_isp11_v4l2_device {
	struct cif_isp11_v4l2_node node[5];
};

spinlock_t iowrite32_verify_lock;

static struct cif_isp11_v4l2_fh *to_fh(struct file *file)
{
	if (file == NULL || file->private_data == NULL)
		return NULL;

	return container_of(file->private_data, struct cif_isp11_v4l2_fh, fh);
}

static struct cif_isp11_v4l2_node *to_node(struct cif_isp11_v4l2_fh *fh)
{
	...
	return container_of(vdev, struct cif_isp11_v4l2_node, vdev);
}

static struct videobuf_queue *to_videobuf_queue(
	struct file *file)
{
	struct cif_isp11_v4l2_fh *fh = to_fh(file);
	struct video_device *vdev = fh ? fh->fh.vdev : NULL;
	struct cif_isp11_v4l2_node *node = to_node(fh);
...
static enum cif_isp11_stream_id to_stream_id(
	struct file *file)
{
	struct cif_isp11_v4l2_fh *fh;
...
static struct cif_isp11_device *to_cif_isp11_device(
...
static const char *cif_isp11_v4l2_buf_type_string(
...
const char *cif_isp11_v4l2_pix_fmt_string(
	int pix_fmt)
{
	switch (pix_fmt) {
...
static int cif_isp11_v4l2_cid2cif_isp11_cid(u32 v4l2_cid)
{
	switch (v4l2_cid) {
	case V4L2_CID_FLASH_LED_MODE:
		return CIF_ISP11_CID_FLASH_MODE;
	case V4L2_CID_AUTOGAIN:
		return CIF_ISP11_CID_AUTO_GAIN;
	case V4L2_EXPOSURE_AUTO:
		return CIF_ISP11_CID_AUTO_EXPOSURE;
	case V4L2_CID_AUTO_WHITE_BALANCE:
		return CIF_ISP11_CID_AUTO_WHITE_BALANCE;
	case V4L2_CID_BLACK_LEVEL:
		return CIF_ISP11_CID_BLACK_LEVEL;
	case V4L2_CID_WHITE_BALANCE_TEMPERATURE:
		return CIF_ISP11_CID_WB_TEMPERATURE;
	case V4L2_CID_EXPOSURE:
		return CIF_ISP11_CID_EXPOSURE_TIME;
	case V4L2_CID_GAIN:
		return CIF_ISP11_CID_ANALOG_GAIN;
	case V4L2_CID_FOCUS_ABSOLUTE:
		return CIF_ISP11_CID_FOCUS_ABSOLUTE;
	case V4L2_CID_AUTO_N_PRESET_WHITE_BALANCE:
		return CIF_ISP11_CID_AUTO_N_PRESET_WHITE_BALANCE;
	case V4L2_CID_SCENE_MODE:
		return CIF_ISP11_CID_SCENE_MODE;
	case V4L2_CID_COLORFX:
		return CIF_ISP11_CID_IMAGE_EFFECT;
	case V4L2_CID_JPEG_COMPRESSION_QUALITY:
		return CIF_ISP11_CID_JPEG_QUALITY;
	case V4L2_CID_HFLIP:
		return CIF_ISP11_CID_HFLIP;
	case V4L2_CID_VFLIP:
		return CIF_ISP11_CID_VFLIP;
...
static enum cif_isp11_image_effect cif_isp11_v4l2_colorfx2cif_isp11_ie(
	u32 v4l2_colorfx)
{
	switch (v4l2_colorfx) {
	case V4L2_COLORFX_SEPIA:
		return CIF_ISP11_IE_SEPIA;
	case V4L2_COLORFX_BW:
		return CIF_ISP11_IE_BW;
	case V4L2_COLORFX_NEGATIVE:
		return CIF_ISP11_IE_NEGATIVE;
	case V4L2_COLORFX_EMBOSS:
		return CIF_ISP11_IE_EMBOSS;
	case V4L2_COLORFX_SKETCH:
		return CIF_ISP11_IE_SKETCH;
	case V4L2_COLORFX_NONE:
		return CIF_ISP11_IE_NONE;
	default:
		cif_isp11_pltfrm_pr_err(NULL,
...
static int cif_isp11_v4l2_register_video_device(
	...
	enum v4l2_buf_type buf_type)
{
	...
	cif_isp11_pltfrm_pr_dbg(dev->dev, "%s(%d)\n",
		cif_isp11_v4l2_buf_type_string(queue->type),
		(stream_ids & CIF_ISP11_STREAM_MP) ? ++streamon_cnt_mp :
		((stream_ids & CIF_ISP11_STREAM_SP) ? ++streamon_cnt_sp :
		((stream_ids & CIF_ISP11_STREAM_Y12) ? ++streamon_cnt_y12 :
		++streamon_cnt_dma)));
	...
	ret = cif_isp11_streamon(dev, stream_ids);
	if (IS_ERR_VALUE(ret)) {
...
err:
	...
	cif_isp11_pltfrm_pr_err(dev->dev, "failed with error %d\n", ret);
	return ret;
}

static int cif_isp11_v4l2_do_streamoff(
	struct file *file)
{
	int ret = 0;
	int err;
	...
	cif_isp11_pltfrm_pr_dbg(dev->dev, "%s\n",
		cif_isp11_v4l2_buf_type_string(queue->type));

	if (node->owner != fh)
		return -EBUSY;

	err = cif_isp11_streamoff(dev, stream_ids);
	if (IS_ERR_VALUE(err))
		ret = -EFAULT;
...
static int cif_isp11_v4l2_streamoff(
	struct file *file,
	void *priv,
	enum v4l2_buf_type buf_type)
{
	int ret = cif_isp11_v4l2_do_streamoff(file);
	if (IS_ERR_VALUE(ret))
		cif_isp11_pltfrm_pr_err(NULL,
			"failed with error %d\n", ret);
...
static int cif_isp11_v4l2_qbuf(
	struct file *file,
	void *priv,
	struct v4l2_buffer *buf)
{
	...
	cif_isp11_pltfrm_pr_dbg(NULL,
		...
		cif_isp11_v4l2_buf_type_string(queue->type),
		cif_isp11_v4l2_buf_type_string(buf->type),
...
static int cif_isp11_v4l2_dqbuf(
	struct file *file,
	void *priv,
	struct v4l2_buffer *buf)
{
...
static void cif_isp11_v4l2_buf_release(
	struct videobuf_queue *queue,
...
static void cif_isp11_v4l2_buf_queue(
	struct videobuf_queue *queue,
	struct videobuf_buffer *buf)
{
	struct cif_isp11_device *dev = to_cif_isp11_device(queue);
	...
	if (!IS_ERR_VALUE(cif_isp11_qbuf(dev, strm, buf)))
		buf->state = VIDEOBUF_QUEUED;
	else
		cif_isp11_pltfrm_pr_err(NULL, "failed\n");
}

static int cif_isp11_v4l2_buf_setup(
	struct videobuf_queue *queue,
	unsigned int *cnt,
	unsigned int *size)
{
	...
	ret = cif_isp11_calc_min_out_buff_size(
		dev, strm, size);
	if (IS_ERR_VALUE(ret)) {
...
static int cif_isp11_v4l2_buf_prepare(
	struct videobuf_queue *queue,
	struct videobuf_buffer *buf,
	enum v4l2_field field)
{
	...
	} else if (strm == CIF_ISP11_STREAM_DMA) {
		buf->width =
	...
	cif_isp11_pltfrm_pr_dbg(NULL, "%s buffer prepared %dx%d, size %d\n",
		cif_isp11_v4l2_buf_type_string(queue->type),
...
err:
	cif_isp11_pltfrm_pr_err(NULL, "failed with error %d\n", ret);
	cif_isp11_v4l2_buf_release(queue, buf);
	return ret;
}

static int cif_isp11_v4l2_reqbufs(
	struct file *file,
	void *priv,
	struct v4l2_requestbuffers *req)
{
	...
	cif_isp11_pltfrm_pr_dbg(NULL,
		...
		cif_isp11_v4l2_buf_type_string(queue->type),
		cif_isp11_v4l2_buf_type_string(req->type),
		req->count);
	...
	cif_isp11_reqbufs(dev, strm, req);
	return ret;
}

static int cif_isp11_v4l2_querybuf(
	struct file *file,
	void *priv,
	struct v4l2_buffer *buf)
{
	...
	cif_isp11_pltfrm_pr_dbg(NULL,
		"%s, index %d\n",
		cif_isp11_v4l2_buf_type_string(queue->type), buf->index);
...
static int cif_isp11_v4l2_s_ctrl(
	struct file *file,
	void *priv,
	struct v4l2_control *vc)
{
	struct videobuf_queue *queue = to_videobuf_queue(file);
	struct cif_isp11_device *dev = to_cif_isp11_device(queue);
	enum cif_isp11_cid id =
		cif_isp11_v4l2_cid2cif_isp11_cid(vc->id);
	int val = vc->value;
	...
	switch (vc->id) {
	case V4L2_CID_COLORFX:
		val = cif_isp11_v4l2_colorfx2cif_isp11_ie(val);
		break;
	case V4L2_CID_FLASH_LED_MODE:
		if (vc->value == V4L2_FLASH_LED_MODE_NONE)
			val = CIF_ISP11_FLASH_MODE_OFF;
		else if (vc->value == V4L2_FLASH_LED_MODE_FLASH)
			val = CIF_ISP11_FLASH_MODE_FLASH;
		else if (vc->value == V4L2_FLASH_LED_MODE_TORCH)
			val = CIF_ISP11_FLASH_MODE_TORCH;
		else
			val = -EINVAL;
		break;
	default:
	...
	return cif_isp11_s_ctrl(dev, id, val);
}

static int cif_isp11_v4l2_s_fmt(
	struct file *file,
	void *priv,
...
static int cif_isp11_v4l2_g_fmt(
	struct file *file,
	void *priv,
	struct v4l2_format *f)
{
...
static int cif_isp11_v4l2_s_input(
	struct file *file,
	void *priv,
	unsigned int i)
{
	...
	cif_isp11_pltfrm_pr_dbg(dev->dev, "setting input to %d\n", i);

	ret = cif_isp11_s_input(dev, i);
	if (IS_ERR_VALUE(ret))
		goto err;
...
static int cif_isp11_v4l2_enum_framesizes(
	struct file *file,
	void *priv,
...
const struct videobuf_queue_ops cif_isp11_qops = {
	.buf_setup = cif_isp11_v4l2_buf_setup,
	.buf_prepare = cif_isp11_v4l2_buf_prepare,
	.buf_queue = cif_isp11_v4l2_buf_queue,
	.buf_release = cif_isp11_v4l2_buf_release,
};

static int cif_isp11_v4l2_open(
	struct file *file)
{
	int ret;
	struct video_device *vdev = video_devdata(file);
	struct cif_isp11_device *dev = video_get_drvdata(vdev);
	struct cif_isp11_v4l2_fh *fh;
	struct cif_isp11_v4l2_node *node;
	enum v4l2_buf_type buf_type;
	enum cif_isp11_stream_id stream_id;
	struct cif_isp11_v4l2_device *cif_isp11_v4l2_dev =
		(struct cif_isp11_v4l2_device *)dev->nodes;

	cif_isp11_pltfrm_pr_dbg(NULL,
		"video device video%d.%d (%s)\n",
		vdev->num, vdev->minor, vdev->name);

	if (vdev->minor == cif_isp11_v4l2_dev->node[SP_DEV].vdev.minor) {
		buf_type = V4L2_BUF_TYPE_VIDEO_OVERLAY;
		stream_id = CIF_ISP11_STREAM_SP;
		...
		stream_id = CIF_ISP11_STREAM_DMA;
	} else {
		cif_isp11_pltfrm_pr_err(NULL,
	...
	ret = cif_isp11_init(dev, to_stream_id(file));
	if (IS_ERR_VALUE(ret)) {
		v4l2_fh_del(&fh->fh);
		v4l2_fh_exit(&fh->fh);
		kfree(fh);
...
static int cif_isp11_v4l2_release(struct file *file)
{
	int ret = 0;
	struct videobuf_queue *queue = to_videobuf_queue(file);
	struct cif_isp11_device *dev = to_cif_isp11_device(queue);
	struct cif_isp11_v4l2_fh *fh = to_fh(file);
	struct cif_isp11_v4l2_node *node = to_node(fh);
	enum cif_isp11_stream_id stream_id = to_stream_id(file);
	...
	if (!node->users) {
		if (queue->streaming)
			if (IS_ERR_VALUE(cif_isp11_v4l2_do_streamoff(file)))
				cif_isp11_pltfrm_pr_warn(dev->dev,
					"streamoff failed\n");

		/* Last close, so uninitialize hardware */
		ret = cif_isp11_release(dev, stream_id);
	}
...
static unsigned int cif_isp11_v4l2_poll(
	struct file *file,
			 ...
			 void *priv, struct v4l2_capability *cap)
{
	...
	if (stream_ids == CIF_ISP11_STREAM_SP)
		cap->capabilities = V4L2_CAP_VIDEO_OVERLAY |
			...
			V4L2_CAP_STREAMING;
	else if (stream_ids == CIF_ISP11_STREAM_MP)
		...
		cap->capabilities = V4L2_CAP_VIDEO_CAPTURE|
			V4L2_CAP_VIDEO_CAPTURE_MPLANE|
			V4L2_CAP_STREAMING;
	else if (stream_ids == CIF_ISP11_STREAM_DMA)
...
static int cif_isp11_v4l2_subscribe_event(struct v4l2_fh *fh,
...
static int cif_isp11_v4l2_unsubscribe_event(struct v4l2_fh *fh,
				const struct v4l2_event_subscription *sub)
{
	return v4l2_event_unsubscribe(fh, sub);
}

static void cif_isp11_v4l2_event(
	struct cif_isp11_device *dev,
	__u32 frame_sequence)
{
	...
	v4l2_event_queue(&cif_isp11_v4l2_dev->node[SP_DEV].vdev, &ev);
}

static void cif_isp11_v4l2_requeue_bufs(
	struct cif_isp11_device *dev,
	enum cif_isp11_stream_id stream_id)
{
	struct videobuf_buffer *buf;
	...
	if (stream_id == CIF_ISP11_STREAM_SP)
		q = &cif_isp11_v4l2_dev->node[SP_DEV].buf_queue;
	else if (stream_id == CIF_ISP11_STREAM_MP)
		q = &cif_isp11_v4l2_dev->node[MP_DEV].buf_queue;
	else if (stream_id == CIF_ISP11_STREAM_Y12)
		q = &cif_isp11_v4l2_dev->node[Y12_DEV].buf_queue;
	else if (stream_id == CIF_ISP11_STREAM_DMA)
		q = &cif_isp11_v4l2_dev->node[DMA_DEV].buf_queue;
	else
		BUG();

	dev = to_cif_isp11_device(q);

	list_for_each_entry(buf, &q->stream, stream) {
		if (!IS_ERR_VALUE(cif_isp11_qbuf(
			to_cif_isp11_device(q), stream_id, buf))) {
			...
			if ((buf->state == VIDEOBUF_QUEUED) ||
				...
				printk(KERN_DEBUG "ERR: buf->state is: %d\n",
					buf->state);
			   ...
			   struct v4l2_input *input)
{
	...
	inp_name = cif_isp11_g_input_name(dev, input->index);
...
static int mainpath_try_fmt_cap(struct v4l2_format *f)
{
	...
	for (ifmt = 0; ifmt < get_cif_isp11_output_format_size(); ifmt++) {
		if (pix->pixelformat == get_cif_isp11_output_format(ifmt)->fourcc)
			break;
	}

	if (ifmt == get_cif_isp11_output_format_size())
		ifmt = 0;

	pix->bytesperline = pix->width *
		get_cif_isp11_output_format(ifmt)->depth / 8;
			     ...
			     struct v4l2_fmtdesc *f)
{
	...
	xgold_num_format = get_cif_isp11_output_format_desc_size();
	if ((f->index >= xgold_num_format)
	    || (get_cif_isp11_output_format_desc(f->index)->pixelformat == 0)) {

		cif_isp11_pltfrm_pr_err(NULL, "index %d\n", f->index);
		return -EINVAL;
	}
	strlcpy(f->description,
		get_cif_isp11_output_format_desc(f->index)->description,
			sizeof(f->description));
	f->pixelformat = get_cif_isp11_output_format_desc(f->index)->pixelformat;
	f->flags = get_cif_isp11_output_format_desc(f->index)->flags;
	...
	struct v4l2_control *vc)
{
	...
	return cif_isp11_img_src_g_ctrl(dev->img_src,
		id, &vc->value);
}
	...
	struct v4l2_cropcap *a)
{
	...
	if ((dev->config.input_sel == CIF_ISP11_INP_DMA) ||
		(dev->config.input_sel == CIF_ISP11_INP_DMA_IE)) {
	...
	} else if (!CIF_ISP11_INP_IS_DMA(dev->config.input_sel)) {
		/* calculate cropping for aspect ratio */
		ret = cif_isp11_calc_isp_cropping(dev,
			&dev->isp_dev.input_width, &dev->isp_dev.input_height,
			&h_offs, &v_offs);
...
int cif_isp11_v4l2_g_crop(struct file *file, void *fh, struct v4l2_crop *a)
{
	return 0;
}
...
const struct v4l2_ioctl_ops cif_isp11_v4l2_sp_ioctlops = {
	.vidioc_reqbufs = cif_isp11_v4l2_reqbufs,
	.vidioc_querybuf = cif_isp11_v4l2_querybuf,
	.vidioc_qbuf = cif_isp11_v4l2_qbuf,
	.vidioc_dqbuf = cif_isp11_v4l2_dqbuf,
	.vidioc_streamon = cif_isp11_v4l2_streamon,
	.vidioc_streamoff = cif_isp11_v4l2_streamoff,
	.vidioc_s_input = cif_isp11_v4l2_s_input,
	.vidioc_enum_input = v4l2_enum_input,
	.vidioc_g_ctrl = v4l2_g_ctrl,
	.vidioc_s_ctrl = cif_isp11_v4l2_s_ctrl,
	.vidioc_s_fmt_vid_overlay = cif_isp11_v4l2_s_fmt,
	.vidioc_g_fmt_vid_overlay = cif_isp11_v4l2_g_fmt,
	...
	.vidioc_subscribe_event = cif_isp11_v4l2_subscribe_event,
	.vidioc_unsubscribe_event = cif_isp11_v4l2_unsubscribe_event,
	.vidioc_default = v4l2_default_ioctl,
};

const struct v4l2_ioctl_ops cif_isp11_v4l2_mp_ioctlops = {
	.vidioc_reqbufs = cif_isp11_v4l2_reqbufs,
	.vidioc_querybuf = cif_isp11_v4l2_querybuf,
	.vidioc_qbuf = cif_isp11_v4l2_qbuf,
	.vidioc_dqbuf = cif_isp11_v4l2_dqbuf,
	...
	.vidioc_s_ctrl = cif_isp11_v4l2_s_ctrl,
	.vidioc_s_fmt_vid_cap = cif_isp11_v4l2_s_fmt,
	.vidioc_g_fmt_vid_cap = cif_isp11_v4l2_g_fmt,
	.vidioc_enum_fmt_vid_cap = v4l2_enum_fmt_cap,
	.vidioc_enum_framesizes = cif_isp11_v4l2_enum_framesizes,
...
const struct v4l2_ioctl_ops cif_isp11_v4l2_y12_ioctlops = {
	.vidioc_reqbufs = cif_isp11_v4l2_reqbufs,
	.vidioc_querybuf = cif_isp11_v4l2_querybuf,
	.vidioc_qbuf = cif_isp11_v4l2_qbuf,
	.vidioc_dqbuf = cif_isp11_v4l2_dqbuf,
...
const struct v4l2_ioctl_ops cif_isp11_v4l2_dma_ioctlops = {
	.vidioc_reqbufs = cif_isp11_v4l2_reqbufs,
	.vidioc_querybuf = cif_isp11_v4l2_querybuf,
	.vidioc_qbuf = cif_isp11_v4l2_qbuf,
	.vidioc_dqbuf = cif_isp11_v4l2_dqbuf,
	.vidioc_streamon = cif_isp11_v4l2_streamon,
	.vidioc_streamoff = cif_isp11_v4l2_streamoff,
	.vidioc_s_fmt_vid_out = cif_isp11_v4l2_s_fmt,
	.vidioc_g_fmt_vid_out = cif_isp11_v4l2_g_fmt,
	.vidioc_cropcap = cif_isp11_v4l2_cropcap,
	.vidioc_s_crop = cif_isp11_v4l2_s_crop,
	.vidioc_g_crop = cif_isp11_v4l2_g_crop,
};
...
static int cif_isp11_v4l2_drv_probe(struct platform_device *pdev)
{
	...
	struct cif_isp11_device *dev = NULL;
	struct cif_isp11_v4l2_device *cif_isp11_v4l2_dev;
	int ret;
	...
	cif_isp11_v4l2_dev = devm_kzalloc(
				...
				GFP_KERNEL);
	if (IS_ERR_OR_NULL(cif_isp11_v4l2_dev)) {
		ret = -ENOMEM;
		goto err;
	}

	match = of_match_node(cif_isp11_v4l2_of_match, node);
	dev = cif_isp11_create(&pdev->dev,
		cif_isp11_v4l2_event,
		cif_isp11_v4l2_requeue_bufs,
	...
	ret = cif_isp11_v4l2_register_video_device(
		dev,
		&cif_isp11_v4l2_dev->node[SP_DEV].vdev,
		SP_VDEV_NAME,
		V4L2_CAP_VIDEO_OVERLAY,
		CIF_ISP11_V4L2_SP_DEV_MAJOR,
		&cif_isp11_v4l2_fops,
		&cif_isp11_v4l2_sp_ioctlops);
	if (ret)
		goto err;

	ret = register_cifisp_device(&dev->isp_dev,
		&cif_isp11_v4l2_dev->node[ISP_DEV].vdev,
		&dev->v4l2_dev,
	...
	ret = cif_isp11_v4l2_register_video_device(
		dev,
		&cif_isp11_v4l2_dev->node[MP_DEV].vdev,
		MP_VDEV_NAME,
		V4L2_CAP_VIDEO_CAPTURE,
		CIF_ISP11_V4L2_MP_DEV_MAJOR,
		&cif_isp11_v4l2_fops,
		&cif_isp11_v4l2_mp_ioctlops);
	if (ret)
		goto err;

	ret = cif_isp11_v4l2_register_video_device(
		dev,
		&cif_isp11_v4l2_dev->node[Y12_DEV].vdev,
		Y12_VDEV_NAME,
		V4L2_CAP_VIDEO_CAPTURE,
		CIF_ISP11_V4L2_Y12_DEV_MAJOR,
		&cif_isp11_v4l2_fops,
		&cif_isp11_v4l2_y12_ioctlops);
	if (ret)
		goto err;

	ret = cif_isp11_v4l2_register_video_device(
		dev,
		&cif_isp11_v4l2_dev->node[DMA_DEV].vdev,
		DMA_VDEV_NAME,
		V4L2_CAP_VIDEO_OUTPUT,
		CIF_ISP11_V4L2_DMA_DEV_MAJOR,
		&cif_isp11_v4l2_fops,
...
static int cif_isp11_v4l2_drv_remove(struct platform_device *pdev)
{
	struct cif_isp11_device *cif_isp11_dev =
		(struct cif_isp11_device *)platform_get_drvdata(pdev);
	struct cif_isp11_v4l2_device *cif_isp11_v4l2_dev =
		(struct cif_isp11_v4l2_device *)cif_isp11_dev->nodes;

	if (IS_ERR_VALUE(cif_isp11_release(cif_isp11_dev,
		CIF_ISP11_ALL_STREAMS)))
		cif_isp11_pltfrm_pr_warn(cif_isp11_dev->dev,
			"CIF power off failed\n");

	video_unregister_device(&cif_isp11_v4l2_dev->node[SP_DEV].vdev);
	video_unregister_device(&cif_isp11_v4l2_dev->node[MP_DEV].vdev);
	video_unregister_device(&cif_isp11_v4l2_dev->node[Y12_DEV].vdev);
	video_unregister_device(&cif_isp11_v4l2_dev->node[DMA_DEV].vdev);
	unregister_cifisp_device(&cif_isp11_v4l2_dev->node[ISP_DEV].vdev);
	v4l2_device_unregister(&cif_isp11_dev->v4l2_dev);
...
static int cif_isp11_v4l2_drv_suspend(struct platform_device *pdev,
	pm_message_t state)
{
	int ret = 0;
	struct cif_isp11_device *cif_isp11_dev =
	...
	cif_isp11_pltfrm_pr_dbg(cif_isp11_dev->dev, "\n");

	ret = cif_isp11_suspend(cif_isp11_dev);
	if (IS_ERR_VALUE(ret))
		goto err;

	cif_isp11_pltfrm_pinctrl_set_state(&pdev->dev,
		CIF_ISP11_PINCTRL_STATE_SLEEP);

	return 0;
err:
	cif_isp11_pltfrm_pr_err(cif_isp11_dev->dev,
		"failed with error %d\n", ret);
	return ret;
}

static int cif_isp11_v4l2_drv_resume(struct platform_device *pdev)
{
	int ret = 0;
	struct cif_isp11_device *cif_isp11_dev =
		(struct cif_isp11_device *)platform_get_drvdata(pdev);
	...
	if (!cif_isp11_dev->img_src) {
		cif_isp11_pltfrm_pr_err(
			cif_isp11_dev->dev,
			"cif_isp11_dev img_src is null!\n");
		goto err;
	}

	ret = cif_isp11_resume(cif_isp11_dev);
	if (IS_ERR_VALUE(ret))
		goto err;

	cif_isp11_pltfrm_pinctrl_set_state(&pdev->dev,
		CIF_ISP11_PINCTRL_STATE_DEFAULT);
...
static int cif_isp11_runtime_suspend(struct device *dev)
{
	cif_isp11_pltfrm_pr_dbg(dev, "\n");
	return cif_isp11_pltfrm_pm_set_state(dev, CIF_ISP11_PM_STATE_SUSPENDED);
}

static int cif_isp11_runtime_resume(struct device *dev)
{
	cif_isp11_pltfrm_pr_dbg(dev, "\n");
	return cif_isp11_pltfrm_pm_set_state(dev, CIF_ISP11_PM_STATE_SW_STNDBY);
}

static const struct dev_pm_ops cif_isp11_dev_pm_ops = {
	SET_RUNTIME_PM_OPS(cif_isp11_runtime_suspend,
			   cif_isp11_runtime_resume, NULL)
};

static struct platform_driver cif_isp11_v4l2_plat_drv = {
	.driver = {
		.name = DRIVER_NAME,
		.of_match_table = of_match_ptr(cif_isp11_v4l2_of_match),
		.pm = &cif_isp11_dev_pm_ops,
		   },
	.probe = cif_isp11_v4l2_drv_probe,
	.remove = cif_isp11_v4l2_drv_remove,
	.suspend = cif_isp11_v4l2_drv_suspend,
	.resume = cif_isp11_v4l2_drv_resume,
};
...
module_exit(cif_isp11_v4l2_exit);

MODULE_DESCRIPTION("V4L2 interface for CIF ISP11 driver");

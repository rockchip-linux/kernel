...
struct rk628_csi {
	...
	struct reset_control *rst_txphy_con;
	struct reset_control *rst_txesc;
	...
	struct clk *clk_txphy;
	struct clk *clk_txesc;
	struct clk *clk_csi0;
	struct regmap *grf;
...
static u8 edid_init_data[] = {
	...
	0x02, 0x01, 0x11, 0x22, 0x05, 0x23, 0x09, 0x07,
...
{
	static const int code_to_rate[] = { 44100 };
...
static const struct v4l2_ctrl_config rk628_csi_ctrl_audio_sampling_rate = {
	.id = RK_V4L2_CID_RK628CSI_AUDIO_SAMPLING_RATE,
...
static const struct v4l2_ctrl_config rk628_csi_ctrl_audio_present = {
	.id = RK_V4L2_CID_RK628CSI_AUDIO_PRESENT,
	.name = "Audio present",
...
static int rk628_csi_probe_of(struct rk628_csi *csi)
{
	...
	csi->clk_txphy = devm_clk_get(dev, "txphy");
	if (IS_ERR(csi->clk_txphy)) {
		ret = PTR_ERR(csi->clk_txphy);
		dev_err(dev, "failed to get clk_txphy: %d\n", ret);
		return ret;
	}

	csi->clk_txesc = devm_clk_get(dev, "txesc");
	if (IS_ERR(csi->clk_txesc)) {
		ret = PTR_ERR(csi->clk_txesc);
		dev_err(dev, "failed to get clk_txesc: %d\n", ret);
		return ret;
	}
	...
	csi->rst_txphy_con = of_reset_control_get(dev->of_node, "txphy");
	if (IS_ERR(csi->rst_txphy_con)) {
		ret = PTR_ERR(csi->rst_txphy_con);
		dev_err(dev, "failed to get rst_txphy_con control: %d\n", ret);
		return ret;
	}

	csi->rst_txesc = of_reset_control_get(dev->of_node, "txesc");
	if (IS_ERR(csi->rst_txesc)) {
		ret = PTR_ERR(csi->rst_txesc);
		dev_err(dev, "failed to get rst_txesc control: %d\n", ret);
		return ret;
	}
...
static int rk628_csi_remove(struct platform_device *pdev)
{
	...
	clk_disable_unprepare(csi->clk_txphy);
	clk_disable_unprepare(csi->clk_txesc);

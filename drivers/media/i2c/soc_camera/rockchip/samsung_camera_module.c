...
struct samsung_camera_module *to_samsung_camera_module(struct v4l2_subdev *sd)
{
	return container_of(sd, struct samsung_camera_module, sd);
}

/* ======================================================================== */

void samsung_camera_module_reset(
	struct samsung_camera_module *cam_mod)
{
	...
	cam_mod->rotation = 0;
	cam_mod->ctrl_updt = 0;
	cam_mod->state = SAMSUNG_CAMERA_MODULE_POWER_OFF;
	cam_mod->state_before_suspend = SAMSUNG_CAMERA_MODULE_POWER_OFF;
	cam_mod->exp_config.exp_time = 0;
	cam_mod->exp_config.gain = 0;
	cam_mod->vts_cur = 0;
}

static void samsung_camera_module_set_active_config(
	struct samsung_camera_module *cam_mod,
	struct samsung_camera_module_config *new_config)
{
	pltfrm_camera_module_pr_debug(&cam_mod->sd, "\n");
	...
	if (IS_ERR_OR_NULL(new_config)) {
		...
		cam_mod->ctrl_updt &= SAMSUNG_CAMERA_MODULE_CTRL_UPDT_AUTO_EXP |
			SAMSUNG_CAMERA_MODULE_CTRL_UPDT_AUTO_GAIN |
			SAMSUNG_CAMERA_MODULE_CTRL_UPDT_AUTO_WB;
		if (new_config->auto_exp_enabled !=
			cam_mod->exp_config.auto_exp) {
			cam_mod->ctrl_updt |=
				SAMSUNG_CAMERA_MODULE_CTRL_UPDT_AUTO_EXP;
			cam_mod->exp_config.auto_exp =
...
static struct samsung_camera_module_config *samsung_camera_module_find_config(
	struct samsung_camera_module *cam_mod,
...
static int samsung_camera_module_write_config(
	struct samsung_camera_module *cam_mod)
{
	int ret = 0;
	...
	ret = samsung_camera_module_write_reglist(cam_mod,
		reg_table, reg_table_num_entries);
	if (IS_ERR_VALUE(ret))
		goto err;
...
static int samsung_camera_module_attach(
	struct samsung_camera_module *cam_mod)
{
	int ret = 0;
...
int samsung_camera_module_try_fmt(struct v4l2_subdev *sd,
	struct v4l2_mbus_framefmt *fmt)
{
	struct samsung_camera_module *cam_mod = to_samsung_camera_module(sd);

	pltfrm_camera_module_pr_debug(&cam_mod->sd, "%dx%d, fmt code 0x%04x\n",
		fmt->width, fmt->height, fmt->code);

	if (IS_ERR_OR_NULL(samsung_camera_module_find_config(cam_mod, fmt, NULL))) {
		pltfrm_camera_module_pr_debug(&cam_mod->sd,
...
int samsung_camera_module_s_fmt(struct v4l2_subdev *sd,
	struct v4l2_mbus_framefmt *fmt)
{
	struct samsung_camera_module *cam_mod =  to_samsung_camera_module(sd);
	...
	config = samsung_camera_module_find_config(cam_mod, fmt, NULL);
	...
	if (cam_mod->frm_intrvl_valid &&
		!IS_ERR_OR_NULL(samsung_camera_module_find_config(
		cam_mod, fmt, &cam_mod->frm_intrvl))) {
		samsung_camera_module_set_active_config(cam_mod,
			samsung_camera_module_find_config(cam_mod,
				fmt, &cam_mod->frm_intrvl));
	} else {
		samsung_camera_module_set_active_config(cam_mod, config);
	}
...
int samsung_camera_module_g_fmt(struct v4l2_subdev *sd,
	struct v4l2_mbus_framefmt *fmt)
{
	struct samsung_camera_module *cam_mod =  to_samsung_camera_module(sd);
	...
	struct v4l2_subdev_frame_interval *interval)
{
	...
	config = samsung_camera_module_find_config(
			cam_mod,
	...
	if (!IS_ERR_OR_NULL(config) &&
		...
		if (cam_mod->state != SAMSUNG_CAMERA_MODULE_STREAMING)
			goto end;
...
int samsung_camera_module_s_stream(struct v4l2_subdev *sd, int enable)
{
	int ret = 0;
	struct samsung_camera_module *cam_mod =  to_samsung_camera_module(sd);
	unsigned int vts;
	...
	if (enable) {
		...
		if (cam_mod->update_config) {
			ret = samsung_camera_module_write_config(cam_mod);
			if (IS_ERR_VALUE(ret))
				goto err;
		}
		...
		ret = samsung_camera_module_ioctl(sd,
			RK_VIDIOC_SENSOR_MODE_DATA,
			&timings);

		cam_mod->state = SAMSUNG_CAMERA_MODULE_SW_STANDBY;
...
int samsung_camera_module_s_power(struct v4l2_subdev *sd, int on)
{
	int ret = 0;
	struct samsung_camera_module *cam_mod =  to_samsung_camera_module(sd);
	...
	if (on) {
		if (SAMSUNG_CAMERA_MODULE_POWER_OFF == cam_mod->state) {
			ret = pltfrm_camera_module_s_power(&cam_mod->sd, 1);
			if (!IS_ERR_VALUE(ret)) {
				mdelay(cam_mod->custom.power_up_delays_ms[0]);
				cam_mod->state = SAMSUNG_CAMERA_MODULE_HW_STANDBY;
			}
		}
		if (SAMSUNG_CAMERA_MODULE_HW_STANDBY == cam_mod->state) {
		...
		if (SAMSUNG_CAMERA_MODULE_SW_STANDBY == cam_mod->state) {
			ret = pltfrm_camera_module_set_pin_state(
			    &cam_mod->sd,
			    PLTFRM_CAMERA_MODULE_PIN_PD,
...
int samsung_camera_module_g_ctrl(struct v4l2_subdev *sd,
	...
	struct v4l2_ext_controls *ctrls)
{
	...
	for (i = 0; i < ctrls->count; i++) {
		...
		switch (ctrl->id) {
		case V4L2_CID_GAIN:
			ctrl_updt = SAMSUNG_CAMERA_MODULE_CTRL_UPDT_GAIN;
		...
		case RK_V4L2_CID_VTS:
			ctrl_updt = SAMSUNG_CAMERA_MODULE_CTRL_UPDT_VTS_VALUE;
		...
		case V4L2_CID_FLASH_LED_MODE:
			...
			break;
		case V4L2_CID_EXPOSURE:
			ctrl_updt = SAMSUNG_CAMERA_MODULE_CTRL_UPDT_EXP_TIME;
			...
			break;
		case V4L2_CID_WHITE_BALANCE_TEMPERATURE:
			ctrl_updt = SAMSUNG_CAMERA_MODULE_CTRL_UPDT_WB_TEMPERATURE;
			...
			break;
		case V4L2_CID_AUTO_N_PRESET_WHITE_BALANCE:
			ctrl_updt = SAMSUNG_CAMERA_MODULE_CTRL_UPDT_PRESET_WB;
			...
			break;
		case V4L2_CID_AUTOGAIN:
			ctrl_updt = SAMSUNG_CAMERA_MODULE_CTRL_UPDT_AUTO_GAIN;
			...
			break;
		case V4L2_CID_EXPOSURE_AUTO:
			ctrl_updt = SAMSUNG_CAMERA_MODULE_CTRL_UPDT_AUTO_EXP;
			...
			break;
		case V4L2_CID_AUTO_WHITE_BALANCE:
			ctrl_updt = SAMSUNG_CAMERA_MODULE_CTRL_UPDT_AUTO_WB;
		...
		case V4L2_CID_FOCUS_ABSOLUTE:
			...
			ctrl_updt =
				SAMSUNG_CAMERA_MODULE_CTRL_UPDT_FOCUS_ABSOLUTE;
	...
	if (ctrl_cnt &&
		(cam_mod->state == SAMSUNG_CAMERA_MODULE_STREAMING ||
		cam_mod->state == SAMSUNG_CAMERA_MODULE_SW_STANDBY)) {
		struct samsung_camera_module_ext_ctrls samsung_ctrls;

		samsung_ctrls.ctrls =
		(struct samsung_camera_module_ext_ctrl *)
		kmalloc(ctrl_cnt*sizeof(struct samsung_camera_module_ext_ctrl),
			GFP_KERNEL);

		if (samsung_ctrls.ctrls) {
			for (i = 0; i < ctrl_cnt; i++) {
				samsung_ctrls.ctrls[i].id = ctrls->controls[i].id;
				samsung_ctrls.ctrls[i].value =
					ctrls->controls[i].value;
			}

			samsung_ctrls.count = ctrl_cnt;

			ret = cam_mod->custom.s_ext_ctrls(cam_mod, &samsung_ctrls);

			kfree(samsung_ctrls.ctrls);
		} else {
			ret = -ENOMEM;
		}
		if (IS_ERR_VALUE(ret))
	...
	struct v4l2_control *ctrl)
{
	...
	return samsung_camera_module_s_ext_ctrls(sd, &ext_ctrls);
}
	...
	void *arg)
{
	...
	if (cmd == RK_VIDIOC_SENSOR_MODE_DATA) {
		struct samsung_camera_module_timings samsung_timings;
		struct isp_supplemental_sensor_mode_data *timings =
		(struct isp_supplemental_sensor_mode_data *)arg;

		if (cam_mod->custom.g_timings)
			ret = cam_mod->custom.g_timings(cam_mod, &samsung_timings);
		else
			ret = -EPERM;
		...
		timings->sensor_output_width = samsung_timings.sensor_output_width;
		timings->sensor_output_height = samsung_timings.sensor_output_height;
		timings->crop_horizontal_start =
			samsung_timings.crop_horizontal_start;
		timings->crop_vertical_start = samsung_timings.crop_vertical_start;
		timings->crop_horizontal_end = samsung_timings.crop_horizontal_end;
		timings->crop_vertical_end = samsung_timings.crop_vertical_end;
		timings->line_length_pck = samsung_timings.line_length_pck;
		timings->frame_length_lines = samsung_timings.frame_length_lines;
		timings->vt_pix_clk_freq_hz = samsung_timings.vt_pix_clk_freq_hz;
		timings->binning_factor_x = samsung_timings.binning_factor_x;
		timings->binning_factor_y = samsung_timings.binning_factor_y;
		timings->coarse_integration_time_max_margin =
			samsung_timings.coarse_integration_time_max_margin;
		timings->coarse_integration_time_min =
			samsung_timings.coarse_integration_time_min;
		timings->fine_integration_time_max_margin =
			samsung_timings.fine_integration_time_max_margin;
		timings->fine_integration_time_min =
			samsung_timings.fine_integration_time_min;
		...
		if (cam_mod->exp_config.exp_time)
			...
			timings->exp_time = samsung_timings.exp_time;
		if (cam_mod->exp_config.gain)
			timings->gain = cam_mod->exp_config.gain;
		else
			timings->gain = samsung_timings.gain;
...
int samsung_camera_module_init(struct samsung_camera_module *cam_mod,
	struct samsung_camera_module_custom_config *custom)
{
	int ret = 0;

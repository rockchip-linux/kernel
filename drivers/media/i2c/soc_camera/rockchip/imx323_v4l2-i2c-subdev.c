...
static int imx323_filltimings(struct imx_camera_module_custom_config *custom)
{
	...
	for (i = 0; i < custom->num_configs; i++) {
		...
		timings->coarse_integration_time_min = IMX323_COARSE_INTG_TIME_MIN;
		timings->coarse_integration_time_max_margin = IMX323_COARSE_INTG_TIME_MAX_MARGIN;

		/* IMX Sensor do not use fine integration time. */
		timings->fine_integration_time_min = IMX323_FINE_INTG_TIME_MIN;
		timings->fine_integration_time_max_margin = IMX323_FINE_INTG_TIME_MAX_MARGIN;
	}
...
static int imx323_check_camera_id(struct imx_camera_module *cam_mod)
{
	...
	if ((pid == IMX323_PID_MAGIC))
		imx_camera_module_pr_debug(cam_mod, "successfully detected camera ID 0x%02x\n", pid);
	else {
		imx_camera_module_pr_err(cam_mod, "wrong camera ID, expected 0x%02x, detected 0x%02x\n", IMX323_PID_MAGIC, pid);
		ret = -EINVAL;
		goto err;
	}
...
static struct of_device_id imx323_of_match[] = {

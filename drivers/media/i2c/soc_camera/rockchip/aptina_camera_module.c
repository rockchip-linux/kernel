...
int aptina_camera_module_s_power(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		if (APTINA_CAMERA_MODULE_POWER_OFF == cam_mod->state) {
		...
		if (APTINA_CAMERA_MODULE_SW_STANDBY == cam_mod->state) {
			ret = pltfrm_camera_module_set_pin_state(
			    &cam_mod->sd,
			    PLTFRM_CAMERA_MODULE_PIN_PD,
			    PLTFRM_CAMERA_MODULE_PIN_STATE_INACTIVE);
	...
	struct v4l2_ext_controls *ctrls)
{
	...
	for (i = 0; i < ctrls->count; i++) {
		...
		case V4L2_CID_EXPOSURE:
			...
			break;
		case V4L2_CID_WHITE_BALANCE_TEMPERATURE:
			ctrl_updt = APTINA_CAMERA_MODULE_CTRL_UPDT_WB_TEMPERATURE;
	...
	if (ctrl_cnt &&
		...
		if (aptina_ctrls.ctrls) {
			for (i = 0; i < ctrl_cnt; i++) {
				aptina_ctrls.ctrls[i].id = ctrls->controls[i].id;
	...
	void *arg)
{
	...
	if (cmd == RK_VIDIOC_SENSOR_MODE_DATA) {
		...
		timings->sensor_output_width = aptina_timings.sensor_output_width;
		timings->sensor_output_height = aptina_timings.sensor_output_height;
		timings->crop_horizontal_start =
			aptina_timings.crop_horizontal_start;
		timings->crop_vertical_start = aptina_timings.crop_vertical_start;
		timings->crop_horizontal_end = aptina_timings.crop_horizontal_end;
		timings->crop_vertical_end = aptina_timings.crop_vertical_end;
		timings->line_length_pck = aptina_timings.line_length_pck;
		timings->frame_length_lines = aptina_timings.frame_length_lines;
		timings->vt_pix_clk_freq_hz = aptina_timings.vt_pix_clk_freq_hz;
		timings->binning_factor_x = aptina_timings.binning_factor_x;
		timings->binning_factor_y = aptina_timings.binning_factor_y;

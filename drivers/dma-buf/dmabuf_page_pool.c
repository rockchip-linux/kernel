...
EXPORT_SYMBOL_GPL(dmabuf_page_pool_alloc_pages_array);
				       ...
				       struct page **pages, int npages)
{
	...
	for (i = 0; i < npages; i++)
		dmabuf_page_pool_free(pool, pages[i]);

	kvfree(pages);
}
EXPORT_SYMBOL_GPL(dmabuf_page_pool_free_pages_array);

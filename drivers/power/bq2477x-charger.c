/*
 * bq2477x-charger.c -- BQ24770/3 Charger driver
 *
...
enum bq2477x_chip_id { BQ24770, BQ24773};
				...
				struct bq2477x_platform_data *pdata)
{
	...
	pdata->charger_detect_gpio = of_get_named_gpio_flags(np,
						"ti,charger-detect-gpio",
						0, &flags);
	if (pdata->charger_detect_gpio >= 0)
		pdata->charger_detect_gpio_active_low = flags &
							OF_GPIO_ACTIVE_LOW;
	else
		dev_warn(&client->dev, "invalid charger_detect_gpio\n");
}
...
MODULE_DESCRIPTION("BQ24770/BQ24773 battery charger driver");

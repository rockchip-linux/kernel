/*
 * drivers/input/sensors/access/ls_stk3410.c
 *
...
static int sensor_init(struct i2c_client *client)
{
	...
	result = sensor_write_reg(client, THDL2_ALS, (unsigned char)(als_val>>8));
	if (result) {
		dev_err(&client->dev, "%s:write THDL1_ALS fail\n", __func__);
		return result;
	}
	...
	result = sensor_write_reg(client, THDH2_ALS,
	...
	result = sensor_write_reg(client, ALS_CTRL,
...
static int sensor_report_value(struct i2c_client *client)
{
	...
	if (sensor->pdata->irq_enable && sensor->ops->int_status_reg) {
		...
		if (value & STK_FLAG_ALSINT) {
			...
			result = sensor_write_reg(client, sensor->ops->int_status_reg,
				value);
			if (result) {

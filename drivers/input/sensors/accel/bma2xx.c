/*
 * drivers/input/sensors/access/bma2xx.c
 *
...
#define Y_AXIS_COMPEN	0		/*Y_AXIS offset is caused by screws, there needs to be compensation 3.5*/
...
static int sensor_report_value(struct i2c_client *client)
{
	 ...
	 case 0x02:
		for (i = 0; i < 3; i++) {
			...
			if (first_value == 1) {
				...
				if (sign_value == 1) {
					if (i == 0)
						input_report_rel(sensor->input_dev,
								 HIGH_G_INTERRUPT,
			...
			DBG("High G interrupt happened,exis is %d,"
	 ...
	 case 0x04:
		for (i = 0; i < 3; i++) {
			...
			if (first_value == 1) {
			 ...
			 DBG("Slop interrupt happened,exis is %d,"
					 "first is %d,sign is %d\n", i,
					 first_value, sign_value);
		 }
		 break;
	 ...
	 case 0x40:
		 bma2x2_get_orient_status(client,
					  &first_value);
		 DBG("orient interrupt happened,%s\n",
			orient[first_value]);
		if (first_value == 0)
			input_report_abs(sensor->input_dev, ORIENT_INTERRUPT,
	 ...
	 case 0x80:
		 bma2x2_get_orient_flat_status(client,
					       &sign_value);
		 DBG("flat interrupt happened,flat status is %d\n",
			sign_value);
		if (sign_value == 1) {

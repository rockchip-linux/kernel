...
#define POWERMODE_BW_VALUE                            0x5e
#define ODR_AXIS_DISABLE_VALUE                        0x06

#ifdef  CONFIG_WLAN
#define NRIPCV HIGH_LEVEL
#else
#define NRIPCV LOW_LEVEL
#endif

#else
#define NRIPCV 0x04
#define POWERMODE_BW_VALUE                            0x1e
#define ODR_AXIS_DISABLE_VALUE                        0x07
...
int                                 Log_level =
	DEBUG_ERR;//|DEBUG_ASSERT|DEBUG_MSG|DEBUG_FUNC|DEBUG_DATA;
...
static int filter_average_enhance(FilterAverageContext *fac, short sample)
{
	...
	if (fac->filter_param_l == fac->filter_param_h) {
		fac->sample_l = fac->sample_h = filter_average(fac->sample_l, sample,
						fac->filter_param_l, &fac->refN_l);
	} else {
		fac->sample_l = filter_average(fac->sample_l, sample, fac->filter_param_l,
					       &fac->refN_l);
		fac->sample_h = filter_average(fac->sample_h, sample, fac->filter_param_h,
					       &fac->refN_h);
				      ...
				      unsigned char count, unsigned char *data)
{
	...
	res = (count == mir3da_gsensor_drv.method->smi.read_block(handle,
			MIR3DA_REG_ADDR(addr), count, data)) ? 0 : 1;
...
static int mir3da_read_raw_data(MIR_HANDLE handle, short *x, short *y, short *z)
{
	...
	if (mir3da_register_read_continuously(handle,
					      mir3da_gsensor_drv.obj[gsensor_mod].data.data_sect[0].addr, 6, tmp_data) != 0) {
		MI_ERR("i2c block read failed\n");
		return -1;
	}

	*x = ((short)(tmp_data[1] <<
		      mir3da_gsensor_drv.obj[gsensor_mod].data.data_fmt.msbw | tmp_data[0])) >>
	     (8 - mir3da_gsensor_drv.obj[gsensor_mod].data.data_fmt.lsbw);
	*y = ((short)(tmp_data[3] <<
		      mir3da_gsensor_drv.obj[gsensor_mod].data.data_fmt.msbw | tmp_data[2])) >>
	     (8 - mir3da_gsensor_drv.obj[gsensor_mod].data.data_fmt.lsbw);
	*z = ((short)(tmp_data[5] <<
		      mir3da_gsensor_drv.obj[gsensor_mod].data.data_fmt.msbw | tmp_data[4])) >>
	     (8 - mir3da_gsensor_drv.obj[gsensor_mod].data.data_fmt.lsbw);
...
int mir3da_read_offset(MIR_HANDLE handle, unsigned char *offset)
{
	...
	for (i = 0; i < MIR3DA_OFF_SECT_LEN; i++) {
		...
		res = mir3da_register_read(handle,
					   mir3da_gsensor_drv.obj[gsensor_mod].offset_sect[i].addr, &offset[i]);
		if (res != 0) {
			return res;
		}
	}
...
static int mir3da_write_offset_to_file(unsigned char *offset)
{
	...
	MI_MSG("====sensor_sync_write, offset = 0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x",
...
static int mir3da_read_offset_from_file(unsigned char *offset)
{
	...
	MI_MSG("====sensor_sync_read, offset = 0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x",
	       offset[0], offset[1], offset[2], offset[3], offset[4], offset[5], offset[6],
	       offset[7], offset[8]);
			      ...
			      int fine_step[3], int xyz[3])
{
	...
	for (i = 0; i < 3; i++) {
		...
		mir3da_cali_off_to_lsb((xyz[i] - target[i]), &coarse_delta[i], coarse_step[i],
				       &fine_delta[i], fine_step[i]);
	...
	MI_MSG("---calibrate_Done, x = %d, y = %d, z = %d, coarse_x = %d, coarse_y = %d, coarse_z = %d, fine_x = %d, fine_y = %d, fine_z = %d",
	       xyz[0], xyz[1], xyz[2], coarse[0], coarse[1], coarse[2], fine[0], fine[1],
	       fine[2]);
			 ...
			 int fine_step[3], int fine_max, int target[3])
{
	...
	for (i = 0; i < 3; i++) {
		...
		coarse[i] += coarse_delta[i];
		fine[i] += fine_delta[i];
		mir3da_register_mask_write(handle, NSA_REG_COARSE_OFFSET_TRIM_X + i, 0x3f,
					   (unsigned char)coarse[i]);
	}
	...
	for (i = 0; i < 3; i++) {
		...
		mir3da_register_mask_write(handle, NSA_REG_COARSE_OFFSET_TRIM_X + i, 0xc0,
...
EXIT_16G_MOD:
	...
	for (i = 0; i < nLoop; i++) {
		...
		for (j = 0; j < 3; j++) {
			MI_MSG("xyz[%d] = %d, caorse[%d] = 0x%x, fine[%d] = 0x%x", j, xyz[j], j,
			       coarse[j], j, fine[j]);
			...
			mir3da_cali_off_to_lsb((xyz[j] - target[j]), &coarse_delta[j], coarse_step[j],
					       &fine_delta[j], fine_step[j]);

			coarse[j] += coarse_delta[j];
			fine[j] += fine_delta[j];
			mir3da_register_write(handle, NSA_REG_FINE_OFFSET_TRIM_X + j,
					      (unsigned char)(fine[j] & 0xff));
			mir3da_register_mask_write(handle, NSA_REG_COARSE_OFFSET_TRIM_X + j, 0xFF,
				  ...
				  int x100_fine[3], int x100_cust[3])
{
	...
	if (mir3da_register_read_continuously(handle, NSA_REG_SENSITIVITY_TRIM_X, 3,
					      fine_gain) != 0) {
		MI_ERR("i2c block read failed\n");
		return -1;
	}
	...
	for (i = 0; i < 3; i++) {
		...
		total_gain[i] = ((1000 + (fine_gain[i] & 0x1F) * 1000 / 32) / 15) *
				fine_dig_gain_map[((fine_gain[i] >> 5) & 0x03)] * coarse_gain_map[coarse_gain &
						0x07];
...
static int SOCLE_auto_calibrate(MIR_HANDLE handle, int xyz[3])
{
	...
	MI_MSG("---calibrate_Done, x = %d, y = %d, z = %d, off_x = %d, off_y = %d, off_z = %d",
...
static int SOCLE_calibrate(MIR_HANDLE handle, int z_dir)
{
	...
	if (mir3da_register_read_continuously(handle, (SOCLE_REG_OTP_XOFF_L | 0x80), 6,
					      tmp_data)) {
		MI_ERR("i2c block read failed\n");
		goto fail_exit;
	}
...
static int SOCLE_interrupt_ops(MIR_HANDLE handle, mir_int_ops_t *ops)
{
	...
	case INTERRUPT_OP_INIT:
		...
		/* active level */
		mir3da_register_mask_write(handle, SOCLE_REG_CTRL_REG6, (1 << 1),
					   (ops->data.init.level << 1));
		/* latch */
		mir3da_register_mask_write(handle, SOCLE_REG_CTRL_REG5, (1 << 3),
					   (ops->data.init.latch << 3));
		mir3da_register_mask_write(handle, SOCLE_REG_CLICK_CFG, (1 << 6),
	...
	case INTERRUPT_OP_CONFIG:
		...
		case INTERRUPT_ACTIVITY:
			...
			if (ops->data.cfg.pin == INTERRUPT_PIN1) {
				...
				mir3da_register_mask_write(handle, SOCLE_REG_INT1_THS, 0x7f,
							   ops->data.cfg.int_cfg.act.threshold);
				mir3da_register_mask_write(handle, SOCLE_REG_INT1_DURATION, 0x7f,
				...
				mir3da_register_mask_write(handle, SOCLE_REG_INT2_THS, 0x7f,
							   ops->data.cfg.int_cfg.act.threshold);
				mir3da_register_mask_write(handle, SOCLE_REG_INT2_DURATION, 0x7f,
							   ops->data.cfg.int_cfg.act.duration);
			}
			break;
		...
		case INTERRUPT_CLICK:
			...
			mir3da_register_mask_write(handle, SOCLE_REG_CLICK_THS, 0x7f,
						   ops->data.cfg.int_cfg.clk.threshold);
			mir3da_register_mask_write(handle, SOCLE_REG_TIME_LIMIT, 0x7f,
						   ops->data.cfg.int_cfg.clk.click_time);
			mir3da_register_write(handle, SOCLE_REG_TIME_LATENCY,
...
static int NSA_interrupt_ops(MIR_HANDLE handle, mir_int_ops_t *ops)
{
	...
	case INTERRUPT_OP_INIT:
		...
		mir3da_register_mask_write(handle, NSA_REG_INT_PIN_CONFIG, 0x0f,
					   ops->data.init.level | (ops->data.init.pin_mod << 1) | (ops->data.init.level <<
							   2) | (ops->data.init.pin_mod << 3));
	...
	case INTERRUPT_OP_CONFIG:
		...
		case INTERRUPT_ACTIVITY:
			...
			mir3da_register_write(handle, NSA_REG_ACTIVE_THRESHOLD,
					      ops->data.cfg.int_cfg.act.threshold);
			mir3da_register_mask_write(handle, NSA_REG_ACTIVE_DURATION, 0x03,
						   ops->data.cfg.int_cfg.act.duration);

			/* Int mapping */
			if (ops->data.cfg.pin == INTERRUPT_PIN1) {
				mir3da_register_mask_write(handle, NSA_REG_INTERRUPT_MAPPING1, (1 << 2),
							   (1 << 2));
			} else if (ops->data.cfg.pin == INTERRUPT_PIN2) {
				printk(" gensor int2  open \n");
				mir3da_register_mask_write(handle, NSA_REG_INTERRUPT_MAPPING3, (1 << 2),
							   (1 << 2));
			}
			break;
		...
		case INTERRUPT_CLICK:
			...
			mir3da_register_mask_write(handle, NSA_REG_TAP_THRESHOLD, 0x1f,
						   ops->data.cfg.int_cfg.clk.threshold);
...
int mir3da_get_enable(MIR_HANDLE handle, char *enable)
{
	...
	res = mir3da_register_read(handle,
				   mir3da_gsensor_drv.obj[gsensor_mod].power.addr, &reg_data);
	if (res != 0) {
		return res;
	}
...
int mir3da_set_enable(MIR_HANDLE handle, char enable)
{
	...
	res = mir3da_register_mask_write(handle,
					 mir3da_gsensor_drv.obj[gsensor_mod].power.addr,
...
int mir3da_set_odr(MIR_HANDLE handle, int delay)
{
	...
	res = mir3da_register_mask_write(handle,
					 mir3da_gsensor_drv.obj[gsensor_mod].odr_sect[odr].addr,
...
static int mir3da_soft_reset(MIR_HANDLE handle)
{
	...
	res = mir3da_register_mask_write(handle,
					 mir3da_gsensor_drv.obj[gsensor_mod].soft_reset.addr,
...
int mir3da_open_interrupt(MIR_HANDLE handle, int num, int on)
{
	...
	res |=  mir3da_register_mask_write(handle, NSA_REG_INT_LATCH, 0x8F, 0x83);
	...
	mir3da_register_read(handle, NSA_REG_INT_LATCH, &rvalue);
	printk("NSA_REG_INT_LATCH(0x%x)=0x%x\n", NSA_REG_INT_LATCH, rvalue);

	mir3da_register_read(handle, NSA_REG_ACTIVE_DURATION, &rvalue);
	printk("NSA_REG_ACTIVE_DURATION(0x%x)=0x%x\n", NSA_REG_ACTIVE_DURATION, rvalue);

	mir3da_register_read(handle, NSA_REG_ACTIVE_THRESHOLD, &rvalue);
	printk("NSA_REG_ACTIVE_THRESHOLD(0x%x)=0x%x\n", NSA_REG_ACTIVE_THRESHOLD,
	       rvalue);

	/* pull high start matchine, pull low close mtachine *
	* level high for pin INT: default is low high       */
	if (on) {
		printk("NSA_REG_ACTIVE_THRESHOLD on\n");
#if 0
		mir3da_register_write(handle, NSA_REG_INT_LATCH, 0x03);// 0x07
		mir3da_register_write(handle, NSA_REG_INTERRUPT_SETTINGS1, 0x20);
		mir3da_register_write(handle, NSA_REG_INTERRUPT_MAPPING1, 0x20);
		mir3da_register_write(handle, NSA_REG_TAP_THRESHOLD, 0x03);
		...
		mir3da_register_write(handle, NSA_REG_INTERRUPT_MAPPING1, 0x20);
		mir3da_register_write(handle, NSA_REG_TAP_THRESHOLD, 0x03);
		mir3da_register_write(handle, NSA_REG_INT_PIN_CONFIG, 0x04);
#endif
	} else {
		printk("NSA_REG_ACTIVE_THRESHOLD off\n");
		mir3da_register_write(handle, NSA_REG_INT_LATCH, 0x00);
		mir3da_register_write(handle, NSA_REG_INTERRUPT_SETTINGS1, 0x00);
		mir3da_register_write(handle, NSA_REG_INTERRUPT_MAPPING1, 0x00);
		mir3da_register_write(handle, NSA_REG_TAP_THRESHOLD, 0x10);
		mir3da_register_write(handle, NSA_REG_INT_PIN_CONFIG, 0x04);
	}

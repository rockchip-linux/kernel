...
static void vtl_ts_free_gpio(void)
{
	...
	gpio_free(ts->config_info.rst_gpio);	
}
#endif
...
void vtl_ts_hw_reset(void)
{
	...
	//gpiod_set_value_cansleep(ts->config_info.rst_gpio, 1);
	//msleep(10);
	gpiod_set_value_cansleep(ts->config_info.rst_gpio, 0);
	msleep(50);
	gpiod_set_value_cansleep(ts->config_info.rst_gpio, 1);
	//msleep(250);
	msleep(5);
...
static void vtl_ts_report_xy_coord(struct ts_info *ts)
{
	...
	//	}
			...
			input_report_abs(input_dev, ABS_MT_PRESSURE, 125);
...
static int vtl_ts_init_input_dev(struct ts_info *ts)
{
	...
	__set_bit(BTN_TOUCH, input_dev->keybit);//20130923
...
int vtl_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ts->config_info.rst_gpio = devm_gpiod_get(&client->dev, "vtl,reset");
	if (IS_ERR(ts->config_info.rst_gpio)) {
		...
		err = gpiod_direction_output(ts->config_info.rst_gpio, 1);
		if (err) {
			dev_err(&client->dev,
	...
	ts->config_info.irq_number = client->irq;

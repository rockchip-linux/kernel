...
static void gt3673_irq_enable(struct gsl_ts *ts)
{
...
static void gt3673_irq_disable(struct gsl_ts *ts)
{
	unsigned long irqflags;
	spin_lock_irqsave(&ts->irq_lock, irqflags);
	if (!ts->irq_is_disable) {

...
static s8 gtp_request_irq(struct goodix_ts_data *ts)
{
	...
	if (ts->irq) {
		...
		ret = devm_request_threaded_irq(&(ts->client->dev), ts->irq, NULL, 
			goodix_ts_irq_handler, /*ts->irq_flags | IRQF_ONESHOT */irq_table[ts->int_trigger_type]| IRQF_ONESHOT,
			ts->client->name, ts);
		if (ret != 0) {
...
static s32 gtp_main_clk_proc(struct goodix_ts_data *ts)
{
	...
	#if GTP_USE_FIXED_CLK
	//here check kernel /data is not same as android /data, so modify it to reduce fw download
	ret = gtp_verify_main_clk(p_main_clk);
	if (FAIL == ret) {
		GTP_ERROR("main clock data verify error");
	} else {
		GTP_INFO("main clock data used fix num, main clock freq: %d", p_main_clk[0]);
